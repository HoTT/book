...
The type theory presented here is a version of Martin-L\"{o}f's type theory~\cite{martin-lof:bibliopolis}

TODO: give original references

 that differs in several important ways from other formulations of the theory such as those that underly NuPRL~\cite{constable+86nuprl-book}, Coq~\cite{bertot+10coq}, and Agda~\cite{agda-citation-needed}.

Most significantly, the type theory described here is based on the \emph{intensional} version of Martin-L\"{o}f's type theory in which judgmental equality is \emph{analytic} in the sense that it is self-evident, whereas propositional equality, expressed by the identity type, is \emph{synthetic} in that it requires evidence in support of an equation.

TODO: Give references for original formulations of intensional vs. extensional MLTT.

The alternative, called the \emph{extensional} version of type theory, makes no distinction between judgmental and propositional equality (more precisely, inhabitation of the identity type has the same force as judgmental equality).  From the perspective of homotopy theory, extensional theories restrict types to sets. 

TODO: say something about K and UIP and give references to systems that use them.
...
Another point of variation is that we do not include a type of propositions, as is done, for example, in Coq, but rather identify propositions with types.

TODO: as in some reference to ML.  And give some more history of props as types.

%As a result, the elements of a proposition such as identity type may have a rich, higher-dimensional structure, giving rise to the homotopy-theoretic interpretation developed here.  
%The concept of \emph{proof irrelevance}, which degenerates the structure of a type to having at most one element, is expressed here through the general concept of $n$-truncation, which is described in \autoref{cha:hlevels}.  In particular, a \emph{mere proposition} is a type that is $(-1)$-truncated, which means that its elements, if any, are collapsed to a point.

Apart from the two judgments $a : A$ and $a \jdeq b : A$, some type theories also have judgments expressing the fact that something is a type and that two types are equal. In our system these are expressed as $A : \type$ and $A \jdeq B : \type$, respectively. In particular, a family of types indexed by a type~$A$ is expressed as a function $A \to \type$, rather than being taken as a primitive notion. A related point is that we regard universes as types whose elements are themselves types (``Russell-style universes", in the terminology of \cite{??}), rather than codes for types (``Tarski-style universes", in the terminology of \cite{??}), and we treat the universe hierarchy as cumulative in that every type in $\UU_i$ is also in $\UU_j$ for each $j\geq i$.

Another departure from some other intensional type theories is that equality of functions is taken to be \emph{extensional}, see \autoref{axiom:funext} for the exact statement. While we include function extensionalty explicitly as an axiom, it also follows from the Univalence axiom and $\eta$-conversion, see \autoref{sec:univalence-implies-funext}, as well as from the existence of the interval, see \autoref{thm:interval-funext}.

TODO: See \cite{PMvML} for the equivalence of the Paulin-Mohring Rule with the original Martin-L\"of rule.
