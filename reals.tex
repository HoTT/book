...
stands for~\eqref{eq:RC-paths} and construction of paths. These abuses of notation are
mitigated by the fact that we are omitting mere propositions. The mysterious $\theta$'s
appearing in the definition are there to give extra ``wiggle room'' which ensures that
$\sim$ behaves like $<$ and not like $\leq$.
...
\begin{proof}
  ...
  \item The second statement has a number of cases, of which we show a representative
    ...
    %
    Suppose $\rceq(\rclim(x), \rclim(y)) : \rclim(x) = \rclim(y)$ and $\rcrat(q)
...
\begin{proof}
  ...
  construction, therefore $\RC$ is a set by Theorem~\ref{egbert-lemma}.
\end{proof}
