...
\begin{thm}\label{thm:h-set-refrel-in-paths-sets}
  ...
  Then $R(x,y)$ is equivalent to $\id[X]{x}{y}$, and thus $X$ is a set.
\end{thm}

\begin{proof}
  Let $\rho : \prd{x:X} R(x,x)$ be a witness for reflexivity of $R$ and let $f
  : \prd{x,y:X} R(x,y) \to \id[X]{x}{y}$ be a witness that $R$ implies
  identity. To show that each $f(x,y) : R(x,y) \to \id[X]{x}{y}$ is an
  equivalence, it suffices, by Lemma \ref{thm:total-fiber-equiv}, to show that
  $f$ induces an equivalence of total spaces:
  \begin{equation*}
    \eqv{\sm{y:X}R(x,y)}{\sm{y:X}\id[X]{x}{y}}.
  \end{equation*}
  By Lemma \ref{thm:contr-paths}, the type on the right is contractible, so it
  suffices to show that the type of the left is contractible. As the center of
  ...
  \end{equation*}
  Since $R(x,y)$ is a mere proposition, by Theorem \ref{thm:path-sigma} it suffices to show that
  \begin{equation*}
    \id[X]{x}{y},
  \end{equation*}
  which we get from $f(H)$.
\end{proof}
