...
The basic judgment of type theory, analogous to ``$A$ has a proof'', is writen ``$a:A$'' and pronounced as ``the term $a$ has type $A$'', or more loosely ``$a$ is an element of $A$'' (or, in homotopy type theory, a ``point'' of $A$).
...
inconsistent (every type is inhabited). Instead we introduce a
