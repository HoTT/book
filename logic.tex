...
\begin{align*}
  ...
  % \prd{A:\UU}(\neg\neg A \to A).
\end{align*}
For emphasis, the proper version~\eqref{eq:lem} % and~\eqref{eq:ldn}
may be denoted $\LEM{-1}$; % and $\mathsf{DN}_{-1}$;
see also \autoref{ex:lemnm}.
Although $\LEM{}$ % and $\mathsf{DN}$
...
More generally, $\LEM{}$ implies that all types have \emph{decidable mere equality}, i.e.\ every $A:\UU$ satisfies
\[ \prd{a,b:A} (\brck{a=b} + \neg\brck{a=b}). \]
...
\end{itemize}
The \define{axiom of choice} $\choice{}$ asserts that under these assumptions,
