...
\begin{equation*}
\sm{h:\im(f)\to X}{L:h\circ\tilde{f}\sim g} h\circ H\sim K
\end{equation*}
...
\end{equation*}
An immediate consequence of the way we constructed our function $h:\im(f)\to X$ is that $h\circ\tilde{f}\sim g$. The result follows
now from the observation that the space
\begin{equation*}
\sm{h^\prime:\im(f)\to X} h^\prime\circ\tilde{f}\sim h\circ\tilde{f}
\end{equation*}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Axioms of choice implies excluded middle}
...
Indeed, we have
$\pairr{\bfalse,\refl{\north}} : \hfiber{f}{\north}$
and $\pairr{\btrue,\refl{\south}}\rangle :\hfiber{f}{\south}$.
...
\begin{proof}
  ...
  \]
  %
  where in the step excluded middle is available as a consequence of the axiom of choice.
...
\begin{proof}
  We leave the proof of well-pointedness as an exercise for the reader, the rest has been established.
  Note that the claims in the theorem are known as Lawvere's axioms for the \emph{Elementary Theory of the Category of Sets}.
\end{proof}
