...
\begin{lem}\label{thm:trunc-htpy}
  Given $f,g:A\to B$ and a homotopy $h:f\htpy g$, there is an induced homotopy $\trunc n h : \trunc n f \htpy \trunc n g$.
\end{lem}
\begin{proof}
  By function extensionality, we have $\funext(h):f=g$.
  Now applying the function $\trunc n- : (A\to B) \to (\trunc nA \to \trunc nB)$, we get
  \[\apfunc{\trunc n-}(\funext(h)):\trunc nf=\trunc ng\]
  ...
  % and then
  Passing back across function extensionality, we have
  \[\trunc nh\defeq\happly(\apfunc{\trunc n-}(\funext(h))): \trunc nf \htpy \trunc ng. \qedhere \]
...
\begin{thm}\label{cor:trunc-prod}
  For any types $A$ and $B$, the induced map $\trunc -{A\times B} \to \trunc nA \times \trunc nB$ is an equivalence.
\end{thm}
\begin{proof}
...
There is a lot of bookkeeping in the proof to deal with higher homotopies, but the idea is simply an adaptation of the standard categorical fact that left adjoints (such as reflectors) preserve colimits.
The reader willing to accept that this standard argument works in our context can freely skip this section.
...
(TODO: Fix up notation to use $\trunc n-$ rather than $\reflect$.)
...
\begin{defn}
  Let
  \[\Ddiag=\quad\vcenter{\xymatrix{C \ar^g[r] \ar_f[d] & B \\ A & }}\]
  be a span in $\type$. We denote by $\reflect(\Ddiag)$ the following
  span in $\P$:
...
\begin{thm}
  ...
  Then $(\reflect(D),\reflect(c))$ is a pushout of $\reflect(\Ddiag)$ in $n$-types.
\end{thm}
\egroup
...
\def\reflect(#1){\trunc n{#1}}
  ...
  proving that it is a composite of five equivalences, as shown.

  The first equivalence, $f_1$, is just \autoref{thm:trunc-reflective}, since $E$ is an $n$-type.

  The second equivalence, $f_2$, comes from the universal property of $(D,c)$, as a pushout of $\Ddiag$:
  \[f_2(u) \defeq (u\circ{}i,u\circ{}j,\mapfunc{u}\circ{}h).\]

  The third equivalence, $f_3$, is defined by
  \[f_3(i,j,h) \defeq (i,j,\funext(h))\]
  which is an equivalence by function extensionality.

  The fourth equivalence, $f_4$, comes from the universal property of truncation applied three
  times, and the fact that pullbacks are invariant under equivalence (as is everything, by univalence):
  ...
  (implicitly) concatenate with the path
  $\ext(i\circ{}f)=\ext(i)\circ\reflect(f)$ and the same for $j$ and $g$.
  We will see later that these do not get in the way.

  The fifth equivalence, $f_5$, is just like $f_3$ in reverse:
...
\section{Truncated and connected functions}
\label{sec:connectivity}

TODO: change this section and the next to refer to $n$-truncation and $n$-connectedness rather than a modality $\modal$.
...
We now show that the $n$-connected functions and the $n$-truncated functions form an orthogonal factorization system.
