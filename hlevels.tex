...
The following theorem will see how we may detect the homotopy level of a type using spheres. The basic idea is that if a type $A$ has
homotopy level $n+1$, then all the maps from $\Sn^n$ to $A$ are null-homotopic. 

\begin{defn}
A function $f:X\to A$ is said to be \emph{null-homotopic} if there is a term of type
\begin{equation*}
\mathsf{hNull}(f):=\sum(a:A),\ f=(\lambda x.a)\big. .
\end{equation*}
\end{defn}
...
\begin{proof}
Suppose we have a term $H:\sum(a:A),\ f=\lambda x.a$. A proof by path induction on $p:x= y$ reveals that there is a homotopy
\begin{equation*}
\apfunc{f}(x,y)=\lambda p.(\pi_2 H(y))^{-1}\bullet(\pi_2 H(x)).
\end{equation*}
\end{proof}

\begin{comment}
\begin{defn}
We define the $-1$-sphere $\Sn^{-1}$ to be $\varnothing$ and for each $n:\N_{\geq-1}$ we define
$\Sn^{n+1}:=\susp(\Sn^n)$, i.e.\ the $n+1$-sphere $\Sn^{n+1}$ is the suspension of the $n$-sphere $\Sn^n$.
We refer to the two points $\mathsf{unit}\to\Sn^{n}$ as the north pole $\north^{n}$ and the south pole $\south^n$. The basic
constructor of $\Sn^{n+1}$ of type
\begin{equation*}
\prod(x:\Sn^n),\ \mathsf{n}^{n+1}=\Sn^{n+1}
\end{equation*}
is called $\merid^{n}$ for meridian.
\end{defn}
\end{comment}

\begin{thm}\label{thm:sphere_n_hnull_to_hlevel_sn}
For every $n:\N$, a type $A$ has homotopy level $n+1$ if all the functions from $\Sn^n$ to $A$ are null-homotopic.
\end{thm}

\begin{proof}
The exact statement which we will prove is
\begin{equation*}
\prod(n:\N)(A:\type),\ \big(\prod(f:\Sn^n\to A),\ \mathsf{hNull}(f)\big)\to\mathsf{isLevel}(n+1,A).
\end{equation*}
The proof is by induction on $n$, so we require that $A$ is in a universe. In the case $n\equiv 0$ we have that
\begin{equation*}
\big(\prod(f:\Sn^{0}\to A),\ \mathsf{hNull}(f)\big)\simeq\prod(x,y:A),\ x= y,
\end{equation*}
so we see immediately that a type $A$ is a proposition whenever every function $f:\Sn^0\to A$ is null-homotopic.

Suppose that there is a term of type
\begin{equation*}
\prod(A:\type),\ \big(\prod(f:\Sn^n\to A)(t:\Sn^n),\ f(t)= f(\north^n)\big)\to\mathsf{isLevel}(n+1,A).
\end{equation*}
and let $A$ be a type and let 
\begin{equation*}
H:\prod(f:\Sn^{n+1}\to A)(t:\Sn^{n+1}),\ f(t)= f(\north^{n+1}).
\end{equation*}
We wish to show that $A$ has homotopy level $n+2$, which we will do by showing that $x= y$ has homotopy level $n+1$ for each $x,y:A$.
Suppose that $x,y:A$, by the induction hypothesis it suffices to show that every function $\Sn^n\to (x= y)$ is null-homotopic.
We have that $\Sn^n\to(x= y)$ is equivalent to $\sum(f:S^{n+1}\to A),\
(f(\north^{n+1})= x)\times(f(\south^{n+1})= y)$. Suppose that $f:\Sn^n\to A$ is a function and suppose that
$\tilde{f}$ corresponds to $f$ via the indicated equivalence. Then $\tilde{f}$ is null-homotopic and hence $\lambda
t.\tilde{f}(\merid^n(t))$ is null-homotopic (by \autoref{lem:hnull_to_map_hnull}). It follows that $f$ is null-homotopic.
\end{proof}
...
The idea is to make use of \autoref{thm:ntype-nloop}, which states that $A$ is an $n$-types just when $\Omega^{n+1}(A,a)$ is contractible
for all $a:A$, and \autoref{lem:susp-loop-adj}, which implies that $\Omega^{n+1}(A,a) \simeq \Map_*(\Sn^{n+1},(A,a))$, where $\Sn^{n+1}$ is
...
$r:\Sn^n \to \trunc n A$ and each $x:\Sn^n$, a path $s_r(x):r(x) = r(\base)$.
...
\item for each $r:\Sn^n \to \trunc n A$, a point $h(r):\trunc n A$, and
\item for each $r:\Sn^n \to \trunc n A$ and each $x:\Sn^n$, a path $s_r(x):r(x) = h(r)$.
\end{itemize}
...
\end{lem}
\begin{proof}
  We must show $\Omega^{n+1}(\trunc nA,b)$ is contractible for all $b:\trunc nA$, which by \autoref{lem:susp-loop-adj} is equivalent to
$\Map_*(\Sn^{n+1},(\trunc nA,b))$.
  As center of contraction for the latter, we choose the function $c_b:\Sn^{n+1} \to \trunc nA$ which is constant at $b$, together with
$\refl b : c_b(\base) = b$.

  Now, an arbitrary element of $\Map_*(\Sn^{n+1},(\trunc nA,b))$ consists of a map $r:\Sn^n \to \trunc n A$ together with a path
$p:r(\base)=b$.
  By function extensionality, to show $r = c_b$ it suffices to give, for each $x:\Sn^n$, a path $r(x)=c_b(x) \jdeq b$.
...
\item For each $r:\Sn^n \to \trunc n A$ and $r':\prd{x:\Sn^n}, P(r(x))$, an element $h'(r,r'):P(h(r))$.
\item For each $r:\Sn^n \to \trunc n A$ and $r':\prd{x:\Sn^n}, P(r(x))$, and each $x:\Sn^n$, a dependent path
...
\begin{proof}
  ...
  Given $r:\Sn^n \to \trunc n A$ and $r':\prd{x:\Sn^n}, P(r(x))$, we have $h(r):\trunc n A$ and $s_r :\prd{x:\Sn^n} (r(x) = h(r))$.
  Define $t:\Sn^n \to P(h(r))$ by $t(x) \defeq \trans{s_r(x)}{r'(x)}$.
  Then since $P(h(r))$ is $n$-truncated, there exists a point $u:P(h(r))$ and a contraction $v:\prd{x:\Sn^n} (t(x) = u)$.
...
TODO: Should probably remark here on how a reflective subuniverse is closed under various things, like the retractions, the terminal object,
finite products, arbitrary dependent products, and path spaces.
...
Of course, we showed in \autoref{thm:pushout-ump} that pushouts exist when $\P$ is \type itself, by giving a direct construction in terms of
