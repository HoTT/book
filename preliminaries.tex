...
just the case where $x$ is $a$ and $p$ is $\refl{a} : a = a$. Logically, this says that the family of types $a =_A x$ is generated by the element $\refl{a} : a = a$.  Homotopically, it expresses the fact that the space of paths from a chosen point to an arbitrary endpoint is contractible to the constant loop on the chosen point.
