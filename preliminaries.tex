...
In general, it is not even clear what those cases would be, but in the special case where $C : (\id[A]{a}{a}) \to \UU$, one might expect that to prove $C(p)$ for all $p$, it would suffice to prove $C(\refl{a})$.
This principle is \textbf{not} provable in homotopy type theory.  Using it, one can easily prove that any $p : \id[A]{a}{a}$ is equal to $\refl{a}$.  This and the univalence axiom create a contradiction, because univalence allows one to create unequal loops.
