...
We then have the following computation rule\indexdef{computation rule!for function types}\footnote{Use of this rule is often referred to as \define{$\beta$-conversion}
...
\[ f \jdeq (\lam{x} f(x)). \]
This equality is the \define{uniqueness principlefor function types}\indexdef{uniqueness!principle!for function types}, because it shows that $f$ is uniquely determined by its values.
...
$\UU_i$. We usually want to avoid mentioning the level\indexdef{universe level}%
...
\begin{rmk}\label{rmk:introducing-new-concepts}
  There is a general pattern for introduction of a new primitive concept in type theory, and because products are our second example following this pattern,\footnote{The description of universes above is an   exception.} it is worth emphasizing the general form:
  To specify a type, we specify
  \begin{enumerate}
  \item how to construct elements of that type.  These are called the   type's \emph{constructors}.  For example, the function type has one   constructor, \emph{$\lambda$-abstraction}.\index{lambda abstraction@$\lambda$-abstraction}
    (Recall that a direct definition like $f(x)\defeq 2x$ can equivalently be phrased as a $\lambda$-abstraction $f\defeq \lam{x} 2x$.)
  ...
  \item 
    a \define{computation rule}\indexdef{computation rule}\footnote{also referred to as \define{$\beta$-reduction}\indexdef{$\beta$-reduction}}, which expresses how an eliminator acts on a constructor.
    For example, for functions, the computation rule states that $(\lamu{x:A}b)(a)$ is judgmentally equal to the substitution of $a$ for $x$ in $b$.
  \item 
    an optional \define{uniqueness principle}\indexdef{uniqueness!principle}\footnote{also referred to as \define{$\eta$-expansion}\indexdef{$\eta$-expansion}}, which expresses how constructors act on eliminators:
    it would say that every element of the type is uniquely determined by the results of applying eliminators to it, and can be reconstructed from those results by applying a constructor.
    ...
    (A uniqueness principle whose conclusion involves propositional equality rather than judgmental equality will be called a \define{propositional uniqueness principle}\indexdef{uniqueness!principle, propositional}\indexsee{propositional uniqueness principle}{uniqueness principle, propositional}, and is said to hold if it can be proven from the other rules for the type.)
  \end{enumerate}
See also \autoref{sec:more-formal-pi}.
\end{rmk}
...
For example, in this way we can prove a propositional uniqueness principle that says that every element of $A\times B$ is equal to a pair.
...
Induction enables us to \emph{prove} a propositional uniqueness principle that asserts that the only inhabitant of the
...
Indeed, this is the cornerstone of the homotopical interpretation, where we regard witnesses of $\id{a}{b}$ as \emph{paths}\indexdef{path} or \emph{equivalences} between $a$ and $b$ in the space $A$, with $\refl{a}$ being the
