...
looks like a helix projecting down onto the circle (see \autoref{fig:winding}).
This map $w$ sends each point on the helix to the point on the circle
...
The version of van Kampen's theorem proven in the previous section (\autoref{thm:naive-van-kampen}) applies to pushouts of arbitrary types, but in all the examples we considered, the types $A$, $B$, and $C$ were sets.
