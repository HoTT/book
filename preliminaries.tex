...
The key observation is that it is not the identity \emph{type} that is inductively defined, but the identity \emph{family}.  In particular, path induction says that the \emph{family} of types $(\id[A]{x}{y})_{x,y:A}$ is inductively defined by the elements of the form $\refl{x}$.  This means that to give an element of any other family $C(x,y,p)$ dependent on a \emph{generic} element $(x,y,p)$ of the identity family, it suffices to consider the cases of the form $(x,x,\refl{x})$.  In the homotopy interpretation, this says that
the type of triples $(x,y,p)$, where $x$ and $y$ are the endpoints of the path $p$, is inductively generated by the constant loops at each point $x$.

Similarly, based path induction says that the \emph{family} of types $(\id[A]{a}{y})_{y:A}$ is inductively defined by the element $\refl{a}$, so to give an element of any other family $C(y,p)$ dependent on a generic element $(y,p)$, it suffices to consider the cases of the form $(a,\refl{a})$.  Homotopically, this expresses the fact that the space of paths from a chosen point to an arbitrary endpoint is contractible to the constant loop on the chosen point.

Neither of these two principles provide a way to give an element of a family $C(p)$ where $p$ has two fixed endpoints $a$ and $b$.  In particular, for a family $C(p : \id[A]{a}{a})$ dependent on a loop, we \emph{cannot} apply path induction and consider only the case for $C(\refl{a})$, and consequently, we cannot prove
