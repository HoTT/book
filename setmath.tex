...
\begin{thm}
  ...
  $$C(y) \defeq \exis{z : V} z \in x \land y = r(z)$$ 
  is a $V$-set. 
  \end{enumerate}
\end{thm}
...
The rule of separation requires some preliminary considerations.  Recall that $V$ lives in the same universe $\UU'$ as the base universe $\UU$ from which it is built. Thus, a priori, the same is true for the identity types $v=_V w$ for any $v,w:V$.  A class $C: V\to \prop$ will be called \emph{separable} if all of its values lie in $\UU$.  

\begin{lem}\label{lem:sep}
If  $C: V \to \prop$ is a separable class, then for any $a:V$ the class $x\in a \wedge C(x)$ is a $V$-set.
\end{lem}

\begin{proof}
Given $a=\vset(A,f)$, let $A' = \exis{x:A}C(fx)$, and take $f' = f\circ i$, where $i : A' \to A$ is the obvious inclusion.  Then we claim that for $a' = \vset(A',f')$ we have $x\in a\wedge C(x) \Leftrightarrow x\in a'$.
\end{proof}
\input{ordcard}
