...
Because $\apfunc{(x \mapsto x)}$ is the identity function and $\apfunc{(x \mapsto c)}$ (where $c$ is a constant) is $x \mapsto\refl{c}$, \cref{cor:transport-path-prepost} is a special case.
