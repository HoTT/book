...
The point of the corrspondence is that in each case, the rules for constructing and using elements of the type on the right correspond to the rules for reasoning about the proposition on the left.
...
proposition  lives in a different, higher, universe than the
...
Just as we remarked in \S\ref{sec:pat} that the propositions-as-types versions of ``or'' and ``there exists'' can include nontrivial information, nothing prevents the type $a=b$ from also including nontrivial information.
...
We will return to this in \S\ref{cha:basics}; for now we focus on the basic rules for the identity type.
...
\begin{align*}
 \ind{=_A}^{ML} :  \dprd{C : \prd{x,y:A}\prd{p:x =_A y} \UU} 
...
%
\[ D:\prd{x,y:A}\prd{p:x =_A y} \UU_{i+1} \]
%
...
Let us also say something about \define{inequality}, which is negation of equality (of
course, it makes no sense to negate judgmental equality $\jdeq$ because judgments are not
subject to logical operations):
%
\begin{equation*}
...
are not unequal, that would be an application the classical law of double negation.
...
Show that if we define $\ind{A+B}$ as $A + B \defeq \sm{x:\bool} \rec{\bool}(\UU,A,B,x)$, then the definitional equalities stated for $\ind{A+B}$ in \S\ref{sec:coproduct-types} hold.
\end{ex}

\begin{ex}\label{ex:prod-via-bool}
Show that if we define $\ind{A\times B}$ as $A \times B \defeq \prd{x:\bool}\rec{\bool}(\UU,A,B,x)$, then the definitional equalities state for $\ind{A\times B}$ in \S\ref{sec:finite-product-types} hold propositionally (i.e.\ using equality types).
\end{ex}
