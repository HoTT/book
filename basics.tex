...
But because their types are different, it is often clearer to use the different notations $\apdfunc{f}$ and $\apfunc{f}$.
...
\end{proof}
\begin{proof}[Second proof] 
By induction, it suffices to find a term of type $(x,u)=(x,\trans{\refl{x}}{u})$. Note that $\trans{\refl{x}}{u}\defeq u$, so we really need to find a term of type $(x,u)=(x,u)$. Here we take the reflexivity term.
\end{proof}
...
More generally, we may consider groupoids or spaces that are \emph{equivalent} to ones of this sort; since everything we do is up to homotopy, it makes no difference.
...
The type \nat of natural numbers is also a set; we defer the proof of this fact to Chapter \S\ref{cha:hlevels}.
...
Operations such as $\transfib{P}{p}{-}$ and $\apfunc{f}$ and the notion of equivalence were first studied extensively in type theory by Voevodsky, using the proof assistant Coq.
He used a definition of ``equivalence'' which says that the homotopy fibers are all contractible.
In Chapter~\ref{cha:hlevels}, we will call this property of a function being \emph{$(-2)$-truncated}; it is equivalent to our notion of coherent equivalence.
The advantage of coherent equivalences is that they include explicitly all the data which we usually care about when \emph{using} an equivalence.
...
Finally, the univalence axiom is also due to Voevodsky.
