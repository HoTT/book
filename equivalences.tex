...
Before we embark on this journey, however, we begin with a little more explanation of why it is necessary.
...
In \S\ref{sec:quasi-inverses} we concluded that $\qinv(f)$ is equivalent to $\prd{x:A} (x=x)$ by discarding a contractible type.
...
The path lemmas in \S\ref{sec:computational} yield the following characterization of paths in fibers:
...
\begin{defn}
  Let $f:A\to B$.
  \begin{itemize}
  \item A function $g:B\ra A$ is a \textbf{left inverse} to $f$ if $g\circ f\htpy \idfunc[A]$.
...
\begin{proof}
  ...
  by \autoref{thm:equiv-compose-equiv} and \autoref{thm:equiv-iso-adj} and \autoref{thm:contr-hae}, it is contractible.
...
\end{thm}
\begin{proof}
  By \autoref{thm:contr-paths} it suffices to assume $f$ to be a half adjoint equivalence and show that $\ishae(f)$ is contractible.
  Now by rearranging $\Sigma$s, the type $\ishae(f)$ is equivalent to $\sm{u : \rinv(f)} \rcoh{f}{\proj{1}(u)}{\proj{2}(u)}$.
  But by Lemmas~\ref{lem:inv-hprop} and~\ref{lem:coh-hprop} and the fact that $\Sigma$ preserves contractibility, the latter type is also contractible.
\end{proof}
...
Using the language introduced in \S\ref{sec:hae}, we can restate the definition proposed in \S\ref{sec:basics-equivalences} as follows.
...
In \S\ref{sec:basics-equivalences} we proved that $\qinv(f)\to\biinv(f)$ and $\biinv(f)\to\qinv(f)$.
...
Note that this also fits the proposal made at the beginning of \S\ref{sec:hae}: we combine $g$ and $\eta$ into a contractible type and add an additional datum which combines with $\epsilon$ into a contractible type.
...
\end{thm}
\begin{proof}
  We have already established a logical equivalence ${\iscontr(f)} \leftrightarrow {\ishae(f)}$, and both are mere propositions (\autoref{thm:contr-hprop} and \autoref{thm:hae-hprop}).
