...
\begin{multline*}
  g : \dprd{w,w':\sm{x:A}P(x)} 
      \Big(\dsm{p:\proj{1}(w)=\proj{1}(w')}\trans{p}{\proj{2}(w)}=\proj{2}(w^\prime)\Big)
      \to
      \narrowbreak
      (w=w')
\end{multline*}
...
\begin{equation*}
\Big(\dsm{p:w_1 = w_1'}\trans{p}{w_2}=w_2^\prime\Big) \to ((w_1,w_2)=(w_1',w_2'))
\end{equation*}
...
\begin{multline*}\label{eq:suc-injective}
  (\suc(m)=\suc(n))
  \xrightarrow{\encode} \narrowbreak
  \code(\suc(m),\suc(n))
...
\begin{multline*}\label{eq:transport-semigroup-step1}
  \transfib{X \mapsto \semigroupstr{X}}{\ua(e)}{(m,a)} = \narrowbreak
  \begin{aligned}[t]
    \big(&\transfib{X \mapsto (X \to X \to X)}{\ua(e)}{m}, \\
     &\transfibf{(X,m) \mapsto \mathsf{Assoc}(X,m)}{(\pairpath(\ua(e),\refl{}))}{a}\big)
  \end{aligned}
\end{multline*}
...
%
\begin{multline*}
  \Transfib{X \mapsto X}{\ua(e)}{ m(\transfib{X \mapsto
      X}{\opp{\ua(e)}}{b_1}, \transfib{X \mapsto
      X}{\opp{\ua(e)}}{b_2}) }
\end{multline*}
%
