...
\sm{y:A} \id[A]{a}{y}
\]
This type classifies paths in $A$ packaged together with one endpoint ($y$), but with one endpoint ($a$) fixed.
From a logical perspective, the based path space can be thought of as a \define{singleton type}: it classifies elements of $A$ that are equal to $a$, and therefore, in a rough sense, the based path space contains only the element $a$. However, it is very important to observe the difference between this singleton type and a singleton set $\{a\}$, which has exactly one element $a$: The singleton type has elements $(b, p)$ where $p : \id[A] a b$.  Because of the homotopy interpretation of equality, the singleton type can have many different elements $b$ that are not strictly equal to $a$, but are only connected to it by a path/propositional equality. Nonetheless, the singleton type behaves like a singleton up to homotopy, in the sense that any two elements of it are identified by a path, i.e., it is contractible.  This follows from the second induction principle for the identity type, which states that the based path space of $A$ at $a$ is inductively generated by the element $(a,\refl{a})$.
