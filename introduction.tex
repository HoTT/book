...
\[
\mathsf{Iso}(A,B) \defeq\ \Sigma_{f : A\to B}\Sigma_{g : B\to A}\big((\Pi_{x:A}\, g(f(x)) = x) \times (\Pi_{y:B}\, f(g(y)) = y)\big)
\]
%
...
The point is that in all of the above examples, the version of the theory we give is the \emph{best} way we know how to do it, whether or not LEM and AC are available.
