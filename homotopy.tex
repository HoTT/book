...
\begin{thm}[Blakers-Massey Theorem]\label{Blakers-Massey}
  ...
  particular, $\trunc {i+j} {P(x,y)} = \trunc {i+j} {\id [X \sqcup^C Y]
    {(\inl(x))} {(\inr(y))}}$.
    \note{What are $P$, $\glue$, and $C(x,y)$?} % https://github.com/HoTT/book/issues/22
\end{thm}
...
% It's premature to call the covering provided by c the universal covering.
...
\begin{proof}
  ...
  {(y,y,\lamt{u:{\trunc{n+1}A}} \refl{y(\ttt)})}\]
...
\begin{cor}
  We have $\eqv{\pi_2(\Sn^2)}{\Z}$ (where the map is induced by the differential
  of the Hopf fibration) and $\eqv{\pi_k(\Sn^3)}{\pi_k(\Sn^2)}$ for
  every $k\ge3$, where the map is induced by the Hopf fibration (seen as a map
  from the total space $\Sn^3$ to the base space $\Sn^2$).
\end{cor}

We actually get the slightly more precise result that the Hopf fibration induces
an equivalence $\eqv{\Omega^2(\Sn^3)}{\Omega^2(\Sn^2)}$. In classical homotopy
theory it is enough to state the result about the homotopy groups because of
Whitehead’s theorem, but we do not expect Whitehead’s theorem to hold in
homotopy type theory, even for the spheres, so the result about the double loop
spaces is more precise.

We first start with a proposition explaining how to construct fibrations over
pushouts.

\begin{lem}
  Let $\Ddiag=A\stackrel f\longleftarrow C\stackrel g\to B$ be a span and assume
  that we have
  \begin{itemize}
  \item Two fibrations $E_A:A\to\type$ and $E_B:B\to\type$
  \item An equivalence between $E_A\circ f:C\to\type$ and $E_B\circ
    g:C\to\type$, ie $e:\prd{c:C}{\eqv{E_A(f(c))}{E_B(g(c))}}$
  \end{itemize}

  Then we can construct a fibration $E:A\sqcup^CB\to\type$ such that TODO
\end{lem}
...
TODO: Should remark on who did all the formalizations!
