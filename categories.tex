...
if $f:\hom_X(x,y)$, we have 
  \[ H_{\alpha\beta}(f)\to H_{\beta\gamma}(g)\ra H_{\alpha\gamma}(g\circ   f).\]\label{item:sigcmp}  
   \end{enumerate}
...
\comment{%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%We assume given a function $fsymbol :|\Omega|\ra\bbB$, where $\bbB$ is the %type of boolean values, having canonical elements $\bbt,\bbf$, so that if %$\omega :\Omega$ then we take
\begin{center}
...
Here
  \[ \begin{array}{ll}
 P_0x &:= \Pi_{\omega:\Omega_0}\; x^{|\omega|}\ra x,
\mbox{ and } \\
     P_1x &:= \Pi_{\omega:\Omega_1}\; x^{|\omega|}\ra \bbP_\bbU,
  \end{array}\]
...
\mbox{ and }\\
H_{1,\alpha\beta}(f) &:= 
  \forall_{\omega:\Omega_1}\forall_{u:x^{|\omega|}}\;\;\;\;\;\;\;\alpha u\ra\beta(f\circ u).
  \end{array}\]
