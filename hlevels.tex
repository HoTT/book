...
\begin{defn}\label{defn:decidable-equality}
 A type $X$ has \define{decidable equality} if for all $x, y : X$ we have
 \[(x =_X y) + \neg (x =_X y).\]
\end{defn}
...
\begin{ex}\label{ex:lemnm}
  For $-1\le n,m \le\infty$, let $\mathsf{LEM}_{n,m}$ denote the statement
  \[ \prd{A:\ntype{n}} \trunc m{A + \neg A},\]
  where $\ntype{\infty} \defeq \type$ and $\trunc{\infty}{X}\defeq X$.
  Show that:
  \begin{enumerate}
  \item If $n=-1$ or $m=-1$, then $\mathsf{LEM}_{n,m}$ is equivalent to $\mathsf{LEM}$ from \autoref{sec:intuitionism}.
  \item If $n\ge 0$ and $m\ge 0$, then $\mathsf{LEM}_{n,m}$ is inconsistent with univalence.
  \end{enumerate}
\end{ex}

\begin{ex}\label{ex:acnm}
  For $-1\le n,m\le\infty$, let $\mathsf{AC}_{n,m}$ denote the statement
  \[ \prd{X:\set}{Y:X\to\ntype{n}}
  ...
  Thus $\mathsf{AC}_{0,-1}$ is the axiom of choice from \autoref{sec:axiom-choice}, while $\mathsf{AC}_{\infty,\infty}$ is \autoref{thm:ttac}.
  It is known that $\mathsf{AC}_{\infty,-1}$ is consistent with univalence, since it holds in Voevodsky's simplicial model.
  \begin{enumerate}
  \item Without using univalence, show that $\mathsf{LEM}_{n,\infty}$ implies $\mathsf{AC}_{n,m}$ for all $m$.
    (On the other hand, in \autoref{subsec:emacinsets} we will show that $\mathsf{AC}=\mathsf{AC}_{0,-1}$ implies $\mathsf{LEM}=\mathsf{LEM}_{-1,-1}$.)
  \item Of course, $\mathsf{AC}_{n,m}\Rightarrow \mathsf{AC}_{k,m}$ if $k\le n$.
    Are there any other implications between the principles $\mathsf{AC}_{n,m}$?
    Is $\mathsf{AC}_{n,m}$ consistent with univalence for any $m\ge -1$?
...
\begin{ex}\label{ex:acconn}
  ...
  \end{quote}
  Note that the $(-1)$-connected axiom of choice is $\mathsf{AC}_{\infty,-1}$ from \autoref{ex:acnm}.
  \begin{enumerate}
  \item Prove that for every $n\ge -1$, the $n$-connected axiom of choice implies the $(n+1)$-connected axiom of choice.
  \item Are there any other implications between the $n$-connected axioms of choice and the principles $\mathsf{AC}_{n,m}$?
