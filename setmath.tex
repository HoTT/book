...
Also, $\|\sum(x:X),\ R(w,x)\|$ is inhabited for every $w:\mathsf{im}(f)$. Indeed, the space
\begin{equation*}
...
\end{equation*}
The space $\prod(a:A)\sum(x:X),\ R^\prime(\tilde{f}(a),x)$ is inhabited by
\begin{equation*}
...
\begin{equation*}
\mathsf{isEqClass}(R ,P)\defeq \sum(x:A)\prod(y:A),\ R (x,y)\simeq P(y).
\end{equation*}
\end{defn}
...
\end{lem}
\begin{proof}
Suppose that $\pairr{P,u}$ is an element of $\sum(P:\prop),\ P$. Then we have $u:P$ and hence there is a term of type $\mathsf{isContr}(P)$. It
...
One wonders what prevents $\set$ from begin a topos. We the lack impredicativity to define the subobject classifier. If we
...
A class $\smal:\prod\{X,Y:\type\},\ (X\to Y)\to\prop$ is \emph{stable} of if it satisfies:
\begin{description}
