...
By Univalence Axiom this is the same as proving the equality.
By the characterization of \cref{thm:path-sigma} (and using Univalence) we see
that it suffices to give a:
\[p:\eqv{\Parens{(a,b)=(a,b)}}{\Parens{(a=a)\times (b=b)}}\]

And to show that transporting $\refl{(a,b)}$ along $\ua(p)$ is equal to
$(\refl{a}, \refl{b})$.  To construct $p$ just use \cref{thm:path-prod}. To
show that transport respects the equality we use
the propositional computation rule in \cref{axiom:univalence} and the definition
...
is the diagram intended in he exercise.

Lets define the type $\Sn^{\infty}$ as the colimit of the diagram we just
...
And the induction:
\[
  \ind{\Sn^{\infty}} : \prd{C:\Sn^\infty \to \UU} \prd{J_{\blank} : \prd{n : \nat} \prd{x : \Sn^n} C(j_n(x))}
  \Parens{\prd{n:\nat} \prd{x:\Sn^n} \dpath C {\glue_n(x)} {J_n(x)} {J_{n+1}\circ i_n (x)}}
...
Each sphere is the ``equator'' of the next one. We say ``equator'' because it passes
trough $\north$ and $\south$ so technically you have to rotate the bigger sphere for
the inclusion to be exactly in the equator.
...
\end{tikzpicture}
\end{center}
we get a path $x \defeq j_n(y) = j_{n+1} \circ i_n(y)$.
But, as we will show, the image of $\Sn^n$ under $i_n$ is contractible,
every point in the image is equal to $\north_{n+1}$. Thus, we are able to show that
$j_{n+1}\circ i_n(y) = j_{n+1}(\north_{n+1})$. Composing the proof of the first step
...
Lets now show that the inclusion of $\Sn^n$ in $\Sn^{n+1}$ can be
continuously retracted to $\north_{n+1}$.
That is, lets construct a homotopy:
\[ H_{\blank} : \prd{n:\nat}\prd{x:\Sn^n} i_n(x) = \north_{n+1} \]
...
suspension induction $i_n(\merid_n(x))$ equals $\merid_{n+1} \circ i_{n-1}(x)$.
And, by inductive hypothesis, $i_{n-1}(x) = \north_n$, which gives us the
desired result.

Composing the two proofs we just gave we get a function:
\[ J_{\blank} \defeq \glue_n(x)\ct \apfunc{j_{n+1}}{H_n(x)}\ct D_{n+1}
...
\[ \prd{n:\nat} \prd{x:\Sn^n} \dpath {x\mapsto (x = j_0(\north_0))}
  {\glue_n(x)} {J_n(x)} {J_{n+1}\circ i_n (x)} \]
By definition this is:
\[ \prd{n:\nat} \prd{x:\Sn^n}
    \transfib{x\mapsto (x = j_0(\north_0))}{\glue_n(x)}{J_n(x)}
    = {J_{n+1}\circ i_n (x)} \]

The LHS is equal to $\glue_{n}(x)^{-1}\ct J_n(x)$, and, by definition of
$J_n(x)$, is just:
...
But we remeber that we have the homotopy
$\glue_{n+1} : j_{n+1} = j_{n+2}\circ i_{n+1}$, so, by a simple application
of \cref{lem:htpy-natural}, the functoriality of $\apfunc{}{}$, and some standard
cancelations, we reduce the goal to:
...
\[ \ind{\Sn^\infty} : \prd{C:\Sn^\infty \to \UU} \prd{n:C(\north)}\prd{s:C(\south)}
...
$c : \prd{y_1,y_2:Y}\trunc {} {y_1 = y_2}$.

We note that it suffices to show that for any $y_1,y_2:Y$ we have
$\trunc {} {\hfib{f}{y_1} = \hfib{f}{y_2}}$ because by Univalence
$\hfib{f}{y_1} = \hfib{f}{y_2}$ is equivalent to $\hfib{f}{y_1}\simeq \hfib{f}{y_2}$
and thus, by Univalence, they are equal.
