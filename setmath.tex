...
The second construction of quotients, which uses impredicativity\index{impredicative!quotient} of mere propositions, is as the set of equivalence classes of $R$ (a subset
...
\begin{proof}
  Let $\beta = 2^\alpha$.
  Now we want to show a mere proposition, so by induction we may assume $\alpha$ is $\cd{A}$, so that $\beta\jdeq \cd{A\to \mathbf{2}}$.
