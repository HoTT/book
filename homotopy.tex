...
\begin{thm}[The Freudenthal Suspension Theorem]
  ...
  Then the canonical map $\sigma:X\to \Omega\susp(X)$ is $2n$-connected.
\end{thm}
\begin{proof}
  ...
  \[\code:\prd{w:\susp X} (\north=w) \to \type\] such that $\code(\north,p) \jdeq \trunc{2n}{\hfib{\sigma}{p}}$.
  If we can show that $\code(w,p)$ is contractible for all $w:\susp X$ and $p:\north=w$, then applying this with $w\defeq \north$ will prove that all fibers of $\sigma$ are $2n$-connected, and thus $\sigma$ is $2n$-connected.
