...
Let $P:A\to\prop$. Define $\mathsf{atMostOne}(P) \defeq \prd{x,y:A}P(x)\to P(y)\to x=y$.  If there is a term
$H:\mathsf{atMostOne}(P)$, then the type $\sm{x:A}P(x)$ is a proposition.
\end{lem}

\begin{proof}
Suppose that $\pairr{x,u}$ and $\pairr{x',u'}$ are terms of $\sm{x:A}P(x)$. Then we have the path $p\defeq
H(x,x',u,u'):x=x'$. Moreover, there is a path $\id{\trans{p}{u}}{u'}$ since $P(x')$ is assumed to be a
proposition.
\end{proof}
...
\end{equation*}
By \autoref{lem:atMostOne} the hypotheses of the principle of unique choice (\autoref{cor:UC}) are satisfied, so we get an element of
\begin{equation*}
