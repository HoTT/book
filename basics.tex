...
It can also be useful to consider the loop space\index{loop space!iterated} \emph{of} the loop space of $A$, which is the space of 2-dimensional loops on the identity loop at $a$.
...
In general, we will only use the word \emph{isomorphism} (and similar words such as \emph{bijection})
\index{bijection}
