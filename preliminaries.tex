...
Informally, there is only one type in set theory: set. According to set theory the natural number 2 is a set, the real exponential function is a set, and a line in euclidean space is also a set. All mathematical objects are interpreted as sets, except propositions and proofs.
To accomodate propositions, a set-theoretic foundation has two ``layers'': the deductive system of first-order logic,\index{first-order!logic} and, formulated inside this system, the axioms of a particular theory, such as ZFC.
...
By contrast, having different types allows to reintegrate propositions and proofs into the same layer as the other mathematical objects. The number 2 becomes of type ``natural number'', the Pythagorean theorem of type ``proposition'' and its proof of type ``Pythagorean theorem''. Therefore type theory is its own deductive system: it need not be formulated inside any superstructure, such as first-order logic.
