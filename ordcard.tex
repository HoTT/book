...
\autoref{thm:ordsucc,thm:ordunion} show that ``there are always more ordinals''.
However, sometimes one needs ordinals which are ``large'' in a stronger way than having large underlying sets.
Possibly the simplest condition of this sort is the following.

\begin{defn}
  Let $Y:X\to \set$ be a family of sets.  An ordinal $A$ is \textbf{$Y$-filtered} if for any $x:X$ and any $f:Y(x) \to A$, there merely exists an $a:A$ such that $f(y)<a$ for all $y:Y(x)$.
\end{defn}

Classically, filteredness is express in terms of \emph{cofinality}; see \autoref{ex:cofinality}.

\begin{eg}
  The ordinal $\omega$ is \bool-filtered (meaning $Y$-filtered for the singleton family containing only \bool).
  This just means that any two natural numbers have a common upper bound.
  On the other hand, $\omega$ is not \nat-filtered, since the identity family $\idfunc:\nat\to\omega$ has no upper bound.
\end{eg}

In classical set theory, the axiom of choice is necessary in order to show that $Y$-filtered ordinals exist for infinite $Y$.
This is also the case in traditional type theory, but with higher inductive types we can write down such ordinals fairly directly.

\begin{thm}\label{thm:enough-filtered-ordinals}
  For any family $Y:X\to \set_\UU$ of sets in a universe \UU, there exists a $Y$-filtered ordinal in \UU.
\end{thm}
\begin{proof}
  TODO: Waiting on \autoref{sec:cumulative-hierarchy}, as it is probably a special case or slight modification of what will be done there.
\end{proof}
...
The equivalence in \autoref{sec:surj-split} between $\mathsf{AC}$ and $\mathsf{LEM}_{\infty}$ is an adaptation to homotopy type theory of, first of all, a well-known theorem of Diaconescu and Bishop of the form $\mathsf{AC} \Rightarrow \mathsf{LEM}$.  The present strengthening to $\mathsf{AC} \Leftrightarrow \mathsf{LEM}_{\infty}$ appears to be folklore.
...
It is proven in~\cite{gitik:unc-sing} (assuming a large cardinal hypothesis) that it is consistent with ZF set theory (without choice) that all uncountable cardinals are singular.
Equivalently, this would mean that no ordinal has a cofinality greater than $\omega$, or that no ordinal is $Y$-filtered for any family $Y$ containing infinite sets.
Thus, \autoref{thm:enough-filtered-ordinals} implies that our type theory can say more about sets than ZF can.
It derives from the observation of~\cite{blass:freealg} that highly filtered ordinals can be constructed if we assume free algebras for all infinitary algebraic theories.
Indeed, the construction of \autoref{thm:enough-filtered-ordinals} is an instance of the general free-algebra construction described in \autoref{sec:free-algebras}.
...
\begin{ex}\label{ex:cofinality}
  If $B$ is an ordinal, a function $f:A\to B$ is said to be \textbf{cofinal} if it is unbounded, i.e.\ for any $b:B$ there merely exists $a:A$ such that $b<f(a)$.
  The set of ordinals $A$ which merely admit a cofinal map to $B$ is inhabited (since $\idfunc[B]$ is cofinal).
  Thus, if we assume excluded middle, it has a least element; this is called the \textbf{cofinality} of $B$.

  Prove (assuming excluded middle) that if $Y:X\to \ord$ is a family of ordinals, then $B$ is $Y$-filtered if and only if $Y(x)$ is (strictly) less than the cofinality of $B$ for all $x:X$.
\end{ex}
