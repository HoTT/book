...
\begin{thm}[Blakers-Massey Theorem]\label{Blakers-Massey}
  Suppose we are given maps $f : C  \rightarrow X$, and $g : C \rightarrow Y$. Taking first the homotopy pushout $X \sqcup^C Y $ of $f$ and $g$ and then the homotopy pullback of its inclusions $\inl : X \rightarrow X \sqcup^C Y \leftarrow Y : \inr$, we have an induced map $C \to X \times_{(X \sqcup^C Y)} Y$.

  If $f$ is $i$-connected and $g$ is $j$-connected, then this induced map is $(i+j)$-connected. In other words, for any points $x:X$, $y:Y$, the corresponding fiber $C_{x,y}$ of $(f,g)$ gives an approximation to the path space $\id{\inl(x)}{\inr(y)}$ in the homotopy pushout.
\end{thm}
...
$n-1$.  
\note{Gap between this and the induction principle for Z stated in the quotients section}
...
mentioned in \ref{hopf-fibration}:
...
necessarily valid in homotopy type theory (we will never use the more precise
version though).
...
\begin{lem}\label{lem:fibration-over-pushout}
  Let $\Ddiag=Y\stackrel j\longleftarrow X\stackrel k\to Z$ be a span and assume
  that we have
  \begin{itemize}
  ...
  \[\xymatrix{
    \sm{x:X}E_Y(j(x)) \ar[r]_\sim^{e_X} \ar[d]_j & \sm{x:X}E_Z(k(x)) \ar[r]^k
...
\begin{proof}
  ...
  and the action of $E$ on paths $\glue(x)$.
  \begin{align*}
    E(\inl(y)) & \defeq E_Y(y)\\
    E(\inr(z)) & \defeq E_Z(z)\\
...
The Hopf fibration is a special case of a construction which works for any
H-space.
...
\begin{proof}
  ...
  enough to find a point in the fiber of $P'$ over $\tproj0e$. We have
...
The \emph{Hopf construction} of a connected H-space $A$ will be a fibration over
$\susp A$ with fiber $A$ and total space the join $A*A$.
...
\begin{defn}
  ...
  We take $F_1\defeq A$, $F_2\defeq A$ and for $a:A$, $e(a)$ is the equivalence
  $\mu(a,-)$.
\end{defn}

According to \autoref{lem:fibration-over-pushout}, the total space of the
fibration we get is a pushout of the following span:

\[\xymatrix{A & A \times A \ar[l]_-{\proj2} \ar[r]^-{\mu} & A}\]

\emph{TODO: to continue}
...
\begin{eg}\label{universe-is-not-truncated}
  \note{TODO}                   % https://github.com/HoTT/book/issues/41
...
Hou developed the theory of covering spaces.  
