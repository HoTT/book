...
There is a general pattern to how we describe types, and since products are our second example following this pattern,\footnote{The way we have   described univeres above does not quite follow this pattern: the   elimination form for universes is essentially the ability to $A : \UU$   in the judgement $a : A$}, it is worth emphasizing the general form. When we specify a type, we say
\begin{enumerate}
\item how to construct elements of that type.  These are called the \emph{constructors}.  For example, for functions, the constructor is $\lambda$-abstraction.    
\item how to use elements of that type.  These are called the   \emph{eliminators}.  For example, for functions, the eliminator is   function application.
...
One important aspect of it is the principle that ``equals may be substituted for equals,'' as expressed by Leibniz's principle of \emph{indiscernability of identicals}:
\begin{description}
...
\]
\end{description}
This says that any construction $C$ represented by a family of types respects equality, in the sense that when $C$ is applied to equal elements of $A$, there is a coercion function between the resulting types. The defining equality states that coercion by reflexivity is the identity function.

Indiscernability of identicals is a recursion principle (recursor) for the identity type, analogous to those given for booleans and natural numbers above.  To fully characterize the identity type, we must generalize this to an induction principle, which not only allows equals to be substituted for equals, but gives information about the evidence for equality $p$.  There are two different, but equivalent, ways to formulate the induction principle.

\subsection{Path induction}

Under the homotopy interpretation, a type $A$ corresponds to a space, and the identity type of $A$ corresponds to a space of paths in $A$.  Based on this correspondence, we define the \define{path space} of $A$ to be the type
\[
\sm{x:A}{y:A} \id[A]{x}{y}
\]
The path space is the type of paths in $A$ packaged together with their endpoints.  The first induction principle for the identity type is called \emph{path induction}, and states that the path space is inductively generated by the elements of the form $(x,x,\refl{x})$:
...
Formally, we write $C$ as family with three arguments, $x$ and $y$ of type $A$ and $p$ of type $\id[A]{x}{y}$.  However, such a family is equivalent (by currying) to a one-argument family, whose argument is an element the path space ($\sm{x:A}{y:A} \id[A]{x}{y}$). Indeed, our notational convention of writing the application of the ternary family $C$ to three arguments as $C(x,y,p)$ suppresses the distinction.
...
We are interested in knowing when the proposition $C(x,y)$ holds for some $x,y:A$.
In this case, the hypothesis of path induction says that we know $C(x,x)$ holds for all $x:A$, i.e.\ that if we substitute the same element for both variables occurring in $C$, we get a true proposition.
...
The general form of the rule allows $C$ to depend on the witness $p:x=y$ as well.  In the premise, we can not only replace $y$ by $x$, but also simultaneously replace $p$ by reflexivity: to prove a property for all elements $(x,y,p)$ of the path space, it suffices to consider all the cases of the form $(x,x,\refl{x})$.  If we view types as like sets, it is unclear what this buys us, but in \autoref{cha:basics} we will see that it is essential to the homotopy interpretation, because it expresses that the path space of a type is inductively generated by the constant loops at each point.

If we package path induction into a function, it becomes:
\symlabel{defn:induction-ML-id}
\begin{align*}
  \indidtwo{A} :  \dprd{C : \prd{x,y:A}{p:x =_A y} \UU} 
  \Big(\tprd{x:A} C(x,x,\refl{x})\Big) \to  \dprd{x,y:A}{p:x =_A y}   C(x,y,p)
\end{align*}
with the equality
\[ \indidtwo{A}(C,c,x,x,\refl{x}) \defeq c(x) \]
This function is also called $J$.  In \cref{sec:fibrations}, we show that the recursor (indiscernability of identicals) follows from path induction.  

\subsection{Based path induction}

Given a proof $p : \id{a}{b}$, 
path induction allows you to equate $a$ and $b$ by replacing them \emph{both} with the same unknown element $x$: to prove a family
$C$ for all \emph{pairs} of elements of $A$, it suffices to prove it for the whole diagonal.
In some proofs, however, it is simpler to use an equation $p : \id{a}{b}$ by replacing all occurrences of $b$ with $a$ (or vice versa), because it is sometimes easier to do the remainder of the proof for the specific element $a$ mentioned in the equality than for a general unknown $x$.  This motivates a second induction principle for identity types.  As we show below, this second induction principle is equivalent to the first, but it is sometimes a more convenient packaging.

Given a type $A$ with an element $a:A$, we define the \define{based path space} of $A$ at $a$ to be the type
\[
\sm{y:A} \id[A]{a}{y}
\]
The elements of this type are paths in $A$ packaged together with one endpoint ($y$), but with one endpoint ($a$) fixed.
From a logical perspective, the based path space can be thought of as a \define{singleton type}: because its elements are the elements of $A$ that are equal to $a$, in a rough sense, it contains only the element $a$. However, it is very important to observe the difference between this singleton type and a singleton set $\{a\}$, which has exactly one element $a$: The singleton type has elements $(b, p)$ where $p : \id[A] a b$.  Because of the homotopy interpretation of equality, the singleton type can have many different elements $b$ that are not strictly equal to $a$, but are only connected to it by a path/propositional equality. Nonetheless, the singleton type behaves like a singleton up to homotopy, in the sense that any two elements of it are identified by a path, i.e., it is contractible.  This follows from the second induction principle for the identity type, which states that the based path space of $A$ at $a$ is inductively generated by the element $(a,\refl{a})$.
...
Formally, we write $C$ as a binary family, with arguments $y$ of type $A$ and $p$ of type $\id[A]{a}{y}$.  However, as we noted above for the path space, such a family is equivalent (by currying) to a unary family whose one argument is an element the based path space ($\sm{y:A} \id[A]{a}{y}$).

Based path induction says that we may prove a family $C$ for all elements of the based path space at $a$ in $A$ by covering the case for $(a,\refl{a})$.  Logically, this says that given an equality $\id{a}{b}$, we can substitute the occurrences of $b$ chosen by $C$ with $a$.  Homotopically, it expresses the that the space of paths from a chosen point to an arbitrary endpoint is inductively generated by the constant loop.

Packaged as a function, based path induction becomes:
\symlabel{defn:induction-PM-id}
\begin{align*}
  \indidone{A} :  \dprd{a:A}{C : \prd{x:A}{p : a =_A x} \UU}
  C(a,\refl{a}) \to \dprd{x:A}{p : a =_A x} C(x,p) 
\end{align*}
with the equality
\[ \indidone{A}(a,C,c,a,\refl{a}) \defeq c. \]
%\[ g(x)(x,\refl{x}) \defeq d(x) \]

Below, we show that path induction and based path induction are equivalent.  Because of this, we will sometimes be sloppy and refer to based path induction simply as ``path induction,'' relying on the reader to infer which principle is meant from the form of the proof.  

\subsection{Double-based path induction?}

One might wonder whether there is a also a ``double-based'' path induction principle, for elements of an identity type with \emph{both} sides fixed. Given a family
\[
C : (\id[A]{a}{b}) \to \UU
\]
is it possible to prove $C(p)$ for all $p$ by covering specific cases?

In general, it is not even clear what those cases would be, but in the special case where $C : \id{a}{a} \to \UU$, one might expect that to prove $C(p)$ for all $p$, it would suffice to prove $C(\refl{a})$.
However, this principle is \textbf{not} provable in homotopy type theory.  Indeed, using it, one can easily prove that any $p : \id{a}{a}$ is equal to $\refl{a}$, which is contradictory in the presence of univalence.  Thus, while path induction states that the path space is inductively generated (it is the least reflexive relation), and based path induction says that the based path space is inductively generated (it is the least predicate on $A$ that holds for $a$), there is no principle saying that the type of \emph{loops} is inductively generated.

\subsection{Equivalence of path induction and based path induction}

The two above induction principles for the identity type are equivalent.
...
Another proof of this fact is to observe that any such $f$ can be obtained as an instance of $\indidtwo{A}$,
so it suffices to define $\indidtwo{A}$ in terms of $\indidone{A}$ as
\[ \indidtwo{A}(C,c,x,y,p) \defeq \indidone{A}(x,C(x),c(x),y,p). \]

The other direction is a bit trickier: it is not clear how we can use a particular instance of the path induction to derive a particular instance of
based path induction. What we can do instead is construct one instance of path induction which shows 
...
Another proof is to observe that any use of based path induction is an instance of $\indidone{A}$, and to define
\begin{align*}
\indidone{A}(a,C,c,x,p) \defeq 
\indidtwo{A}( & \lamu{x,y:A}{p:x =_A y} \tprd{C : \prd{z:A}{p : x =_A z} \UU} C(x,\refl{x}) \to C(y,p),\\
...
model $\indidone{A}$ with $C : \prd{x:A}{p : a =_A x} \UU_i$, we need
to use $\indidtwo{A}$ with 
%
...
derive $\indidone{A}$ without using universes: we can show that $\indidtwo{A}$ entails \autoref{lem:transport,thm:contr-paths}, and that these two principles imply $\indidone{A}$ directly.
We leave the details to the reader as \autoref{ex:pm-to-ml}.

We can use either eliminator for equality, $\indidtwo{A}$ or $\indidone{A}$, to establish that equality is an equivalence relation, that every function preserves equality and that every family is stable under equality. We leave the details to the next chapter, where we will derive these principles and explain them in the context of homotopy type theory.
...
Give an alternative derivation of $\indidone{A}$ from $\indidtwo{A}$ which avoids the use of universes.
