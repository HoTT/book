...
Given types $A$ and $B$, we can construct the type $A \to B$ of functions
\indexdef{function}%
\indexsee{map}{function}%
with domain $A$ and codomain $B$.
\index{domain!of function}%
\index{codomain!of function}%
...
\indexdef{lambda abstraction@$\lambda$-abstraction}%
\indexdef{function!lambda abstraction@$\lambda$-abstraction}%
...
\indexfoot{eta conversion@$\eta$-conversion}%
\indexfoot{conversion!eta@$\eta$-}%
or \define{$\eta$-expansion.
\indexfoot{eta expansion@$\eta$-expansion}%
\indexfoot{expansion!eta@$\eta$-}%
}}
\[ f \jdeq (\lam{x} f(x)). \]
...
Previously, the substituted $y$ was referring to our assumption, but now it is referring to the parameter of the anonymous function. Hence, this naive substitution would destroy the binding structure, allowing us to perform calculations which are semantically unsound.
...
other variable, preserving the binding structure. Indeed, $\lamu{y:\nat} x + y$ is declared to be judgmentally equal\footnote{Use of this equality is called \define{$\alpha$-conversion.
...
\define{cumulative},
\indexdef{universe!cummulative}%
\indexdef{cummulative universes}%
...
\indexdef{cartesian product}%
\indexdef{type!cartesian product}%
...
\indexdef{recursor!for cartesian product}%
\indexdef{cartesian product!recursor for}%
for product types.
We may also speak of the \define{recursion principle}
\indexdef{recursion principle!for cartesian product}%
\indexdef{cartesian product!recursion principle for}%
...
\indexdef{cartesian product!induction principle for}%
\indexdef{induction principle!for cartesian product}%
...
\indexdef{cartesian product!dependent eliminator for}%
\indexdef{eliminator!dependent!for cartesian product}%
and recursion the \define{non-dependent eliminator}.
\indexdef{cartesian product!non-dependent eliminator for}%
\indexdef{eliminator!non-dependent!for cartesian product}%
...
\indexdef{axiom!of choice!type-theoretic}%
...
\indexdef{coproduct}%
\indexdef{type!coproduct of}%
\indexsee{disjoint sum}{coproduct}%
...
As before, we use the equality types which we have not yet introduced formally, but we need only the fact that everything is equal to itself by $\refl{x}:x=x$.
...
\indexdef{path!induction}%
\indexdef{induction!for paths}%
...
\footnote{We use ``inequality'' to refer to $<$ and $\leq$. Also, note that this is negation of the \emph{propositional} identity type.
