...
referred to as $\eta$-equivalence rules.  Since the corresponding judgmental
equalities are not asserted, we don't have to present them here.
...
\begin{mathparpagebreakable}
  \inferrule*[]
  {\oftp \Gm A {\UU_i} \\ \oftp \Gm B {\UU_i} }
  {\oftp \Gm {\idtoeqv(\UU_i,A,B)} {(\id[\UU_i] A B) \to (\eqv A B)}}
\end{mathparpagebreakable}
...
\begin{mathparpagebreakable}
  ...
  {\oftp\Gm{A}{\UU_i} \\
   \oftp\Gm{B}{\UU_i}}
  {\oftp\Gm{\idtoeqv\text{-}\isequiv(\UU_i,A,B)}{\isequiv(\id[\UU_i] A B, \eqv A B, \idtoeqv(\UU_i,A,B))}}
\end{mathparpagebreakable}
