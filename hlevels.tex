...
\begin{lem}\label{lem:h-set-refrel-in-paths-sets}
Suppose $X$ is a type with a binary relation $R:X\to X\to\prop$ over it, which
is reflexive and which has the property that
\begin{equation*}
\prd{x,y:X}R(x,y)\to x=y.
\end{equation*}
Then we have the equivalence $\eqv{R(x,y)}{x=y}$ for all $x,y:X$. Consequently,
$X$ is a set.
\end{lem}

\begin{proof}
Let $\rho$ be a witness of the reflexivity of $R$ and let $f:\prd{x,y:X}R(x,y)
\to x=y$. To show that each $f(x,y)$ is an equivalence, it suffices to show that
...
\begin{equation*}
\prd{y:X}{H:R(x,y)}\pairr{y,K}=\pairr{x,\rho(x)}.
\end{equation*}
Since $R$ is propositional, the above type is equivalent to the type
\begin{equation*}
\prd{y:X}R(x,y)\to y=x,
\end{equation*}
which we get from our assumption $f$ and from symmetry of the $=$ relation.
\end{proof}
