...
in a variety of settings: Homotopy type theory has an interpretation in
Kan simplicial sets, which are one model of $\infty$-groupoids.  Thus,
our proofs apply to this model, and transfering them along the geometric
realization functor from simplicial sets to topological spaces gives
proofs of the correponding theorems in classical homotopy theory.
...
suggests that type theory is a very convenient way to do homotopy theory
using $\infty$-groupoids: by using the rules for the identity type, we
can avoid the complicated combinatorics involved in many definitions of
$\infty$-groupoids/functors, and simply explicate as much of the
structure as is needed in any particular proof.  

Second, our synthetic approach has suggested some new type-theoretic
methods and proofs.  Some of the proofs that we have done are rather
...
One reason we have discovered these new proofs is that type theory
...
as the universal cover of the circle and the Hopf fibration, using the
...
application of this to computing homotopy groups of spheres in
\cref{sec:moreresults}.    

Third, our synthetic approach is very amenable to computer-checked
proofs in proof assistants such as Coq and Agda.  Almost all of proofs
described in this chapter have been computer-checked, and many of these
proofs were first given in a proof assistant, and then ``informalized''
...
operations (identity, composition, inverses, ...).  Thus, the
higher-inductive presentation of a space allows you to pose the question
"what does the identity type of $X$ really turn out to be?", though it
can take some significant mathematics to answer it.  This is a
higher-dimensional generalization of a familiar fact in type theory:
that characterizing the identity type of $X$ can take some work, even if
$X$ is an ordinary inductive type, such as the natural numbers or
booleans.  For example, the theorem that $\mathsf{true}$ is different
...
\note{needs section refs} In this chapter, we first calculate some
homotopy groups of spheres, including $\pi_1(\Sn ^1)$, $\pi_{k<n}(\Sn
^n)$, $pi_2(\Sn ^2)$ and $pi_3(\Sn ^2)$ by way of the Hopf fibration,
and $\pi_n(\Sn ^n)$ by way of the Freudenthal suspension theorem.  Next,
we discuss the van Kampen theorem, which characterizes the fundamental
group of pushouts, and the status of Whitehead's principle (when is a
map that induces an equivalence on all homotopy groups an equivalence?).
Finally, we include brief summaries of additional results that are not
included in the book, such as $\pi_{n+1}(\Sn ^n)$ for $n\ge 3$, the
Blakers-Massey theorem, and a construction of Eilenberg-Mac Lane spaces.    
...
\autoref{tab:theorems} lists the proofs that we have done, and whether
they have been computer-checked.  For the spheres, we have used two
different definitions of the $n$-sphere $\Sn ^n$: the first as the
