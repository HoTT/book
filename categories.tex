...
\subsection{Categories of First Order Structures}
...
So we define an {\em FO-signature} to be a set of symbols, each symbol $\omega$, being either a function symbol or a relation symbol and each having an arity $|\omega |$ that is a set.  A {\em structure} $a$ for the signature is a set $|a|$ together with an assignment of an $|\omega|$-ary function $\omega^a:|a|^{|\omega|}\ra |a|$ on $|a|$ for each function symbol $\omega$ and an $|\omega|$-ary relation $\omega^a$ on $|a|$ assigning a mere proposition $\omega^ax$ to each $x:|a|^{|\omega|}$. Given structures $a,b$, a function $f:|a|\ra |b|$ is a {\em homomorphism $a\ra b$} if it preserves the structure; i.e. for each symbol $\omega$ of the signature and each $x:|a|^{|\omega|}$,
\begin{enumerate}
...
We describe this notion of FO signature and its structures in our abstract terms.  
