...
\subsection{Iterated Identity Types}
\label{sec:iterated-identity-types}

In many logical systems (say, first-order logic), a distinction is made
between individuals (e.g. $x$), which are the subjects of propositions,
and proofs of propositions (e.g. evidence for $\id[A]{x}{y}$).  Type
theory unifies these two concepts. For example, an individual of type
$\mathbb{Z}$, which represents an integer, is the same general kind of
thing as evidence $p : \id[\mathbb{Z}]{x}{y}$---an individual is a term
of type $\mathbb{Z}$; a proof of equality of integers is a term of type
$\id[\mathbb{Z}]{x}{y}$.  Because of this, the identity type can be
\emph{iterated}: one can form the type $\id[{\id[A]{x}{y}}]{p}{q}$ of equalities
between evidence $p,q$, and the type 
$\id[{\id[{\id[A]{x}{y}}]{p}{q}}]{r}{s}$, and so on.  In familiar cases, these
types will be uninteresting; for example, evidence for
$\id[{\id[\mathbb{Z}]{x}{y}}]{p}{q}$ can always be constructed, expressing
the fact that there is only one way to prove say
$\id[{\mathbb{Z}}]{1}{1}$.

Non-trivial iterated identity types arise from the interpretation of the
identity type as paths: when $p,q : \id[A]{x}{y}$ are thought of as
paths from $x$ to $y$, a term $r : \id[{\id[A]{x}{y}}]{p}{q}$ can be
thought of as a \emph{path between paths}, or a \emph{homotopy}.  The
term $r$
can be visualized as a 2-dimensional path that fills in the space
between $p$ and $q$.  Similarly, $\id[{\id[{\id[A]{x}{y}}]{p}{q}}]{r}{s}$
is the type of 3-dimensional paths between two 2-dimensional paths, and
so on.  

One use of paths between paths is to state the properties of the
operations of path inversion and concatenation.  Because of
proof-relevance, we can't stop after proving ``symmetry'' and
``transitivity'' of equality: we need to know that these
\emph{operations} on equalities are well-behaved.  (This issue is
invisible to set-level mathematics, where symmetry and transitivity are
mere \emph{properties} of equality, rather than structure on paths.)
For instance, we need to know that concatenation is \emph{associative},
and that inversion provides \emph{inverses} with respect to
concatenation.  This is to be expected from the topological point of
view, where these are regarded as \emph{operations} on paths.
...
Another application of iterated identity types is representing a
fundamental concept of homotopy theory called \emph{iterated loop
  spaces}.  Given a type $A$ with a point $a:A$, the \emph{loop space}
$\Omega(A,a)$ is the space of loops in $A$ from $a$ to $a$.  This is
represented in type theory by the type $\id[A]{a}{a}$.  It is then
useful to consider the loops space in the loop space of $A$, which is
the space of 2-dimensional loops on the identity loop in $a$.  This is
written $\Omega^2(A,a)$ and represented in type theory by the type
$\id[{\id[A]{a}{a}}]{\refl{a}}{\refl{a}}$.  And so on.  In general, we can
make the following definition:  

\begin{defn} \label{def:pointedtype}
  A \emph{pointed type} $(A,a)$ is a type $A:\type$ together with a
  point $a:A$.  We write $\type_* \defeq \sm{A:\type} A$ for the type of
  pointed types in the universe $\type$.  
\end{defn}
...
However, inductive types with recursion, like the natural numbers, have more complicated universal properties; see \autoref{cha:induction}.
...
Sets are the first rung on a ladder of \emph{homotopy $n$-types}.  For
example, a $1$-type is an analogous notion one level up, which is like a
$1$-groupoid in category theory.  The defining property of a set, also
called a $0$-type, is that it has no non-trivial paths.  The defining
property of a $1$-type is that it has no non-trivial paths between
paths:

\begin{defn}\label{defn:1type}
  A type $A$ is a \textbf{1-type} if for all $x,y:A$ and $p,q:x=y$ and $r,s:x=y$, we have $r=s$.
\end{defn}

Similary, we can define $2$-types, $3$-types, and so on.  We will define
the general notion inductively in \cref{cha:hlevels}, and study the
relationships between them.  

However, for now it is useful to have two facts in mind.  First, the
levels are upward-closed (if $A$ is an $n$-type then $A$ is an
$n+1$-type); for example:
...
\end{eg}
We will study other types that are not sets in more detail starting in
\autoref{cha:hits}.

An even more special kind of type than a set is a set with at most one
element.  Such types may be regarded as \emph{propositions in a narrow
  sense}, and their study is just what is usually called "logic".
...
\end{equation}
which looks like the classical \emph{axiom of choice}, is always true under this reading. This is a noteworthy, and often useful, feature of the propositions-as-types logic, but it also illustrates how significantly it differs from the classical interpretation of logic.
...
In \autoref{cha:hlevels} we will learn to also call them \emph{$(-1)$-truncated types}.
...
Although they are not consequences of the basic type theory described in \autoref{cha:typetheory}, they may be consistently assumed as axioms.
...
We will say that a type $A$ is \textbf{inhabited} to mean that we assert $A$ itself as a proposition (i.e.\ we construct a term belonging to $A$, usually unnamed).\footnote{We've been using this already, so probably it should be mentioned in \autoref{cha:typetheory}.}
...
And this sort of observation is even more pronounced in \emph{homotopy} type theory, where the powerful tools of univalence and higher inductive types allow us to constructively attack many problems that traditionally would require classical reasoning.
...
We may also replace the propositional truncation by the more general $n$-truncations to be considered in \autoref{cha:hlevels}, obtaining a spectrum of axioms interpolating between~\eqref{eq:ac} and \autoref{thm:ttac}.
However, we cannot relax the requirement that $X$ be a set.
...
Among its useful consequences is the following.
...
Essentially this same problem arises in set-theoretic mathematics when all we can do is to show that some thing exists, but we want to actually have a particular such thing.
Classically, however, we are not bothered by this because we have the axiom of choice, allowing us to simply choose such a thing.
However, in homotopy type theory, the available forms of AC do not solve this problem, due to the presence of non-sets and the naturality imposed by the univalence axiom.
[TODO: I am not satisfied with this paragraph; there must be a better way to explain it.]
...
Note that under the usual propositions-as-types reading, we can pronounce $\iscontr(A)$ as ``$A$ contains exactly one element''.
...
One reason the notion is useful is that sometimes, a collection of data which are individually nontrivial will collectively form a contractible type.
...
\begin{proof}
  ...
  But we can take $q\defeq p$, in which case $\trans{q}{\refl a} = p$ follows from the characterization of transport at path types.
\end{proof}

When this happens, it can allows us to simplify a complicated construction up to equivalence, using the principle that contractible data can be freely ignored.
...
The fact that it is possible to define sets in type theory using finitely many data, with all higher homotopies automatically taken care of as in \autoref{sec:basics-sets}, was first observed by Voevodsky.
His original definition of $\isset$ was a bit more complicated than ours, but has the advantage of fitting neatly into an infinite hierarchy; see \autoref{cha:hlevels}.
...
His original definition was slightly more complicated than ours, but fits into the more general framework of \autoref{cha:hlevels}.
...
\begin{ex}
  ...
  It is an open question whether this is true without LEM (although we expect not).
\end{ex}
