...
derive $\ind{=_A}^{PM}$ without using universes: we can show that $\ind{=_A}^{ML}$ entails \autoref{lem:transport} and  \autoref{thm:contr-paths}, and that these two principles imply $\ind{=_A}^{PM}$ directly.
