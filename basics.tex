...
For more examples, see \autoref{ex:isset-coprod,ex:isset-sigma}.

In fact, sets are just the first rung on a ladder of what are called \emph{homotopy $n$-types}.
...
Note that under the usual propositions-as-types reading, we can pronounce $\iscontr(A)$ as ``$A$ contains exactly one element'', or more precisely ``$A$ contains an element, and every other element of $A$ is equal to that element''.
...
They will be the base case of our inductive definition of $n$-types in \autoref{cha:hlevels}.
