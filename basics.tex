...
\indexdef{path!2-dimensional}%
Similarly, $\id[{(\id[{(\id[A]{x}{y})}]{p}{q})}]{r}{s}$ is the type of
3-dimensional paths between two parallel 2-dimensional paths, and so on.  If the
...
\begin{defn} \label{def:loopspace}
  ...
  \end{align*}
  An element of it will be called an \define{$n$-fold loop}\indexdef{loop!$n$-fold} or an $n$-dimensional loop\indexdef{loop!$n$-dimensional} at $a$.
\end{defn}
