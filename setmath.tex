...
factorization system\index{factorization!system} $(\mathcal{E},\mathcal{M})$ with $\mathcal{M}$ the monomorphisms, in which case $\mathcal{E}$ consists automatically of 
...
This requires propositional resizing\index{propositional resizing}\index{impredicative!quotient} in order to remain in the same universe as $A$ and $R$.
...
We've just shown that $\uset$ is regular (\autoref{thm:set_regular}) and that quotients are effective (\autoref{lem:sets_exact}). We thus have a locally cartesian closed pretopos. Finally, since the $n$-types are closed under the formation of $\w$-types
(\autoref{sec:w-types}), which by \autoref{thm:w-hinit} are initial algebras for polynomial endofunctors, we see that $\uset$ is a $\Pi\mathsf{W}$-pretopos. 
\end{proof}
