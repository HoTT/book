...
On the other hand, the conception of ``or'' is so strong that the usual \textbf{law of excluded middle} cannot be established: if we always had a proof of ``$A$ or not $A$'', then for \emph{every} proposition $A$ we would either have a proof of it or a refutation --- which would of course be nice, but it would also allow us to construct the Halting Oracle and other non-computable functions. We prefer good computational behavior of type theory, and in any case the Univalence Axiom implies that Excluded Middle is false, cf.\ \autoref{thm:not-dneg}.
...
Type-theoretically, this means there are many paths that are equal to reflexivity, but not \emph{judgmentally} equal to it.
