...
\begin{defn}\label{ct:precategory}
  A \define{precategory} $A$ consists of the following.
  \begin{enumerate}
  \item A type $A_0$ of \define{objects}.  We write $a:A$ for $a:A_0$.
...
\begin{defn}\label{ct:isomorphism}
  A morphism $f:\hom_A(a,b)$ is an \define{isomorphism} if there is a morphism $g:\hom_A(b,a)$ such that $\id{g\circ f}{1_a}$ and $\id{f\circ g}{1_b}$.
...
\begin{defn}\label{ct:category}
  A \define{category} is a precategory such that for all $a,b:A$, the function $\idtoiso_{a,b}$ from \autoref{ct:idtoiso} is an equivalence.
\end{defn}
...
\begin{eg}\label{ct:orders}
  ...
  In a preorder, a proof $f: a\le b$ is an isomorphism just when there exists some proof $g: b\le a$.
...
\begin{eg}\label{ct:discrete}
  ...
  If $X$ is a set, we call this the \define{discrete} category on $X$.
  In general, we call it a \define{groupoid} (see \autoref{ct:groupoids}).
\end{eg}
...
\begin{eg}\label{ct:fundgpd}
  ...
  We call this the \define{fundamental pregroupoid} of $X$.
...
\begin{defn}\label{ct:functor}
  ...
  A \define{functor} $F:A\to B$ consists of
  \begin{enumerate}
...
\begin{defn}\label{ct:nattrans}
  For functors $F,G:A\to B$, a \define{natural transformation} $\gamma:F\to G$ consists of
  \begin{enumerate}
...
\begin{defn}
  A functor $F:A\to B$ is a \define{left adjoint} if there exists
  \begin{itemize}
...
\begin{defn}\label{ct:equiv}
  A functor $F:A\to B$ is an \define{equivalence of (pre)categories} if it is a left adjoint for which $\eta$ and $\epsilon$ are isomorphisms.
...
\begin{defn}
  We say a functor $F:A\to B$ is \define{faithful} if for all $a,b:A$, the function
  \[F_{a,b}:\hom_A(a,b) \to \hom_B(Fa,Fb)\]
  is injective, and \define{full} if for all $a,b:A$ this function is surjective.
  If it is both (hence each $F_{a,b}$ is an equivalence) we say $F$ is \define{fully faithful}.
\end{defn}

\begin{defn}
  We say a functor $F:A\to B$ is \define{split essentially surjective} if for all $b:B$ there exists an $a:A$ such that $Fa\cong b$.
\end{defn}
...
\begin{defn}
  A functor $F:A\to B$ is \define{essentially surjective} if for all $b:B$, there \emph{merely} exists an $a:A$ such that $Fa\cong b$.
  We say $F$ is a \define{weak equivalence} if it is fully faithful and essentially surjective.
\end{defn}
...
\begin{defn}\label{ct:isocat}
  A functor $F:A\to B$ is an \define{isomorphism of (pre)cat\-ego\-ries} if $F$ is fully faithful and $F_0:A_0\to B_0$ is an equivalence of types.
\end{defn}
...
\begin{defn}\label{ct:opposite-category}
  For a precategory $A$, its \define{opposite} $A\op$ is a precategory with the same type of objects, with $\hom_{A\op}(a,b) \defeq \hom_A(b,a)$, and with identities and composition inherited from $A$.
\end{defn}

\begin{defn}
  For precategories $A$ and $B$, their \define{product} $A\times B$ is a precategory with $(A\times B)_0 \defeq A_0 \times B_0$ and
...
\begin{defn}\label{ct:representable}
  A functor $F:\uset^{A\op}$ is said to be \define{representable} if there exists $a:A$ and an isomorphism $\y a \cong F$.
\end{defn}
...
\begin{defn}
  A \define{strict category} is a precategory whose type of objects is a set.
\end{defn}
...
\begin{defn}
  A \define{$\dagger$-precategory} is a precategory $A$ together with the following.
  \begin{enumerate}
...
\begin{defn}\label{ct:unitary}
  A morphism $f:\hom_A(x,y)$ in a $\dagger$-precategory is \define{unitary} if $\dgr f \circ f = 1_x$ and $f\circ \dgr f = 1_y$.
\end{defn}
...
\begin{defn}
  A \define{$\dagger$-category} is a $\dagger$-precategory such that for all $x,y:A$, the function
...
\begin{defn}\label{ct:sig}
  A \define{notion of structure} $(P,H)$ over $X$ consists of the following.
  \begin{enumerate}
  \item A function $P:X_0 \to \type$.
    For each $x:X_0$ the elements of $Px$ are called \define{$(P,H)$-structures} on $x$.
  \item For $x,y:X_0$ and $\alpha:Px$, $\;\beta:Py$, to each $f:\hom_X(x,y)$ a mere proposition 
  \[ H_{\alpha\beta}(f).\]
    If $H_{\alpha\beta}(f)$ is true, we say that $f$ is a \define{$(P,H)$-homomorphism} from $\alpha$ to $\beta$.
...
if $f:\hom_X(x,y)$, we have 
  ...
  We say that $(P,H)$ is a \define{standard notion of structure} if this preorder is in fact a partial order, for all $x:X$.
\end{defn}

Note that for a standard notion of structure, each type $Px$ must actually be a set.
We now define, for any notion of structure $(P,H)$, a precategory of \define{$(P,H)$-structures}, $A = \mathsf{Str}_{(P,H)}(X)$.
\begin{itemize}
...
We define a \define{first order signature}, $\Omega$, to consist of sets $\Omega_0$ and $\Omega_1$ of function symbols, $\omega:\Omega_0$, and relation symbols, $\omega:\Omega_1$, each having an arity $|\omega|$ that is a set.
An \define{$\Omega$-structure} $a$ consists of a set $|a|$ together with an assignment of an $|\omega|$-ary function $\omega^a:|a|^{|\omega|}\to |a|$ on $|a|$ to each function symbol, $\omega$, and an assignment of an $|\omega|$-ary relation $\omega^a$ on $|a|$, assigning a mere proposition $\omega^ax$ to each $x:|a|^{|\omega|}$, to each relation symbol.
And given $\Omega$-structures $a,b$, a function $f:|a|\to |b|$ is a \define{homomorphism $a\to b$} if it preserves the structure; i.e.\ if for each symbol $\omega$ of the signature and each $x:|a|^{|\omega|}$,
\begin{enumerate}
...
We call the construction $A\mapsto \widehat A$ the \define{Rezk completion}, although there is also an argument (coming from higher topos semantics) for calling it the \define{stack completion}.
...
Independently of philosophical considerations about category theory, Rezk~\cite{rezk01css} discovered that when defining a notion of $(\infty,1)$-cat\-e\-go\-ry, it was very convenient to use not merely a \emph{set} of objects with spaces of morphisms between them, but a \emph{space} of objects incorporating all the equivalences and homotopies between them.
...
\begin{ex}\label{ct:pre2cat}
  Define a \define{pre-2-category} to consist of the structure formed by precategories, functors, and natural transformations in \autoref{sec:transfors}.
  Similarly, define a \define{pre-bicategory} by replacing the equalities in \autoref{ct:functor-assoc,ct:units} with natural isomorphisms satisfying analogous coherence conditions.
...
\begin{ex}\label{ct:2cat}
  Define a \define{2-category} to be a pre-2-category satisfying a condition analogous to that of \autoref{ct:category}.
...
\begin{ex}
  Recall that a \define{strict category} is a precategory whose type of objects is a set.
...
\begin{ex}
  Define the pre-2-category of $\dagger$-categories, which has $\dagger$-struc\-tures on its hom-precategories.
...
\begin{ex}\label{ex:rezk-vankampen}
  Using fundamental pregroupoids from \autoref{ct:fundgpd,ct:rezk-fundgpd-trunc1} and the Rezk completion from \autoref{sec:rezk}, give a different proof of van Kampen's theorem (\autoref{sec:van-kampen}).
\end{ex}

\begin{ex}\label{ex:stack}
  Let $X$ and $Y$ be sets and $p:Y\to X$ a surjection.
  \begin{enumerate}
  \item Define, for any precategory $A$, the category $\mathrm{Desc}(A,p)$ of \define{descent data} in $A$ relative to $p$.
  \item Show that any precategory $A$ is a \define{prestack} for $p$, i.e.\ the canonical functor $A^X \to \mathrm{Desc}(A,p)$ is fully faithful.
  \item Show that if $A$ is a category, then it is a \define{stack} for $p$, i.e.\ $A^X \to \mathrm{Desc}(A,p)$ is an equivalence.
