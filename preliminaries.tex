...
\index{axiom!Streicher's Axiom K}%
By carefully restricting the case distinctions that are allowed, we can define a form of deep pattern matching that can be justified entirely in terms of the induction principles for inductive types~\cite{CDP14}, thus avoiding Axiom K. This can be enabled in \Agda by means of a flag called without-K. However, this does not include pattern matching on higher inductive types (yet). For these reasons, we have chosen to regard the induction principle as the basic property of an inductive definition, and derive an appropriate pattern matching principle from them.
