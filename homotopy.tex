...
The $k^{th}$ homotop group ($\pi_k$) of the $n$-dimensional sphere
...
\begin{array}{lll}
  & {\encode'(\lloop^{n+1})}  \\
= & {\encode'(\lloop^{n} \ct \lloop)} & \text{by definition of $\lloop^{-}$} \\
= & \transfib{\code}{(\lloop^{n} \ct \lloop)}{0} & \text{by definition of $\encode$}\\
...
\begin{defn}
  ...
  We take $A$ for $F_1$ and $F_2$, and for $a:A$, $e(a)$ is the equivalence
  $\mu(a,-)$.
\end{defn}
...
and the fibration we constructed is a fibration over $\susp A$ whose total space
is a pushout of the top line.
...
\begin{proof}
  ...
  \end{align*}
  where (thanks to \autoref{thm:dpath-path}) in order to define $\alpha$ it is
...
\end{lem}
\begin{proof}
  More precisely, the map $f:(A*B)*C\to A*(B*C)$ is defined by the following
  equations:
  \begin{align*}
...
\begin{proof}
  ...
  Hence using Fubini’s theorem for colimits (that we haven’t proved) we have an
  equivalence $\eqv{(A*B)*C}{A*(B*C)}$. The proof of Fubini’s theorem for
  colimits still requires the long and tedious computation, though.
\end{proof}
...
\begin{proof}
  ...
  and \autoref{lem:suspbool} we have:
  \begin{align*}
