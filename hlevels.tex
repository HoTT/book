...
\begin{ex}\label{ex:connectivity-inductively}
  Prove that for $n\ge -1$, a type $A$ is $n$-connected if and only if it is $(-1)$-connected (i.e.\ merely inhabited) and for all $a,b:A$ the type $\id[A]ab$ is $(n-1)$-connected. Thus, $n$-connectedness of types can be defined using only propositional truncations.
\end{ex}
