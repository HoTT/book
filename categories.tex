...
We will also see in \S\ref{sec:strict-categories}--\ref{sec:dagger-categories} that in slightly more exotic contexts, there are uses for certain kinds of precategories other than categories.
...
\begin{defn}\label{ct:sig}
  ...
  \item A function $P:X_0 \to \type$.
    The elements of $Px$ are called \emph{$(P,H)$-structures} on $x$.
  \item For each $(x,y:X)$, $(f:\hom_X(x,y))$, $(\alpha:Px)$, $(\beta:Py)$, a mere proposition $H_{\alpha\beta}(f)$.
    If $H_{\alpha\beta}(f)$ is true, we say that $f$ is an \emph{$(P,H)$-homomorphism} from $\alpha$ to $\beta$.
...
So we define an {\em FO-signature}, $\Omega$, to consist of a set $|\Omega|$ of symbols, each symbol $\omega:|\Omega|$, being either a function symbol or a relation symbol and each having an arity $|\omega |$ that is a set.  An {\em $\Omega$-structure} $a$ consists of a set $|a|$ together with an assignment of an $|\omega|$-ary function $\omega^a:|a|^{|\omega|}\ra |a|$ on $|a|$ for each function symbol $\omega$ and an $|\omega|$-ary relation $\omega^a$ on $|a|$ assigning a mere proposition $\omega^ax$ to each $x:|a|^{|\omega|}$. Given $\Omega$-structures $a,b$, a function $f:|a|\ra |b|$ is a {\em homomorphism $a\ra b$} if it preserves the structure; i.e. for each symbol $\omega$ of the signature and each $x:|a|^{|\omega|}$,
\begin{enumerate}
\item $f(\omega^ax) = \omega^b(f\circ x)$ if $\omega$ is a function symbol, and
\item $\omega^ax\ra\omega^b(f\circ x)$ if $\omega$ is a relation symbol.
\end{enumerate}
Note that each $x:|a|^{|\omega|}$ is a function $x:|\omega|\ra |a|$ so that $f\circ x : b^\omega$.
...
We use the FO-signature $\Omega$ to give us a standard notion of structure $(P,H)$ over $\uset_\bbU$.  We assume given a function $fsymbol :|\Omega|\ra\bbB$, where $\bbB$ is the type of boolean values, having canonical elements $\bbt,\bbf$, so that if $\omega :\Omega$ then we take
\begin{center}
...
For each $\bbU$-small set $x$ define
  \[ P^\omega x := \left\{
         \begin{array}{ll}
x^{|\omega|}\ra x &\mbox{ if } fsymbol(x)=\bbt\\
x^{|\omega|}\ra \bbP_\bbU &\mbox{ if } fsymbol(x)=\bbf
         \end{array}\right.
  \]
where $\bbP_\bbU$ is the set of $\bbU$-small propositions, and
\item For $\bbU$-small sets $x,y$ and 
$\alpha:P^\omega x,\;\beta:P^\omega y,\; f:x\ra y$, define
  \[ H^\omega_{\alpha\beta}(f) := \left\{
         \begin{array}{ll}
\forall_{u:x^{|\omega|}}\; [f(\alpha u)=\beta(f\circ u)]  &\mbox{ if } fsymbol(x)=\bbt\\
\forall_{u:x^{|\omega|}}\; [\alpha u\ra\beta(f\circ u)] &\mbox{ if } fsymbol(x)=\bbf
         \end{array}\right.
  \]
\end{enumerate}
\item 
...
\item for $\bbU$-small sets $x,y$ and $\alpha :Px,\; \beta :Py,\; f:x\ra y$
define 
