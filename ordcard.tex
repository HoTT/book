...
Classically, filteredness is expressed in terms of \emph{cofinality}; see \autoref{ex:cofinality}.
...
\begin{eg}\label{thm:ord-filtered}
  For any $X:\set_\UU$ and $Y:X\to \set_\UU$ in a fixed universe \UU, the ordinal $\ord_\UU$ in the next higher universe is $Y$-filtered.
  This follows from \autoref{thm:ordsucc,thm:ordunion}.
\end{eg}

\autoref{thm:ord-filtered} shows that the question of the existence of filtered ordinals is only interesting if we ask it about ordinals of the same size as $X$ and $Y$.
In classical set theory, the axiom of choice is necessary in order to show that small $Y$-filtered ordinals exist for infinite $Y$.
...
\begin{thm}\label{thm:enough-filtered-ordinals}
  ...
  Then for any $X:\UU$ and $Y:X\to \set_\UU$, there exists a $Y$-filtered ordinal in \UU.
\end{thm}
\begin{proof}
  TODO.
\end{proof}
