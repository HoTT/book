...
Specifically, if we write $\mathsf{transportconst}^P_p(b)$ for the path defined in \autoref{thm:trans-trivial}, we have the following.

\begin{lem}
  For $f:A\to B$ and $p:\id[A]xy$, we have
  \[ \apdfunc f(p) = \mathsf{transportconst}^P_p(f(x)) \ct \apfunc f (p) \]
\end{lem}
\begin{proof}[First proof]
  Let $D:\prd{x,y:A}{p:\id xy} \type$ be the type family defined by
  \[ D(x,y,p) \defeq \big(\apdfunc f (p) = \mathsf{transportconst}^P_p(f(x)) \ct \apfunc f (p)\big). \]
  Thus, we have
  \[D(x,x,\refl x) \jdeq \big(\apdfunc f (\refl x) = \mathsf{transportconst}^P_{\refl x}(f(x)) \ct \apfunc f ({\refl x})\big).\]
