...
\begin{proof}
  ...
  But since $f$ is an embedding, we have $(\id[X]{x}{x'}) \simeq (\id[Y]{f(x)}{f(x')})$, and the latter is an $\nminusone$-type by assumption.
\end{proof}
...
\begin{thm}\label{thm:h-set-uip-K}
 A type $X$ is a set if and only if it satisfies \define{Axiom K}: for all $x : X$ and $p : (x =_A x)$ we have $p = \refl{x}$.
\end{thm}
...
\begin{equation*}
\eqv{\trunc n{\sm{x:A}\trunc n{P(x)}}}{\trunc n{\sm{x:A}P(x)}}.
\end{equation*}
\end{thm}

\begin{proof}
We use the universal property of $n$-truncation several times to construct
functions
\begin{align*}
\varphi & : \trunc n{\sm{x:A}\trunc n{P(x)}}\to\trunc n{\sm{x:A}P(x)}\\
\psi & : \trunc n{\sm{x:A}P(x)}\to \trunc n{\sm{x:A} \trunc n{P(x)}}
\end{align*}
and the homotopies $H:\varphi\circ\psi\htpy \idfunc$ and $K:\psi\circ\varphi\htpy
\idfunc$. We define $\varphi$ by setting $\varphi(\tproj n{\pairr{x,\tproj nu}})\defeq\tproj n{\pairr{x,u}}$.
...
\begin{cor}
  ...
  \[\trunc n{\Omega^k(A,a)} = \Omega^k\mathopen{}\left(\trunc{n+k}{(A,a)}\right). \]
\end{cor}
\begin{proof}
...
\begin{align*}
  \prd{b:B} P(b) & \simeq \prd{b:B} \Parens{\trunc n{\hfib{f}b} \to P(b)}
  && \text{(since $\trunc n{\hfib{f}b}$ is contractible)}\\
  & \simeq \prd{b:B} \Parens{\hfib{f}b\to P(b)}
  && \text{(since $P(b)$ is an $n$-type)}\\
  & \simeq \prd{b:B}{a:A}{p:f(a)= b} P(b)
  && \text{(by the left universal property of $\Sigma$-types)}\\
  & \simeq \prd{a:A} P(f(a))
  && \text{(by the left universal property of path types)}.
\end{align*}
...
is a fiberwise $n$-connected family of functions, i.e.\ each function $g_a : P(a) \to Q(f(a))$ is $n$-connected. Then the function
\begin{align*}
...
\begin{align*}
\trunc n{\hfib{\varphi}{\pairr{b,v}}} & \simeq \Trunc n{\sm{a:A}{u:P(a)}{p:f(a)= b} \trans{\ap f p }{g_a(u)}= v}\\
& \simeq \Trunc n{\sm{w:\hfib{f}b}{u:P(\proj1(w))} g_{\proj 1 w}(u)= \trans{\opp{\ap f {\proj2 w}}}{v}}\\
& \simeq \Trunc n{\sm{w:\hfib{f}b} \hfib{g(\proj1 w)}{\trans{\opp{\ap f {\proj 2 w}}}{v}}}\\
& \simeq \Trunc n{\sm{w:\hfib{f}b} \trunc n{\hfib{g(\proj1 w)}{\trans{\opp{\ap f {\proj 2 w}}}{v}}}}\\
...
\begin{cor}\label{thm:refl-over-ntype-base}
  ...
  \[ \eqv{\Trunc n{\sm{a:A}P(a)}}{\sm{a:A} \trunc n{P(a)}} \]
\end{cor}
\begin{proof}
  Consider the canonical fiberwise transformation $\prd{a:A} (P(a) \to \trunc n{P(a)})$.
  By \autoref{cor:totrunc-is-connected}, each map $P(a) \to \trunc n{P(a)}$ is $n$-connected, so by \autoref{prop:nconn_fiber_to_total} the induced map
  \begin{equation*}
    \Parens{\sm{a:A} P(a)} \to \sm{a:A} \trunc n{P(a)}
  \end{equation*}
  is $n$-connected, and thus induces an equivalence between $n$-truncations by \autoref{lem:connected-map-equiv-truncation}.
  But since $A$ is an $n$-type, so is $\sm{a:A} \trunc n{P(a)}$.
\end{proof}
...
% \end{equation*}
% defined by $\varphi(s) \defeq s\circ f$ is an equivalence.
% \end{cor}
...
% \end{equation*}
% defined by $\varphi(h,H) \defeq \pairr{h\circ f,H\circ f}$ is an equivalence.
% \end{cor}
...
\begin{proof}
Note that $A\simeq\sm{b:B}\hfib{f}b$. The function $\tilde{f}$ is the function on total spaces induced by the canonical fiberwise transformation
\begin{equation*}
...
\begin{align*}
\hfib{h_1}b
& \simeq \sm{w:\hfib{h_1}b} \trunc n{ \hfib{g_1}{\proj1 w}}
&& \text{(since $g_1$ is $n$-connected)}\\
& \simeq \Trunc n{\sm{w:\hfib{h_1}b}\hfib{g_1}{\proj1 w}}
&& \text{(by \autoref{thm:refl-over-ntype-base}, since $h_1$ is $n$-truncated)}\\
& \simeq \trunc n{\hfib{h_1\circ g_1}b}
&& \text{(by \autoref{ex:unstable-octahedron})}
\end{align*}
and likewise for $h_2$ and $g_2$.
Also, since we have a homotopy $H:h_1\circ g_1\sim h_2\circ g_2$, there is an obvious equivalence $\hfib{h_1\circ g_1}b\simeq\hfib{h_2\circ g_2}b$.
Hence we obtain
\begin{equation*}
...
Recall that if we have paths $p,p^\prime:x= y$, $s:p= p^\prime$ and $q:y= z$, then left whiskering provides a path $q\ct p=q\ct p^\prime$, which is denoted by $q\leftwhisker s$.
Likewise, if $f,f^\prime:X\to Y$ and $g:Y\to X$ are functions and if $H:f\sim f^\prime$ is a homotopy, then pointwise whiskering provides a homotopy $g\leftwhisker H:g\circ f\sim g\circ f^\prime$.
...
\begin{defn}\label{defn:reflective-subuniverse}
  A \define{reflective subuniverse} is a predicate $P:\type\to\prop$ such that
...
For any modality $\modal$, we define a map $f:A\to B$ to be \define{$\modal$-connected} if $\modal(\hfib f b)$ is contractible for all $b:B$, and to be \define{$\modal$-truncated} if $\hfib f b$ is modal for all $b:B$.
...
On the computational side, monads (and hence modalities) are used to model effects in functional programming~\cite{Moggi89}.
...
\begin{ex}\label{ex:ntype-from-nconn-const}
  Show that \autoref{thm:nconn-to-ntype-const} also works as a characterization in the other direction: $B$ is an $n$-type if and only if every map into $B$ from an $n$-connected type is constant.
...
\begin{ex}\label{ex:acconn}
  Define the \define{$n$-connected axiom of choice} to be the statement
  \begin{quote}
  ...
  \begin{enumerate}
  \item Prove that the $(-1)$-connected axiom of choice implies the $n$-connected axiom of choice for all $n\ge -1$.
...
\begin{ex}
  ...
  \item Show that $X\mapsto (P\to X)$ is a left exact modality.
    This is called the \define{open modality} associated to $P$.
...
\begin{ex}
  Let $f:A\to B$ be a map; a type $Z$ is \define{$f$-local} if $(-\circ f):(B\to Z) \to (A\to Z)$ is an equivalence.
  \begin{enumerate}
