...
collection of objects, and then a collection of \emph{morphisms}\indexdef{morphism!in an $\infty$-groupoid} between
objects, and then \emph{morphisms between morphisms}, and so on,
equipped with some complex algebraic structure; a morphism at level $k$ is called a \define{$k$-morphism}\indexdef{$k$-morphism}.  Morphisms at each level
...
\begin{center}
  ...
  \begin{tabular}{ll}
    ...
    Equality & Homotopy \\
    \midrule
    reflexivity\index{equality!reflexivity of} & constant path\\
    symmetry\index{equality!symmetry of} & inversion of paths\\
    transitivity\index{equality!transitivity of} & concatenation of paths \\
    \bottomrule
  \end{tabular}
  \medskip
\end{center}
...
In set theory, the propositon $a=a$ is entirely uninteresting, but in homotopy theory paths from a point to itself are called \emph{loops}\index{loop} and carry lots of interesting higher structure.
...
\end{equation*}
%
between them, defined as follows: first let $\alpha \rightwhisker r : p\ct r = q\ct r$ be determined by path induction on $r$, then let $q\leftwhisker \beta : q\ct r = q\ct s$ be given by induction on $q$.  Since these paths are composable in the type of 2-paths, we can define the \define{horizontal composition}
...
\index{function|(}%
\index{functoriality@``functoriality''!of functions in type theory}%
...
We think of a type family $P:A\to \type$ as a \emph{fibration} with base space $A$, with $P(x)$ being the fiber over $x$, and with $\sm{x:A}P(x)$ being the \define{total space} of the fibration.
...
\index{functoriality@``functoriality''!of functions in type theory}%
\index{continuity@``continuity''!of functions in type theory}%
The first level of the continuity/naturality\index{naturality@``naturality''!of homotopies} of homotopies can be expressed as follows:
...
As will become clear in later chapters, it would be most convienient and intuitive if these characterizations of identity types, transport, and so on were \emph{judgmental}\index{judgmental equality} equalities.
...
Similarly, the two homotopies which witness~\eqref{eq:path-prod-inverse} as quasi-inverse to~\eqref{eq:path-prod} consist respectively, of \emph{computation rules}:
\begin{align*}
...
\end{align*}
and a \emph{uniqueness principle}:
\[
...
\begin{narrowmultline*}
  g : \prd{w,w':\sm{x:A}P(x)} 
      \Big(\dsm{p:\proj{1}(w)=\proj{1}(w')}\trans{p}{\proj{2}(w)}=\proj{2}(w')\Big)
      \to
      \narrowbreak
      (w=w')
\end{narrowmultline*}
...
A positive type is one which is ``presented'' by certain constructors, with the universal property of a presentation\index{presentation!of a positive type by its constructors} being expressed by its elimination rule.
...
In the case of cartesian products, the non-dependent version simply expresses the cartesian closedness adjunction\index{adjoint!functor}:
...
Limits of diagrams over directed graphs are the easiest general sort of limit to formalize; the problem with diagrams over categories (or more generally $(\infty,1)$-categories\index{.infinity1-category@$(\infty,1)$-category}) is that in general, infinitely many coherence conditions are involved in the notion of (homotopy coherent) diagram.
