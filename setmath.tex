...
considers the category with objects $A$ and hom-sets $R$. The Rezk completion (see \autoref{sec:rezk}) of this category will be the
...
One naturally wonders what, if anything, prevents $\uset$ from being a topos? In addition to the structure already mentioned, a topos has a
...
The proof in~\cite{Mines/R/R:1988} uses the power set operation (which is impredicative), although it can also be seen as a predicative proof of a weaker statement (that a map in a universe $\UU_i$ is surjective if it is an epimorphism in the next universe $\UU_{i+1}$).
