...
\begin{defn}
  ...
  Identities are defined by $1_{(a,b)}\defeq (1_a,1_b)$ and composition by $(f,f');(g,g') \defeq ((f;g),(f';g'))$.
\end{defn}
...
\begin{proof}
  ...
  \begin{equation*}
    G_{a,a'}(f)_b ; G(a')_{b,b'}(g) = G(a)_{b,b'}(g) ; G_{a,a'}(f)_{b'}
  \end{equation*}
...
\begin{proof}
  ...
  \begin{align*}
    \alpha_{a'}(f)
    &= \alpha_{a'} (\y a_{a',a}(f))\\
    &= (\y a_{a',a}(f);\alpha_{a'})(1_a)\\
    &= (\alpha_a; F_{a',a}(f))(1_a)\\
