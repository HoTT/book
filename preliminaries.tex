...
\index{pattern matching}%
Formally, one may describe type theory by taking either \emph{induction principles}, or a general sort of \emph{pattern matching}, as basic and deriving the other; see \cref{cha:rules}.
(In this context, ``pattern matching'' refers to something even more general than that discussed in \cref{sec:pattern-matching}; for instance, it allows defining a function $f:\nat\to\nat$ by three clauses for $f(0)$, $f(1)$, and $f(\suc(\suc(n)))$.
This is sometimes called \emph{deep} pattern matching.)

For several reasons, we take the point of view that induction principles are fundamental, while pattern matching must be justified by reducing it to induction principles.
One reason is that when constructing categorical models of type theory, induction principles are what we obtain naturally.
Another is that it is quite tricky to specify exactly what kinds of pattern matching can be reduced to the induction principle without requiring additional axioms.
For instance, the form of pattern matching implemented by default in \Agda
\index{proof!assistant!Agda@\textsc{Agda}}%
allows proving Axiom K.
\index{axiom!Streicher's Axiom K}%
By carefully restricting the case distinctions that are allowed~\cite{CDP14}, one can obtain a form of deep pattern matching that can be reduced to induction principles without Axiom K (this restriction can be enabled in \Agda by the \texttt{--without-K} flag) --- but while verifying allowability of a pattern match and reducing it to an induction principle can be done algorithmically by a computer, they are rather difficult for a human to perform informally.
Finally, deep pattern matching is difficult to make general sense of for \emph{higher} inductive types, which we will introduce in \cref{cha:hits}.
Therefore, although we will use pattern matching syntax as described in \cref{sec:pattern-matching}, we restrict ourselves to pattern matches that are directly equivalent to the application of an induction principle.
