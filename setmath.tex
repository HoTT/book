...
To these, we may add the following for consideration:
%
\begin{align*}
  \mathsf{LEM}_0\;\defeq\;&\ 
  \prd{A:\set} (A + \neg A) 
\end{align*}
%
This says that for all \emph{sets} $A$, we have $A + \neg A$, while AC says that for any family $A: X\to \set$ of \emph{sets} on a \emph{set} $X$, if a \emph{proposition}-valued relation $P(x,y)$ is total on $X$, then there is a choice function.  More simply, for non-dependent types AC takes the form:
%
\begin{align*}
    & \prd{X:\set}  \prd{A:\set} \prd{P:X\to A\to\prop} \\
    & \Big(\fall{x:X} \exis{a:A} P(x,a)\Big) \to \exis{g:X\to A} \fall{x:X} P(x,g(x))
\end{align*}
%
Finally, taking $P(x,a)$ to be $f(a)=x$ for some function $f:A\to X$, we arrive at the formulation: for any surjection $f:A\to X$ of sets, there is a ``section" $g:X\to A$ with $f\circ g = 1_X$.  The converse implication from this statement to AC can be taken as an exercise. 

The logical relations among these conditions are as follows:
\[
\mathsf{LEM}_\infty \Rightarrow  \mathsf{AC} \Rightarrow \mathsf{LEM}_0 \Rightarrow \mathsf{LEM}\Rightarrow \mathsf{AC}_\infty
\]
At the extremes, we have that $\mathsf{LEM}_\infty$ is inconsistent with the univalence axiom $\mathsf{UA}$, and $\mathsf{AC}_\infty$ is provable in pure type theory, without any further assumptions (not even function extensionality).
In this subsection, we will first show that if every surjective function between sets has
a section, then for all propositions $A$ we can decide whether $A$ or $\neg A$
holds, i.e.\ the implication $\mathsf{AC} \Rightarrow \mathsf{LEM}$.  The implication $\mathsf{AC} \Rightarrow \mathsf{LEM}_0$ then also follows.
...
$0=1$ in $\susp(A)$ is equivalent to $A$. 
\end{lem}
...
Suppose that all surjections between sets merely have a section. Then for all sets $A$, we have $A + \neg A$. 
\end{cor}
