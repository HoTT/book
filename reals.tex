...
We also need an induction principle for statements about all Cauchy approximations. The
following lemma explains how this can be done.

\begin{lem} \label{RC-seq-induction}
  Define the type of Cauchy approximations
  %
  \begin{equation*}
    C \defeq \sm{x : \Qp \to \RC} \prd{\epsilon, \delta : \Qp} x_\delta \sim_{\delta + \epsilon} x_\epsilon.
  \end{equation*}
  %
  Suppose $P : C \to \prop$ respects coincidence of Cauchy approximations,
  %
  \begin{equation*}
    \prd{x,y : C}
  ...
  \end{equation*}
  %
  To show $\prd{x : C} P(x)$ it suffices to verify
  %
...
\begin{proof}
  We define an auxiliary function $Q : \RC \to \prop$ by induction as follows:
  %
  \begin{align*}
    Q(\rcrat(q)) &\defeq P(\lam{\blank : \Qp} q), \\
    Q(\rclim(x)) &\defeq P(x).
  \end{align*}
  %
  ...
  $\prd{\epsilon : \Qp} x \sim_\epsilon y$ and so $P(x)$ and $P(y)$ are equivalent by
  assumption. If $\rcrat(q) = \rclim(x)$ is constructed through~\eqref{eq:RC-rat-eq-lim},
  then $\prd{\epsilon : \Qp} (\lam{\blank : \Qp} q) \sim_\epsilon x$ and so $P(\lam{\blank
    : \Qp} q)$ and $P(x)$ are equivalent.
  ...
  that $P(x)$ holds using~\eqref{eq:RC-seq-induction-ii}. Suppose $\delta : \Qp$, $y : C$
  and $x_\delta = \rclim(y)$. By assumption $Q(x_\delta)$ holds, but then $Q(\rclim(y))$
  holds too, which is just $P(y)$.
\end{proof}
...
second one gets artithmetic going. We use the obvious fact that $\sim_\epsilon$ is a
