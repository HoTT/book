...
Recall that type theory (unlike, say, first-order logic) does not distinguish between individuals (e.g.\ $x$), which are the subjects of propositions, and proofs of propositions (e.g.\ evidence for $\id[A]{x}{y}$).
Thus, the identity type can be iterated: we can form the type $\id[{\id[A]{x}{y}}]{p}{q}$ of equalities between identifications $p,q$, and the type $\id[{\id[{\id[A]{x}{y}}]{p}{q}}]{r}{s}$, and so on.

If the original type $A$ is ``set-like'', such as \nat, these iterated identity types will be uninteresting (see \autoref{sec:basics-sets}).
...
(This issue is invisible to set-level mathematics, where symmetry and transitivity are mere \emph{properties} of equality, rather than structure on
paths.)
...
We are still not really done with the higher groupoid structure: the paths~\ref{item:omg1}--\ref{item:omg4} must also satisfy their own higher coherence laws, which are themselves higher paths, and so on all the way up to infinity.
...
\begin{defn} \label{def:pointedtype}
  ...
  We write $\type_* \defeq \sm{A:\type} A$ for the type of pointed types in the universe $\type$.
\end{defn}
...
Now recall that a non-dependently typed function $f:A\to B$ is just the special case of a dependently typed function $f:\prd{x:A} P(x)$ when $P$ is a constant type family, $P(x) \defeq B$.
...
\end{lem}
\begin{proof}
  Suppose $f:\isprop(A)$; thus for all $x,y:A$ we have $f(x,y):x=y$.
  Fix $x:A$ and define $g(y)\defeq f(x,y)$; then for any $y,z:A$ and $p:y=z$ we have $\apd g p : \trans{p}{g(y)}{g(z)}$, hence $g(y)\ct p = g(z)$.
  Thus, for any $p,q:x=y$, we have $p = \opp{g(x)}\ct g(y) = q$.
\end{proof}
...
\end{lem}
\begin{proof}
  Suppose $f,g:\isprop(A)$.
  By function extensionality, to show $f=g$ it suffices to show $f(x,y)=g(x,y)$ for any $x,y:A$.
  But $f(x,y)$ and $g(x,y)$ are both paths in $A$, which is a mere proposition (because of $f$, or because of $g$) and hence a set; thus they are equal.
  A similar argument applies to $\isset(A)$.
\end{proof}
...
See also \autoref{sec:object-classification}.
...
In \autoref{ex:lem-brck,ex:impred-brck,sec:hittruncations} we will describe two ways to construct $\brck{A}$ in terms of more general things; for now we merely assume it as an additional rule alongside those of \autoref{cha:typetheory}.
