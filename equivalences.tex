...
These names are allowed to be somewhat clunky, because after we know that they are all equivalent and have properties~\ref{item:beb1}--\ref{item:beb3}, we will revert to saying simply ``equivalence'' without needing to specify which particular definition we choose.
...
\begin{defn}
  ...
  of \emph{left} resp. \emph{right inverses} to $f$. We call $f$ \emph{left} resp. \emph{right invertible} if the type $\linv$ resp. $\rinv$ is inhabited.    
  \end{itemize}
\end{defn}
...
From \autoref{cha:hlevels} onwards we will call such a map \emph{$(-2)$-truncated}.
