...
In other words, while the pure propositions-as-types logic may be called ``constructive'', the default $(-1)$-truncated logic may be called ``intuitionistic'' (although one should be aware that these terms are often used differently); and to the latter we may freely add the axioms of choice and excluded middle to obtain a logic that may be called ``classical''.
