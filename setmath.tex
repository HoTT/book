...
\begin{thm}\label{thm:ETCS} Assuming the axiom of choice for sets in the form $\mathsf{AC}$, the category $\set$ is a boolean elementary topos with choice.  In fact, it is also well-pointed and thus satisfies Lawvere's axioms for the \emph{Elementary Theory of the Category of Sets}.
 \end{thm}
...
\begin{ex}
Show that with $\mathsf{AC}$, the category $\set$ is well-pointed, in the sense that the following statement holds: for any $f, g : A\to B$, if $f \neq g$ then there is a function $a : 1\to A$ such that $fa \neq ga$.  Show that the ``slice category" $\set/\bool$ consisting of functions $A\to \bool$ and commutative triangles does not have this property. (Hint: the terminal object in $\set/\bool$ is the identity function $\bool \to \bool$, so in this category, there are objects $X$ that have no elements $1\to X$.)
\end{ex}
