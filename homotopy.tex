...
contributed to these results, either by being the principal author of a
proof, or by suggesting problems to work on, participating in many
discussions and seminars about these problems, or giving feedback on
results.  The following people were the principal authors of the first
HoTT proofs of the above theorems; unless we indicate otherwise, for the
theorems that have been computer-checked, the principal authors were
also the first ones to formalize the proof in a computer proof
assistant:
%
\begin{itemize}
\item 
...
\item Vladimir Voevodsky gave the proof that the sequence of homotopy
  groups of a fibration is long-exact.

\item Steve Awodey suggested investigating the Hopf fibration, and that
  it would be a way to calculate e.g. $\pi_2(\Sn ^2)$ and $\pi_3(\Sn
  ^3)$ by a long-exact-sequence argument.  Peter Lumsdaine gave the
  first construction of the Hopf fibration.  Guillaume Brunerie gave the
  first proof that the total space of the Hopf fibration is $\Sn ^3$,
  with $\pi_2(\Sn ^2)$ and $\pi_3(\Sn ^3)$ as corollaries.  Dan Licata
  later gave an encode-decode proof of $\pi_2(\Sn ^2)$, for the
...
\item Guillaume Brunerie proved $\pi_4(\Sn ^3)$, following a suggestion
  by Andr\'e Joyal to investigate the James Construction.  
...
development of these results.  For example, the first proof of
