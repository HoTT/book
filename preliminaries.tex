...
\index{expansion, eta-@expansion, $\eta $-|footstyle}%
}}
\[ f \jdeq (\lam{x} f(x)). \]
...
(For example, for functions, the computation rule states that $(\lamu{x:A}\Phi)(a)$ is judgmentally equal to the substitution of $a$ for $x$ in $\Phi$.)
  ...
  \item 
    an optional \define{uniqueness principle}\indexdef{uniqueness!principle}\footnote{also referred to as \define{$\eta$-expansion}\indexdef{$\eta$-expansion}}, which expresses
