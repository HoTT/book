...
\begin{proof}
  Let $\rho : \prd{x:X} R(x,x)$ witness the reflexivity of $R$, and let $f : \prd{x,y:X} R(x,y) \to (\id[X]{x}{y})$ be a witness that $R$ implies identity.
  Note first that the two statements in the theorem are equivalent.
  For on one hand, if $X$ is a set, then $\id[X]xy$ is a mere proposition, and since it is logically equivalent to the mere proposition $R(x,y)$ by hypothesis, it must also be equivalent to it.
  On the othre hand, if $\id[X]xy$ is equivalent to $R(x,y)$, then like the latter it is a mere proposition for all $x,y:X$, and hence $X$ is a set.
...
Suppose that $f:\Sn^n\to A$ is a function and suppose that $\tilde{f}$ corresponds to $f$ via the indicated equivalence. Then $\tilde{f}$ is null-homotopic, and hence $\lam{t} \tilde{f}(\merid^n(t))$ is null-homotopic (by \autoref{lem:hnull_to_map_hnull}).
...
\begin{enumerate}
\item functions $\mreturn_A:A\to\modal(A)$ for every type $A$. 

We define $\ismodal A\defeq \mathsf{isEquiv}(\mreturn_A)$ and $\type_\modal\defeq\sum A:\type, \ismodal A$.
\item for every $A:\type$ and every dependent type $P:\modal(A)\to\type$, a function
\begin{equation*}
R^\modal_A:\big(\prod(a:A),\ \modal(P(\mreturn(a)))\big)\to\prod(z:\modal(A)),\ \modal(P(z)).
\end{equation*}
\item A path $R^\modal_A(f)(\mreturn(a))= f(a)$ for each $f:\prod(a:A),\ \modal(P(\mreturn(a)))$.
\item For any $z,z':\modal(A)$ the type $z= z$ is modal.
\end{enumerate}
\end{defn}

Consequently, $\modal$ is an idempotent monad on $\type$.

\begin{defn}
We define $\modaltype:=\sum(X:\type),\ \mathsf{isEquiv}(\mreturn^\modal_X)$. 
\end{defn}

\begin{prop}\label{prop:lv_n_deptype_sec_equiv_by_precomp}
Let $A$ be a type and let $P:\modal(A)\to\modaltype$. Then the function
\begin{equation*}
\prod(z:\modal(A)),\ P(z)\to\prod(a:A),\ P(\mreturn(a))
\end{equation*}
given by $\lambda s.s\circ \mreturn$ is an equivalence.
\end{prop}

\begin{proof}
We only need to find a homotopy
\begin{equation*}
\prod(z:\modal(A)),\ s(z)= R^\modal_X(s\circ \mreturn)(z)
\end{equation*}
for each $s:\prod(z:\modal(A)),\ P(z)$. By assumption we have that $P(z)$ is
modal for each $z:\modal(A)$ and hence it follows that $s(z)= R^\modal_X(s\circ \mreturn)(z)$
is modal for each $z$. Hence it suffices to find a function of type
\begin{equation*}
\prod(a:A),\ s(\mreturn(a))= R^\modal_X(s\circ \mreturn)(\mreturn(a)),
\end{equation*}
...
\beta(f) & : \prod(u:P(b)),\ f(u)=\alpha(f)\\
\gamma(f) & : \prod\{w:\mathcal{L}_P(X)\}(H:f\sim\lambda u.w),\ w=\alpha(f)\\
\delta(f) & : \prod\{w:\mathcal{L}_P(X)\}(H:f\sim\lambda u.w)(u:P(b)),\ \gamma(f,H)
\bullet H(u)=\beta(f,u)
\end{align*}
...
\begin{align*}
I & : \prod(x:X),\ \Lambda(i(x))
\intertext{and for every $b:B$, $f:P(b)\to\mathcal{L}_P(X)$ and $F:\prod(u:P(b)),\ \Lambda(f(u))$}
\epsilon(F) & : \Lambda(\alpha(f))\\
\zeta(F) & : \prod(u:P(b)),\ \beta(f,u)\cdot F(f(u))= \epsilon(F)
\intertext{and for every $w_0:\mathcal{L}_P(X)$, $w_1:\Lambda(w_0)$, 
$H_0:f\sim\lambda u.w$ and $H_1:\prod(u:P(b)),\ H_0(u)\cdot F(f(u))= w_1$
a path}
...
\end{equation*}
but it should obviously be there), then there is a section $s:\prod(w:\mathcal{L}_P(X))$
with the property that there is a term
\begin{align*}
\kappa & : \prod(x:X),\ s(i(x))= I(x)
\intertext{and for every $b:B$ and $f:P(b)\to\mathcal{L}_P(X)$ there are terms}
\mu(f) & : s(\alpha(f))= \epsilon(s\circ f)\\
\nu(f) & : \prod(u:P(b)),\ \mu(f)\bullet s(\beta(f,u))=\zeta(s\circ f,\beta(f))\\
\xi(f) & : \prod(w:\mathcal{L}_P(X))(H:f\sim\lambda u.w),\ \mu(f)\bullet s(\gamma(f,H))=\mreturn(s\circ f,s\circ H)
\end{align*}
...
\begin{equation*}
\mathcal{E}_\modal(f)\equiv \prod(b:B),\
\mathsf{isContr}(\modal\mathsf{hFiber}(f,b)).
\end{equation*}
...
\begin{equation*}
\mathsf{conn}_\modal(f):=\prod(b:B),\ \mathsf{isContr}(\modal\mathsf{hFiber}(f,b)). 
\end{equation*}
...
$\modal$-connected if and only if $\mathsf{hFiber}(f,b)$ is \modal-connected for every $b:B$. 
...
If a function $g:A\to B$ is a retract of a function $f:X\to Y$, then $\mathsf{hFiber}(g,b)$ is a retract of $\mathsf{hFiber}(f,s^\prime(b))$
...
\begin{align*}
\varphi & :=\lambda\pairr{a,p}.\pairr{s(a),s^\prime(p)\bullet L(a)} & & :\mathsf{hFiber}(g,b)\to\mathsf{hFiber}(f,s^\prime(b))\\
\psi & := \lambda\pairr{x,q}.\pairr{r(x),R^\prime(b)\bullet r^\prime(q)\bullet K(x)} & &
:\mathsf{hFiber}(f,s^\prime(b))\to\mathsf{hFiber}(g,b)
\end{align*}
Then we have $\psi(\varphi(\pairr{a,p}))\equiv\pairr{r(s(a)),R^\prime(b)\bullet r^\prime(s^\prime(p)\bullet L(a))\bullet K(s(a))}$. Our goal
is to show that
\begin{equation*}
\prod(b:B)(\pairr{a,p}:\mathsf{hFiber}(g,b)),\ \psi(\varphi(\pairr{a,p}))= \pairr{a,p},
\end{equation*}
which is of course equivalent to the type
\begin{equation*}
\prod(a:A),\ \psi(\varphi(\pairr{a,\idfunc{g(a)}})=\pairr{a,\idfunc{g(a)}}
\end{equation*}
...
$H(a)$. This finishes the proof that $\mathsf{hFiber}(g,b)$ is a retract of $\mathsf{hFiber}(f,s^\prime(b))$ for every $b:B$.
\end{proof}
...
\begin{align*}
\modal\mathsf{hFiber}(g\circ f,c)& \simeq \modal \sum(w:\mathsf{hFiber}(g,c)),\ \mathsf{hFiber}(f,\proj1 w)\\
& \simeq \modal\sum(w:\mathsf{hFiber}(g,c)),\ \modal\mathsf{hFiber}(f,\proj1 w)\\
& \simeq \modal\mathsf{hFiber}(g,c).
\end{align*}
It follows that $\modal\mathsf{hFiber}(g,c)$ is contractible if and only if $\modal\mathsf{hFiber}(g\circ f,c)$ is
contractible.
\end{proof}
...
\begin{equation*}
\varphi :\big(\prod(b:B),\ P(b)\big)\to\big(\prod(a:A),\ P(f(a))\big)
\end{equation*}
...
\begin{align*}
\prod(b:B),\ P(b) & \simeq \prod(b:B),\ \modal\mathsf{hFiber}(f,b)\to P(b)\\
& \simeq \prod(b:B),\ \mathsf{hFiber}(f,b)\to P(b)\\
& \simeq \prod(b:B)(a:A)(p:f(a)= b),\ P(b)\\
& \simeq \prod(a:A),\ P(f(a)).
\end{align*}
We omit the proof that this equivalence is indeed given by $\lambda s.s\circ f$. 

For the other direction, suppose that the function $\lambda s.s\circ f$ from $(\prod(b:B),\ P(b))$ to $(\prod(a:A),\ P(f(a)))$ is an
equivalence for each dependent type $P:B\to\type$ of homotopy level $n$. Considering the dependent type 
\begin{equation*}
b:B\vdash\modal\mathsf{hFiber}(f,b):\modaltype,
\end{equation*}
we obtain from the assumed equivalence a term $c$ of type $\prod(b:B),\ \modal\mathsf{hFiber}(f,b)$ with
$c(f(a))\equiv\mreturn(\pair{a,\idfunc{f(a)}})$. To show that each $\modal\mathsf{hFiber}(f,b)$ is contractible 
we will find a function of type
\begin{equation*}
\prod(b:B)(w:\modal\mathsf{hFiber}(f,b)),\ w= c(b).
\end{equation*}
By the universal property of \modal\ the above type is equivalent to
\begin{equation*}
\prod(b:B)(a:A)(p:f(a)= b),\ \mreturn(\pairr{a,p})= c(b),
\end{equation*}
which is furthermore equivalent to the type
\begin{equation*}
\prod(a:A),\ \mreturn(\pairr{a,\idfunc{f(a)}})= c(f(a)).
\end{equation*}
...
\begin{equation*}
\varphi:\big(\prod(b:B),\ \mathsf{hFiber}(g,b)\big)\to\big(\prod(a:A),\ \mathsf{hFiber}(g,f(a))\big)
\end{equation*}
defined by $\varphi(s):=s\circ f$ is an equivalence.
\end{cor}
...
Let $f:A\to B$ be a function and $P:A\to\type$ and $Q:B\to\type$ be dependent types. Suppose that $g:\prod\{a:A\},\ P(a)\to Q(f(a))$
...
\begin{align*}
\modal\mathsf{hFiber}(\varphi,\pairr{b,v}) & \simeq \modal\sum(a:A)(u:P(a))(p:f(a)= b),\ f(p)\cdot g(u)= v\\
& \simeq \modal\sum(w:\mathsf{hFiber}(f,b))(u:P(\proj1(w))),\ g(u)= f(p)^{-1}\cdot v\\
& \simeq \modal\sum(w:\mathsf{hFiber}(f,b)),\ \modal\mathsf{hFiber}(g(\proj1 w),f(p)^{-1}\cdot v)\\
& \simeq \modal\mathsf{hFiber}(f,b)
\end{align*}
...
\begin{equation*}
\im_\modal(f):=\sum(b:B),\ \modal\mathsf{hFiber}(f,b)
\end{equation*}
...
equivalence
\begin{equation*}
\|\mathsf{hFiber}(P,X)\|\to (X\simeq\mathsf{unit}))\simeq(\mathsf{hFiber}(P,X)\to(X\simeq\mathsf{unit})).
\end{equation*}
The latter type is clearly inhabited and the type $\|\mathsf{hFiber}(P,X)\|$ is inhabited by assumption. Therefore, there is a term
...
\begin{equation*}
\mathsf{hFiber}(\Sigma_A f,\pairr{x,v})\simeq\mathsf{hFiber}(f(x),v)
\end{equation*}
for each $x:A$ and $v:Q(x)$. Hence $\modal\mathsf{hFiber}(\Sigma_A f,\pairr{x,v})$ is contractible if and only if
$\modal\mathsf{hFiber}(f(x),v)$ is contractible.
\end{proof}
...
\begin{proof}
Note that $A\simeq\sum(b:B),\ \mathsf{hFiber}(f,b)$. The function $\tilde{f}$ is the function on total spaces induced by the fiberwise
transformation
\begin{equation*}
\lambda b.\eta^{\mathsf{hFiber}(f,b)}:\prod(b:B),\ \mathsf{hFiber}(f,b)\to\modal\mathsf{hFiber}(f,b).
\end{equation*}
...
\begin{prop}\label{prop:factor_equiv_fiber}
...
\begin{equation*}
E(H,b):\mathsf{hFiber}(h_1,b)\simeq\mathsf{hFiber}(h_2,b)
\end{equation*}
...
\begin{align*}
\mathsf{hFiber}(h_1,b) & \simeq \sum(w:\mathsf{hFiber}(h_1,b)),\ \modal \mathsf{hFiber}(g_1,\proj1 w)\\
& \simeq \modal\sum(w:\mathsf{hFiber}(h_1,b)),\ \mathsf{hFiber}(g_1,\proj1 w)\\
& \simeq \modal\mathsf{hFiber}(h_1\circ g_1,b)
\end{align*}
...
homotopy $H:h_1\circ g_1\sim h_2\circ g_2$, there is an obvious equivalence $\mathsf{hFiber}(h_1\circ g_1,b)\simeq\mathsf{hFiber}(h_2\circ
g_2,b)$. Hence we obtain
\begin{equation*}
\mathsf{hFiber}(h_1,b)\simeq\mathsf{hFiber}(h_2,b)
\end{equation*}
...
\begin{equation*}
\prod(b:B),\ \mathsf{hFiber}(h_1,b)\to\mathsf{hFiber}(h_2,b).
\end{equation*}
This induces an equivalence of total spaces, i.e.\ we have
\begin{equation*}
\sum(b:B),\ \mathsf{hFiber}(h_1,b)\simeq\sum(b:B),\ \mathsf{hFiber}(h_2,b)
\end{equation*}
Of course, we also have the familiar equivalences $X_1\simeq\sum(b:B),\ \mathsf{hFiber}(h_1,b)$ and $X_2\simeq\sum(b:B),\
\mathsf{hFiber}(h_2,b)$. This gives us our equivalence $e(H):X_1\simeq X_2$. The reader may verify that the underlying function
...
\begin{equation*}
\prod C, \prod(g:\modalfunc(B\to C)),\ \mathsf{isContr}\big(\sum(h:\modalfunc(B\to C)),\ \underline{h}\circ
...
Let $P:X\to\type$ and $R:\prod(x:X),$ $(P(x)\to\modaltype)$. Then there is a function of type
\begin{equation*}
\prod(x:X),\ \existsmodalunique(R(x))\to\sum\big(f:\prod(x:X),\ P(x)\big)\prod(x:X),\ R(x,f(x)).
\end{equation*}
\end{thm}

\begin{proof}
Suppose that $\prod(x:X),\ \existsmodalunique(R(x))$. By lemma \ref{lem:iota}, 
...
\begin{equation*}
\mathsf{atMostOne}(P) \equiv  \prod(x,y:A),\ P(x)\to P(y)\to(x= y)
\end{equation*}
...
\begin{equation*}
\mathsf{isprop}(P)\simeq \prod(x,y:A),\ P(x)\to P(y)\to\mathsf{isContr}(x= y)
\end{equation*}
...
\begin{equation*}
\mathsf{atMostOne}(P)\to\prod(x,y:A),\ P(x)\to P(y)\to\mathsf{isContr}(x= y).
\end{equation*}
...
\begin{lem}
Suppose that $H:\prod(x,y:A),\ P(x)\to P(y)\to (x= y)$. Then for any $p:x= x^\prime$ and $q:y= y^\prime$ there are paths
\begin{align*}
...
\begin{equation*}
\mathsf{atMostOne}(P)\to\sum(H^\prime:\mathsf{atMostOne}(P))\prod(x:X)(u:P(x)),\ H^\prime(x,x,u,u)=\idfunc{x}.
\end{equation*}
This is easy, since we can define the function $H^\prime:\prod(x,y:X),\ P(x)\to P(y)\to(x= y)$ by
\begin{equation*}
...
Let $P:X\to\type$ and $R:\prod(x:X),$ $(P(x)\to\mathsf{hProp})$. Then there is a function
\begin{equation*}
\big(\prod(x:X),\ \exists !(u:P(x)),\ R(x,u)\big)\to\sum\big(f:\prod(x:X),\ P(x)\big)\prod(x:X),\ R(x,f(x)).
\end{equation*}
\end{cor}
...
\end{tikzpicture}
\end{equation*}
is a pullback square and let $b:B$. Then $\mathsf{hFiber}(f,b)\simeq\mathsf{hFiber}(g,h(b))$.
\end{lem}
...
\end{tikzpicture}
\end{equation*}
is the pullback of the left square if and only if it is the pullback of the outer rectangle: $\mathsf{hFiber}(f,b)$ is the pullback of the
square on the left and $\mathsf{hFiber}(g,h(b))$ is the pullback of the outer rectangle.
\end{proof}
...
& \simeq \sum(b:B)(d:D)(w:\modal\mathsf{hFiber}(g,d)),\ h(b)= d\\
& \simeq \sum(b:B),\ \modal\mathsf{hFiber}(g,h(b)).\\
& \simeq \sum(b:B),\ \modal\mathsf{hFiber}(f,b)\\
& \equiv \im_\modal(f).
\end{align*}
...
& \simeq \sum(b:B),\ \mathsf{hFiber}(f,b)\\
& \simeq A.
\end{align*}
\end{proof}
