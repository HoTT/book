...
\indexdef{axiom!excluded middle}%
\indexdef{law of excluded middle}%
...
\begin{defn}\label{defn:decidable-equality}
  \mbox{}
  \begin{enumerate}
  \item A type $A$ is called \define{decidable} if $A+\neg A$.
  \item Similarly, a type family $B:A\to \type$ is \define{decidable} if \narrowequation{\prd{a:A} (B(a)+\neg B(a)).}
  \item In particular, $A$ has \define{decidable equality} if \narrowequation{\prd{a,b:A} ((a=b) + \neg(a=b)).}
  \end{enumerate}
\end{defn}
...
If $A$ is a set, we call \eqref{eq:subset} a \define{subset} of $A$; for general $A$ we might call it a \define{subtype}.
\symlabel{subset}
If $\setof{x:A | Q(x)}$ is another subset of $A$, then we say that $P$ is \define{contained} in $Q$, and write $P\subseteq Q$, if $\prd{x:A}(P(x)\to Q(x))$.
