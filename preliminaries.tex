...
paradox \cite{thierry:trees}, or alternatively, in order to avoid the use of
...
construct 
\[ (\lam{x:A}b) : A \to B \]
We may also write it more briefly as
\[ (x \mapsto b) : A \to B \]
...
We note parenthetically that we do not assume that $\lam{x:A}f(x)$ is judgmentally equal to $f$, as is often done in type theory; here $f:A \to B$ should contain no occurrence of $x$.  However, we will be able to prove $\lam{x:A} f(x) = f$ using the principle of function extensionality, which we are going to assume.
...
given $A:U$, $B : A \to U$ and $C : \prd{x:A}B(x) \to C(x,y)$ we construct
...
\end{tabular}
\end{center}
Clearly, this is something we don't want to do in general --- however, it is good to have a precise reference when in doubt. As an exercise we can verify 
propositional tautologies, like \emph{If not ($A$ or $B$), then  (not $A$) and (not $B$)} by exhibiting an element of 
...
The intuitive reason is that the premise doesn't offer any information that would help us to decide wither we should prove the disjunction using $\inl$ or $\inr$.

In predicate logic types play a dual role: they serve is propositions and as types in the conventional sense, i.e. domains we quantify over. Indeed, we are going to exploit this duality meaning that we can also quantify over the type of proofs of a proposition. A predicate over a type $A$ is representing as a family $P : A \to U$ assigning to every element $a : A$ a type $P(a)$ corresponding to the proposition that $P$ holds for $a$. We extend our translation with an explanation of the quantifiers:
\begin{center}
\begin{tabular}{c|l}
  \textbf{English} & \textbf{Translation into Type Theory}\\\hline
...
where $A : \UU$ and $a,b : A$. However, unless we assume the existence of an impredicative universe of propositions, note that the proposition lives in a different, higher universe, then the propositions we are quantifying over.
