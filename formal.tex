...
\]
Here each pair $x_i:A_i$ in the list 
$\Gamma$ consists of a variable $x_i$, distinct from each of the variables 
$x_1,\dots,x_{i-1}$, and a
type $A_i$ in which at most the variables $x_1,\dots,x_{i-1}$ occur freely, 
signifying
...
%equivalence 
relation generated by all instances of the $\beta$-reduction
...
Together with the $\beta$-reduction defining equation for lambda abstraction
\[
  (\lam{x} t)(u) \defeq t[u/x]
\]
and the defining equations for the defined constants we have
$\defeq$ as a rewriting rule (unfolding definitions).  This forms a rewriting 
...
%TODO
%TODO we give some examples of inductive types
...
\and
  ...
  {\oftp{\Gm,\tmtp xA}{B}{\UU_i} \\
   ...
   \oftp{\Gm,\tmtp y{\tsm{x:A} B}}{C}{\UU_i} \\
   \oftp{\Gm,\tmtp aA,\tmtp b{B[a/x]}}{g}{C[\tup ab/y]}}
  {\oftp\Gm{\ind{\tsm{x:A} B}(y.C,a.b.g,\tup{a'}{b'})}{C[a',b'/a,b]}}
\end{mathparpagebreakable}
...
%
%\begin{mathparpagebreakable}
%  \inferrule*[right=$\wtypesym$-\form]{\Gamma,\ x \oftype A \vdash B(x)\UU_i}{\oftp\Gm \wtypesym_{x \oftype A} B(x)\UU_i}
%\and
