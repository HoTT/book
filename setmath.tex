...
Recall that a function $f:A\to B$ is called \define{surjective} if $\fall{b:B}\brck{\hfib f b}$, or equivalently $\fall{b:B} \exis{a:A} f(a)=b$.
...
Recall from \autoref{sec:logic} the following definitions of the variations on the axiom of choice and the law of excluded middle, where $A$ is a type in $\type$, $B : A \to \type$ is a type family over $A$, while $P$ has type $\prd{a : A} B(a) \to \type$ and $Q$ has type $\prd{a : A} B(a) \to \prop$:
%
\begin{align*}
...
\begin{ex}
  Show that, unlike  the category $\set$, the category $\mathcal{T}ype$ of all types is not a pretopos.
  \end{ex}
