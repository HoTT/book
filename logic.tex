...
\begin{align*}
  ...
  \prd{A:\UU}(\neg\neg A \to A).
\end{align*}
For emphasis, the proper versions~\eqref{eq:lem} and~\eqref{eq:ldn} may be denoted $\mathsf{LEM}_{-1}$ and $\mathsf{DN}_{-1}$; see also \autoref{ex:lemnm}.
...
\begin{defn}\label{defn:decidable-equality}
  Let $A$ be a type.
  \begin{enumerate}
  \item $A$ is called \define{decidable} if $A+\neg A$.
  \item Similarly, a type family $B:A\to \type$ is \define{decidable} if $\prd{a:A} (B(a)+\neg B(a))$.
  \item In particular, $A$ has \define{decidable equality} if $\prd{a,b:A} ((a=b) + \neg(a=b))$.
  \end{enumerate}
\end{defn}

Thus, $\mathsf{LEM}$ is exactly the statement that all mere propositions are decidable, and hence so are all families of mere propositions.
In particular, $\mathsf{LEM}$ implies that all sets (in the sense of \autoref{sec:basics-sets}) have decidable equality.
More generally, $\mathsf{LEM}$ implies that all types have \emph{decidable mere equality}, i.e.\ every $A:\UU$ satisfies
\[ \prd{a,b:A} (\brck{a=b} + \neg\brck{a=b}). \]
Having decidable equality is much stronger than decidable mere equality; it says that a path $x=y$ can be chosen, when it exists, continuously (or computably, or functorially) in $x$ and $y$.
This turns out to imply that the type must be a set; see \autoref{thm:hedberg}.
...
We refer to this axiom as \define{propositional resizing}, since it means that any mere proposition in the universe $\UU_{i+1}$ can be ``resized'' to an equivalent one in the smaller universe $\UU_i$.
...
\end{itemize}
The \define{axiom of choice} $\mathsf{AC}$ asserts that under these assumptions,
...
We may also replace the propositional truncation by the more general $n$-truncations to be considered in \autoref{cha:hlevels}, obtaining a spectrum of axioms AC$_n$ interpolating between~\eqref{eq:ac}, which we call simply AC (or AC$_{-1}$ for emphasis), and \autoref{thm:ttac}, which we shall call AC$_\infty$.
...
By definition, a \define{retraction} is a function $r : A \to B$ such that there exists a function $s : B \to A$, called its \define{section}, and a homotopy $\epsilon:\prd{b:B} (r(s(y))=y)$; then we say that $B$ is a \define{retract} of $A$.
...
The implication that the propositions-as-types form of LEM contradicts univalence was observed by Mart\'\i n Escard\'o on the \Agda\ 
mailing list.
...
The principle of unique choice fails in the category of so-called setoids in \Coq~\cite{Spiwack}, in logic enriched type 
theory~\cite{aczel2002collection} and in minimal type theory~\cite{maietti2005toward}. The univalence axiom thus makes our type theory 
behave more like the internal logic of a topos;~see also \autoref{cha:set-math}.

Martin-L\"of~\cite{martin2006100} provides a discussion on the history of axioms of choice. 
For background on constructive logic see for instance~\cite{TroelstraI,TroelstraII}.
