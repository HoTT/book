...
In homotopy theory, a type family $P:A\to\type$ corresponds to a \emph{fibration} over $A$, and we may sometimes use that name equivalently.
...
Another piece of homotopical terminology is that given two type families $P,Q:A\to\type$, we may refer to a function $f:\prd{x:A} (P(x)\to Q(x))$ as a \emph{fiberwise map} or a \emph{fiberwise transformation}.
...
\begin{ex}
  Prove that equivalences satisfy the \emph{2-out-of-3 property}: given $f:A\to B$ and $g:B\to C$, if any two of $f$, $g$, and $g\circ f$ are equivalences, so is the third.
\end{ex}
