...
\begin{defn}\label{def:hlevel}
  ...
  \indexdef{ntype@$n$-type}%
  \indexdef{ntruncated@$n$-truncated!type}%
  \indexdef{type!ntype@$n$-type}%
  \indexdef{type!ntruncated@$n$-truncated}%
...
% Suppose that $x,y:A$; then by the inductive hypothesis it suffices to show that every function $\Sn^n\to (x= y)$ is null-homotopic.
...
\begin{proof}
  ...
  But since $(-\circ \tprojf n)$ is an equivalence, there must be a path $\trunc nf = \trunc ng$ inducing it, and the coherence laws for function extensionality imply~\eqref{eq:trunc-htpy}.
\end{proof}
...
\indexdef{function!nconnected@$n$-connected}%
\indexdef{nconnected@$n$-connected!function}%
...
\indexdef{nconnected@$n$-connected!type}%
\indexdef{type!nconnected@$n$-connected}%
...
\begin{defn}
  ...
  \indexdef{ntruncated@$n$-truncated!function}%
  \indexdef{function!ntruncated@$n$-truncated}%
...
\indexdef{image}%
\indexdef{image!nimage@$n$-image}%
\indexdef{nimage@$n$-image}%
\indexdef{function!nimage of@$n$-image of}%
of $f$ is defined as
\begin{equation*}
...
\[E(H,h_1(g_1(a)))(\pairr{g_1(a),\refl{h_1(g_1(a))}}) \jdeq \pairr{g_2(a),H(a)^{-1}}.\]
\end{lem}
...
\begin{equation*}
(\lam{b} E(H,b)) : \prd{b:B} \hfib{h_1}b\to\hfib{h_2}b.
\end{equation*}
...
\end{equation*}
where $E$ is as in \autoref{prop:factor_equiv_fiber}.
\item By \autoref{prop:factor_equiv_fiber} we have $e(g_1(a))\jdeq g_2(a)$, so we may choose $\zeta(a)\defeq \refl{g_2(a)}$. \label{item:orth-fact-2}
\item For every $x:X_1$, we have
\begin{equation*}
\proj2(E(H,h_1(x))({x,\refl{h_1(x)}})) :h_2(e(x))= h_1(x),
\end{equation*}
giving us a homotopy $\eta:h_2\circ e\htpy h_1$.
\item By \autoref{prop:factor_equiv_fiber} we have $\eta(g_1(a)) \jdeq H(a)^{-1}$ and by~\ref{item:orth-fact-2} we have
$\apfunc{h_2}({\zeta(a)})\jdeq \refl{h_2(g_2(a))}$. Thus we have
\begin{align*}
  \opp{\apfunc{h_2}(\zeta(a))} \ct \eta(g_1(a)) \ct H_1(a)
  &\jdeq \opp{(\refl{h_2(g_2(a))})} \ct H(a)^{-1} \ct H_1(a)\\
  &= H_2(a) \ct \opp{H_1(a)} \ct H_1(a)\\
  &= H_2(a).\qedhere
\end{align*}
\end{enumerate}
\end{proof}
...
\indexdef{function!.connected@$\modal$-connected}%
\indexdef{.circle-connected@$\modal$-connected!function}%
if $\modal(\hfib f b)$ is contractible for all $b:B$, and to be \define{$\modal$-truncated}
\indexdef{function!.truncated@$\modal$-truncated}%
\indexdef{.circle-truncated@$\modal$-truncated!function}%
...
On the computational side, monads (and hence modalities\index{modality}) are used to model effects in functional programming~\cite{Moggi89}.%
\index{programming}%
\index{effect}%
...
\begin{ex}\label{ex:acconn}
  ...
  \indexdef{nconnected@$n$-connected!axiom of choice}%
  \indexdef{axiom!of choice!nconnected@$n$-connected}%
  to be the statement
  \begin{quote}
...
\begin{ex}
  ...
  \indexdef{flocal type@$f$-local type}%
  \indexdef{type!flocal@$f$-local}%
