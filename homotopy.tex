...
\note{move to chapter 6 section on connectedness}

\subsection{Consequences for Homotopy Groups of Spheres}

As a corollary, we have 
\begin{cor}[Freudenthal Equivalence]
Suppose that $X$ is $n$-connected and pointed, with $n\geq 0$.
...
spheres are stable in a certain range
\begin{cor}[Stability of Homotopy Groups of Spheres]
\note{TODO}
\end{cor}
...
\begin{proof}
We already have $\pi_1(\Sn ^1) = \Z$ (\cref{cor:pi1s1}) and $\pi_2(\Sn
^2) = \Z$ (\cref{cor:pis2-hopf}).  To complete the proof, we will show
that $\pi_{n+1}(\Sn ^{n+1}) = \pi_n(\Sn ^ n)$ for $n \ge 2$ as a
corollary of the Freudenthal suspension theorem.  Then the result
