...
As mentioned in the introduction, \emph{univalence} is the identification of $\id[\type]AB$ with the type $(A\eqv B)$ of equivalences from $A$ to $B$, which we described in \S\ref{sec:basics-equivalences}.
...
\end{lem}
\begin{proof}
  Given $p:A=B$, we need to construct an equivalence $A\eqv B$.
  By induction, it suffices to assume that $p$ is $\refl A$, in which case we require an equivalence $A\eqv A$.
  But it is easy to show that the identity function is an equivalence.
\end{proof}
