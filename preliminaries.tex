...
We then have the definitional equality\footnote{Use of this equality is often referred to as $\beta$-conversion%
...
Formally, this means it does not include certain classical principles, such as the law of excluded middle (\LEM{}) or proof by contradiction\footnote{Note that the pattern ``assume $A$ and derive a contradiction'' is perfectly valid in constructive logic as a way (in fact, \emph{the} way) to prove ``not $A$''; indeed, we used it in our example above. The form of ``proof by contradiction'' which is disallowed is ``assume not $A$ and derive a contradiction'' as a way of proving $A$.\indexfoot{proof!by contradiction}},
