...
with a topology, and a path between points $x$ and $y$ is represented by
...
\emph{homotopy}.  A homotopy between a pair of continuous maps $f \colon
X_1 \to X_2$ and $g \colon X_1\to X_2$ is a continuous map $H \colon X_1
...
$q$, or a 2-dimensional \emph{path between paths}.  For example, because
...
equipped with some complex algebraic structure.  Morphisms at each level
...
Every topological space X has a \emph{fundamental $\infty$-groupoid} whose
$k$-morphisms are the $k$-dimen\-sional paths in $X$.  The weakness of the
$\infty$-groupoid corresponds directly to the fact that paths form a
...
One of the nice things about homotopy type theory is that all of this structure can be \define{proven} starting from only the inductive property of identity types, so we can make explicit as much or as little of it as we need.
...
Thus, given a type $A$ with a point $a:A$, we define its \define{loop space} $\Omega(A,a)$ to be the type $\id[A]{a}{a}$.
...
\end{equation*}
%
between them, defined as follows: first let $\alpha \rightwhisker r : p\ct r = q\ct r$ be determined by path induction on $r$, then let $q\leftwhisker \beta : q\ct r = q\ct s$ be given by induction on $q$.  Since these paths are composable in the type of 2-paths, we can define the \define{horizontal composition}  by:
\[
...
\begin{defn} \label{def:pointedtype}
  A \define{pointed type} $(A,a)$ is a type $A:\type$ together with a point $a:A$.
  We write $\pointed{\type} \defeq \sm{A:\type} A$ for the type of pointed types in the universe $\type$.
\end{defn}

\begin{defn} \label{def:loopspace}
  Given a pointed type $(A,a)$, we define the \define{loop space} of $(A,a)$ to be the following pointed type:
  \[\Omega(A,a)=((\id[A]aa),\refl{A}(a)).\]
  For $n:\N$, the \define{$n$-fold iterated loop space} of a pointed type $(A,a)$ is defined recursively by:
  \begin{align*}
    \Omega^0(A,a)&=(A,a)\\
...
% \item If $f:\prd{x:A}P(x)$ then $\mapdep{f}{p}:\trans{p}{f(x)}=f(y)$, for $p:x=y$, defined by
...
\begin{defn} \label{defn:homotopy}
  ...
  A \define{homotopy} from $f$ to $g$ is a dependent function of type
  \begin{equation*}
...
\begin{defn}
  For a function $f:A\to B$, a \define{quasi-inverse} of $f$ is a triple $(g,\alpha,\beta)$ consisting of a function $g:B\to A$ and homotopies
$\alpha:f\circ g\htpy \idfunc[B]$ and $\beta:g\circ f\htpy \idfunc[A]$.
\end{defn}

Thus,~\eqref{eq:qinvtype} is \define{the type of quasi-inverses of $f$}; we may denote it by $\qinv(f)$.
...
\begin{proof}
  ...
  But now by path induction (in the Paulin-Mohring style), it suffices to consider $x\jdeq\inl(a_0)$ and $p\jdeq \refl{\inl(a_0)}$:
  \begin{align*}
    \decode(x,\encode(x,p))
...
We now consider one example to illustrate the groupoid structure of type
formers.  In the introduction to the book, we remarked that one of the
...
therefore interchangable, which includes the common practice in set
theory of identifying bijective sets.  Moreover, when we define other
mathematical concepts as types equipped with structure or properties, we
...
isomorphism, etc.  In this section, we describe a simpler example,
coming from algebra.
...
Given a type $A$, the type \semigroupstr{A} of semigroup structures on $A$ is defined by
\[
\semigroupstr{A} \defeq \sm{m:A \to A \to A} \prd{x,y,z:A} m(x,m(y,z)) = m(m(x,y),z)
\]

A semigroup is a type paired with such a structure:
\[
...
it easier to work with semigroups.
...
$\transfibf{C}{(\opp \alpha)}$ (by \cref{thm:transport-concat} and
\cref{thm:omg}).  
...
\begin{align} 
  & \transfib{X \mapsto \semigroupstr{X}}{\ua(e)}{(m,a)} \notag \\
= & (\transfib{X \mapsto (X \to X \to X)}{\ua(e)}{m}, 
   \transfibf{(X,m) \mapsto \mathit{Assoc}(X,m)}{(\pairpath(\ua(e),\refl{}))}{a}) \label{eq:transport-semigroup-step1}
\end{align}
where $\mathit{Assoc}(X,m)$ is the type $\prd{x,y,z:X} m(x,m(y,z)) = m(m(x,y),z)$.  
...
\cref{eq:transport-arrow} twice, we have that $m'(b_1,b_2)$ is equal to
\[
\transfib{X \mapsto X}{\ua(e)}{m(\transfib{X \mapsto X}{\opp{\ua(e)}}{b_1}, \transfib{X \mapsto X}{\opp{\ua(e)}}{b_2})} 
\]
Then, by \cref{axiom:univalence}, this is equal to
\[
...
in \cref{eq:transport-semigroup-step1}) is equal to a function sending
$b_1,b_2,b_3 : B$ to a path given by the following steps:
\begin{align}
  & m'(b_1,m'(b_2,b_3)) \label{eq:transport-semigroup-assoc}\\
...
two such associativity proofs are always equal.

\subsection{Equality of Semigroups}

Using the equations for path spaces discussed in the previous sections,
we can investigste when two semigroups are equal. Given semigroups
$(A,m,a)$ and $(B,m',a')$, by \cref{thm:path-sigma}, the type of paths
$(A,m,a) =_\semigroup (B,m',a')$ is equal to the type of pairs
\begin{flalign*}
& p_1 : A =_{\type} B \\
...
\transfibf{X \mapsto \semigroupstr X}{}, $p_2$ is equivalent to a pair
of proofs, the first of which shows that
\begin{align} \label{eq:equality-semigroup-mult}
...
\cref{eq:transport-semigroup-assoc}.  But by cancelation of inverses
\cref{eq:equality-semigroup-mult} is equivalent to
\[
...
(again, see \cref{cha:logic}), associativity becomes a property, so the
...
that preserves the multiplcation, and showing that equality of
semigroups is the same as two mutually inverse homomorphisms; but we
will not show the details here.

This shows that univalence allows us to construct a notion of semigroup
in such a way that isomorphic semigroups are equal.  More generally,
when we construct algebraic structures in HoTT, all constructions on
them automatically respect isomorphism, without any tedious proofs or
abuse of notation.
