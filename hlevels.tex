...
A function $f:A\to B$ is $n$-connected if and only if for every $P:B\to\ntype{n}$, the function
\begin{equation*}
...
For the other direction, suppose that the function $\lam{s} s\circ f$ from $\prd{b:B} P(b)$ to $\prd{a:A} P(f(a))$ is an
equivalence for each $P:B\to\ntype{n}$. Considering the type family defined by
\begin{equation*}
P(b)\defeq \trunc n{\hfib{f}b}
\end{equation*}
we obtain from the assumed equivalence a function $c:\prd{b:B} \trunc n{\hfib{f}b}$ with
