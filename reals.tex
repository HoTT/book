...
It is worth pointing out that the universal cover of the circle, which was constructed in
\autoref{S1-universal-cover}, is \emph{not} the type of reals we are looking for.
That type is contractible, and thus equivalent to the singleton type, so it cannot be
equipped with a non-trivial algebraic structure.
...
Let us first recall informally the basic idea of Dedekind's construction. A \emph{Dedekind
  cut} consists of a pair $(L, U)$ of subsets $L, U \subseteq \Q$, called the \emph{lower}
and \emph{upper cut} respectively, which are:
% 
\begin{enumerate}
...
\item \emph{disjoint:} $\lnot (q \in L \land q \in U)$.
\item \emph{located:} $q < r \Rightarrow q \in L \lor r \in U$.
\end{enumerate}
%
You might have seen a different formulation, for example one which only uses lower cuts,
but that one only works classically, whereas double-sided cuts work constructively as
well. Reading the roundedness condition from left to right tells us that cuts are
\emph{open}, and from right to left that they are \emph{lower} sets. The locatedness
condition states that there is no large gap between $L$ and $U$.
...
\section{A Taste of Analysis}
\label{sec:taste-analysis}
