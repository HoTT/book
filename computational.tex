...
\begin{equation*}
g : \prd{w,w':\sm{x:A}P(x)} \sm{p:\proj{1}(w)=\proj{1}(w')}\trans{p}{\proj{2}(w)}=\proj{2}(w^\prime) \;\to\; (w=w')
\end{equation*}
by first inducting on $w$ and $w'$, which splits them into $(w_1,w_2)$ and
$(w_1',w_2')$ respectively, so it suffices to show 
\begin{equation*}
\sm{p:w_1 = w_1'}\trans{p}{w_2}=w_2^\prime \;\to\; ((w_1,w_2)=(w_1',w_2'))
\end{equation*}
...
$r$, where $r$ has type $\sm{p:\proj{1}(w)=\proj{1}(w')}\trans{p}{\proj{2}(w)}=\proj{2}(w^\prime)$.
First, we break apart the pairs $w$, $w'$, $r$ by pair induction, as in the
...
The action of transport on $\Sigma$-types is a generalization of the
action for binary cartesian products:
\begin{thm}
...
\end{equation*}
The path $\mathsf{lift}(u,p)$ is defined in \autoref{thm:path-lifting}.
\end{thm}
...
the reflexivity, inverses, and composition of $\sm{x:A} B$
componentwise.
...
Unlike the case in the previous sections, the basic type theory presented in \autoref{cha:typetheory} is insufficient to prove~\eqref{eq:path-forall}.
...
We will see in later chapters that this axiom follows either from univalence (see \autoref{sec:compute-universe} and Appendix~\ref{cha:univalence}) or from an interval type (see \autoref{sec:interval}).
...
As we did in \autoref{sec:compute-cartprod} and \autoref{sec:compute-pi}, it is useful to break this equivalence into:
...
\item The elimination rule, which is $\idtoeqv$:
  \[
  \idtoeqv \jdeq \transfibf{X \mapsto X} : (\id{A}{B}) \to (\eqv A B)
  \]
\item $\beta$-reduction: 
  \[
  \begin{array}{l}
...
Thus, we postpone them until \autoref{cha:equivalences}.
...
paths $p,q : \id[A]{a}{a'}$.
However, our other general classes of theorems do extend to identity types such, such as the fact that they respect equivalence.
...
For example:
\begin{itemize}
\item Paths $p = q$, where $p,q : \id[A \times B]{w}{w'}$, are equivalent
  to pairs of paths $\id[{\id[A]{\proj{1} w}{\proj{1} w'}}]{\projpath{1}{p}}{\projpath{1}{q}}$ 
  and $\id[{\id[B]{\proj{2} w}{\proj{2} w'}}]{\projpath{2}{p}}{\projpath{2}{q}}$.  

\item Paths $p = q$, where $p,q : \id[\prd{x:A} B(x)]{f}{g}$, are equivalent
  to homotopies $\prd{x:A} \id[B(x)] {\happly(p)(x)}{\happly(q)(x)}$.  
\end{itemize}

Next we consider $\transfib{x \mapsto \id[B(x)]{f(x)}{g(x)}}{p}{q}$, i.e.\ transport in a path fibration. This is given by pre- and post-composition, analogously to transport at $\to$.

\begin{thm}\label{thm:transport-path}
Let $ A : \type$ with $a,a':A$ and $B : A \to \type$ and $f,g : \prd{x:A} B(x)$ and $p :
\id[A]{a}{a'}$ and $q : \id[B(a)]{f(a)}{g(a)}$.  

A common case is when $B(x)$ does not mention the variable $x$:
\begin{equation*}
\id[f(a') = g(a')]{\transfib{x \mapsto \id[B(x)]{f(x)}{g(x)}}{p}{q}}
{\opp{(\apfunc{f}{p})} \ct q \ct \apfunc{g}{p}}
\end{equation*}

When $B(x)$ is dependent, the rule is
\begin{equation*}
...
\end{thm}
\begin{proof}
Path induction on $p$, followed by the unit law for composition.
\end{proof}

Because $\apfunc{(x \mapsto x)}$ is the identity function, and 
$\apfunc{(x \mapsto c)}$ (where $c$ is a constant) is \refl{c}, the
following are special cases:
\begin{cor}\label{cor:transport-path-prepost}
\[
\begin{array}{l}
\transfib{x \mapsto \id{c}{x}} {p} {q} = q \ct p \\
\transfib{x \mapsto \id{x}{c}} {p} {q} = \opp {p} \ct q \\
\end{array}
\]
\end{cor}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &= c.
  \end{align*}
  In the second line, we have used \autoref{cor:transport-path-prepost}.
