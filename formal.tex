...
In this appendix, we present two precise formulations of homotopy type theory.
The first presentation (\autoref{sec:syntax-informally}) describes our domain of
...
consisting of $\Gamma$, $x : A$, and $\Delta$, in that order.
...
\]
%
Such judgments are sensible only for \define{well-formed} contexts, a
notion captured our third and final judgment
\[
  \vdash (x_1:A_1, x_2:A_2,\dots,x_n:A_n)\; \ctx
\]
expressing that each $A_i$ is a valid
...
a
judgment holds exactly when it can be concluded by a derivation composed of
these inference rules. The derivation must be \emph{closed}, which means that it
is finite and does not have unfinished parts.
