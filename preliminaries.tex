...
As mentioned in the introduction, to state that a proposition is true in type theory corresponds to exhibiting an element of a type corresponding to that proposition.
We regard the elements of this type as \emph{evidence} or \emph{witnesses} that the proposition is true.%
\footnote{They are also sometimes called \emph{proofs}, but this word can have misleading connotations, so we generally avoid it.}
...
The basic principle of the logic of type theory is that a proposition is not merely true or false, but rather as classifying all possible witnesses of its truth (``proofs'').
Under this conception, proofs are not just the means by which mathematics is communicated, but rather are mathematical objects in their own right, on a par with familiar objects such as numbers, mappings, groups, and so on.
Thus, since types classify the available mathematical objects and govern how they interact, propositions are nothing other than types --- namely, types that classify proofs.

The observation which makes this identification feasible is that we have the following natural correspondence between \emph{logical} operations on propositions and \emph{type-theoretic} operations on their corresponding types of witnesses.
\begin{center}
\medskip
\begin{tabular}{ll}
  \toprule
...
However, once we are familiar with propositions-as-types, is convenient to also use the standard equality symbol for this.
...
There are many small variations in type theories of the sort we have described here, both in presentation and in the details of the rules.
For instance, we have chosen to include a judgmental equality $f \jdeq \lam{x} f(x)$, which is called \emph{$\eta$-conversion}.
This rule has a slightly different status from the other judgmental equalities, but it is quite convenient to have.
It is also possible to write down similar ``$\eta$-conversion'' rules for other types: for instance, the $\eta$-rule for cartesian products would be a \emph{judgmental} version of surjective pairing, $u \jdeq (\proj1(u),\proj2(u))$.
We have not included any of these, because they do not follow from the general framework of inductive definitions (see \autoref{cha:induction}).
Moreover, the corresponding ``$\eta$-rule'' for identity types would trivialize all the higher structure to be discussed in \autoref{cha:basics}, so we are \emph{forced} to leave it out, and the question then becomes where to draw the line.
It is not unreasonable to include judgmental $\eta$-rules at least for cartesian products and more generally $\Sigma$-types, but we have chosen not to.
