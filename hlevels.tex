...
\begin{cor}\label{thm:nconn-to-ntype-const}
...
If $f : A \to B$ is $n$-connected, then 
$\eqv{\trunc{n}{A}}{\trunc{n}{B}}$.
\end{lem}
\begin{proof}
Let $c$ by the proof that $f$ is $n$-connected.  From left to right, we
...
first component is $\trunc n{\hfiber{f}y}$, and we can obtain an
...
cover the case for $\tprojf{}$.  

In one direction, we must show that for all $x:A$, 
\[
\trunc{n}{\proj{1}}{(\proj{1}{(c(f(x)))})} = \tproj{}{x}
\]
But $\tproj{}{(x , \refl{})} : \trunc n{\hfiber{f}y}$, and
$c(y)$ says that this type is contractible, so 
\[
\proj{1}{(c(f(x)))} = \tproj{}{(x , \refl{})}
\]
...
\[
\trunc{n}{f}(\trunc{n}{\proj{1}} (\proj{1}{(c(y))})) = \tproj{}{y}
\]
...
In general, suppose we are given $p:\trunc{n}{\sm{x:A}{B}}$ and wish to prove
$P(\trunc{n}{\proj{1}{}}(p))$. By truncation induction, it suffices to
prove $P(\tproj{}{a})$ for all $a:A$ and $b:B(a)$.  Applying this
principle in this case, it suffices to prove
\[
\trunc{n}{f}(\tproj{}{a}) = \tproj{}{y}
\]
given $a:A$ and $b:f a = y$.  But the left-hand side equals $\tproj{}{f a}$,
so applying $\tprojf{}$ to both sides of $b$ gives the result.
...
\begin{defn}
  A \textbf{reflective subuniverse} is a predicate $P:\type\to\prop$ such that
...
\begin{ex}
  ...
  \item Show that $X\mapsto (P\to X)$ is a left exact modality.
    This is called the \emph{open modality} associated to $P$.
  \item Show that $X\mapsto P*X$ is a left exact modality, where $*$ denotes the join (see \autoref{sec:colimits}).
    This is called the \emph{closed modality} associated to $P$.
  \end{enumerate}
\end{ex}
