...
Usually, we prove that a function is an equivalence by exhibiting a quasi-inverse, but sometimes this definition is convenient to use instead.
...
\begin{defn}
  ...
  \item We say $f$ is a \textbf{monomorphism} if for every $x,y:A$ the function $\apfunc f : (\id[A]xy) \to (\id[B]{f(x)}{f(y)})$ is an equivalence.
  \end{enumerate}
\end{defn}
...
Of course, this cannot be used as a definition of ``equivalence'', since the definition of monomorphy refers to equivalences.
However, this characterization can still be useful; see \autoref{sec:whitehead}.
We will generalize it in \autoref{cha:hlevels}.

% TODO: modify whitehead.
