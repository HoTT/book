...
using product types, which is called \textbf{currying} in functional programming.
...
n_{n+1}$. 
Another important class of dependent function types are functions which are \emph{polymorphic} over a given universe.
...
Formally, this means it does not include certain classical principles, such as the law of excluded middle (LEM) or proof by contradiction (proving $A$ by deriving a contradiction from $\lnot A$)%
\footnote{Note that the pattern ``assume $A$ and derive a contradiction'' is perfectly valid in constructive logic as a way (in fact, \emph{the} way) to prove ``not $A$''; indeed, we used it in our example above.
  The form of ``proof by contradiction'' which is disallowed is ``assume not $A$ and derive a contradiction'' as a way of proving $A$.},
...
For example, while by default there is no construction witnessing LEM, the logic is still compatible with the existence of one (see \autoref{sec:logic}).
...
In the next chapter (specifically, \autoref{sec:logic}) we will introduce a modification to this logic that is sometimes appropriate, in which this additional information is discarded.
...
\[\refl{} : \prd{a:A} (a =_A a)\]
called the witness of \emph{reflexivity}, which says that every element of $A$ is equal to itself (in a specified way).
...
If we package the Martin-L\"of rule as an induction constant, it becomes:
\begin{align*}
...
The other direction is a bit more tricky: it is not clear how we can use a particular instance of the Martin-L\"of rule to derive a particular instance of
...
The type theory presented here is a version of Martin-L\"{o}f's type theory~\cite{martin-lof:bibliopolis} that differs in several important ways from other formulations of the theory such as those that underly NuPRL~\cite{constable+86nuprl-book}, Coq~\cite{bertot+10coq}, and Agda~\cite{agda-citation-needed}.

Most significantly, the type theory described here is based on the \emph{intensional} version of Martin-L\"{o}f's type theory in which judgmental equality is \emph{analytic} in the sense that it self-evident, whereas propositional equality, expressed by the identity type, is \emph{synthetic} in that it requires evidence in support of an equation.  The alternative, called the \emph{extensional} version of type theory, makes no distinction between judgmental and propositional equality (more precisely, inhabitation of the identity type has the same force as judgmental equality).  From the perspective of homotopy theory extensional theories, such as NuPRL, restrict types to sets, eliminating any higher-level structure.  (See \autoref{cha:basics,cha:equivalence,cha:hlevels} for more on propositional equality and levels.)

Among intensional type theories the present theory differs in that we include the judgmental equality $f \jdeq \lam{x} f(x)$, which is called \emph{$\eta$-conversion}.  This rule has a slightly different status from the other judgmental equalities, but it is quite convenient to have.  It is also possible to write down similar ``$\eta$-conversion'' rules for other types: for instance, the $\eta$-rule for cartesian products would be a \emph{judgmental} version of surjective pairing, $u \jdeq (\proj1(u),\proj2(u))$.  We have not included any of these, because they do not follow from the general framework of inductive definitions (see \autoref{cha:induction}).  Moreover, the corresponding ``$\eta$-rule'' for identity types would trivialize all the higher structure to be discussed in \autoref{cha:basics}, so we are \emph{forced} to leave it out, and the question then becomes where to draw the line.  It is not unreasonable to include judgmental $\eta$-rules at least for cartesian products and more generally $\Sigma$-types, but we have chosen not to do so.

Another point of variation is that we do not include a type of propositions, as is done, for example, in Coq, but rather identify propositions with types.  In particular the elements of an identity type may have a rich higher-dimensional structure that gives rise to the homotopy-theoretic interpretation developed here.  The concept of \emph{proof irrelevance}, which degenerates the structure of a type to having at most one element, is expressed here through the general concept of $n$-truncation, which is described in \autoref{cha:hlevels}.  In particular, a \emph{mere proposition} is a type that is $-1$-truncated, which means that its elements, if any, are collapsed to a point.

Following the practice of NuPRL, we replace, as a technical convenience, the judgments $A\;\textsf{type}$ and $A\jdeq B\;\textsf{type}$, expressing that $A$ is a type and that $A$ and $B$ are judgmentally equal types by the judgments $A:\UU$ and $A\jdeq B:\UU$, respectively, expressing that $A$ is a member of some (unspecified) universe, and that $A$ and $B$ are judgmentally equal elements of some universe (the same for both).  In particular an $A$-indexed family of types $B$ is represented by a map $B:A\to \UU$, rather than being taken as a primitive notion.  A related point is that we regard universes as types whose elements are themselves types, rather than codes for types, and we treat the universe hierarchy as cumulative in that every type in $\UU_i$ is also in $\UU_j$ for each $j\geq i$.

Another departure from other intensional type theories is that we require that equality of functions be \emph{extensional}, meaning that we have an element $\funext(f,g,a):\idtype[\prd{x:A} B(x)]{f}{g}$ whenever $a:\prd{x:A} \idtype[B(x)]{f(x)}{g(x)}$.  Function extensionality is not postulated explicitly, but rather arises as a consequence of the univalence axiom (see \autoref{cha:basics}), together with $\eta$ equivalence for function types, or, alternatively, as a consequence of the definability of the interval as a higher inductive type (see \autoref{cha:hits}).
