...
\begin{equation*}
(\mathsf{im}(P)\to B)\simeq \big(\big(\sm{X:\type}(a:A),\ X\simeq P(a)\big)\to B\big) \simeq A\to B,
\end{equation*}
...
\begin{equation*}
\prd C\prd{g:\modalfunc(B\to C)} \mathsf{isContr}\big(\sm{h:\modalfunc(B\to C)}\underline{h}\circ
...
\begin{defn}
Suppose $P:A\to\type$ is a dependent type over a type $A$. We write $\{A\mid P\}\defeq\sm{A}{\|P\|}$ and define:
\begin{align*}
...
$x= x^\prime$ for any two terms $\pairr{x,u},\pairr{x^\prime,u^\prime}:\sm{x:A}{\|P(x)\|}$.
...
\begin{equation*}
\exists!(x:A),\ P(x)\equiv \|\sm{x:A}P(x)\|\times\mathsf{atMostOne}(P).
\end{equation*}
\end{defn}
