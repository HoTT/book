...
\begin{thm}[Blakers-Massey Theorem]\label{Blakers-Massey}
  ...
  If $f$ is $i$-connected and $g$ is $j$-connected, then this induced map is $(i+j)$-connected. In other words, for any points $x:X$, $y:Y$, the corresponding fiber $C_{x,y}$ of $(f,g)$ gives an approximation to the path space $\id{\inl(x)}{\inr(y)}$ in the pushout.
\end{thm}
