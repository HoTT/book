...
\begin{proof}
  ...
  Then for any $x:X$ and $p:\id[X]xx$, we have
  \[\apdfunc{f(x)}(p) : \transfib{y\mapsto (R(x,y) \to (x=y))}{p}{f(x,x)} = f(x,x).
  \]
  By transport in function types and path types, the left-hand side of this equality is the function $R(x,x) \to (x=x)$ which sends $r:R(x,x)$ to $f(x,x,\trans{(\opp p)}r) \ct p$.
  But since $R(x,x)$ is a mere proposition, this is equal to $f(x,x,r)\ct p$.
  Thus, we have
  \[ \happly(\apdfunc{f(x)}(p),\rho(x)) : f(x,x,\rho(x)) \ct p = f(x,x,\rho(x)) \]
  and hence $p=\refl{x}$ by cancellation.
