...
\autoref{thm:naive-van-kampen} allows us to calculate the fundamental groups of many spaces.
...
We could, however, easily define a concatenation operation on $\code$ (by concatenating sequences) and show that $\encode$ and $\decode$ form an isomorphism respecting this structure.
...
\begin{eg}\label{eg:suspension}
  ...
  in a group.
  Indeed, it looks quite similar to the free group on $A$ (or equivalently on $\trunc0A$; see \autoref{thm:freegroup-nonset}), but we are considering only words which start with a non-inverted element and end with an inverted one.
  This effectively ``reduces the size of the generating set by one''.
  For instance, if $A$ has a point $a:A$, then we can identify $\pi_1(\Sigma A)$ with the group presented by $\trunc0A$ as generators with the relation $\tproj0a = e$; see \autoref{ex:vksusppt,ex:vksuspnopt} for details.
\end{eg}
...
\begin{ex}\label{ex:vksusppt}
  Continuing from \autoref{eg:suspension}, prove that if $A$ has a point $a:A$, then we can identify $\pi_1(\Sigma A)$ with the group presented by $\trunc0A$ as generators with the relation $\tproj0a = e$.
...
\begin{ex}\label{ex:vksuspnopt}
  Again continuing from \autoref{eg:suspension}, but this time without assuming $A$ to be pointed, show that we can identify $\pi_1(\Sigma A)$ with the group presented by generators $\trunc0A \times \trunc0A$ and relations
  \begin{gather*}
    (a,b)=\opp{(b,a)}\\
    (a,b)\cdot (b,c) = (a,c)\\
    (a,a)=e.
