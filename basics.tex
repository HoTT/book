...
It can also be useful to consider the loop space\index{loop space!iterated}\index{iterated loop space} of the loop space of $A$, which is the space of 2-dimensional loops on the identity loop at $a$.
...
\[
\alpha\hct'\beta\ \jdeq\ (\refl{a}\leftwhisker \beta)\ct (\alpha\rightwhisker \refl{a}) = \beta\ct\alpha.
\]
...
\begin{proof}[Second proof]
  By induction, it suffices to assume $p$ is $\refl x$.
  But in this case, the desired equation is $\trans{(\refl{x})}{f(x)}= f(x)$, which is inhabited by $\refl{f(x)}$.
\end{proof}
...
For instance, if we denote the path in \cref{thm:omg}\ref{item:omg4} by $\ctassoc(p,q,r)$ and the last path displayed above by $\pairct(p,q,p',q')$, then for any $u,v,z,w:A\times B$ and $p,q,r,p',q',r'$ of appropriate types we have
\begin{equation*}
  \begin{array}{l}
