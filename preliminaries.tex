...
To state that a proposition is true in type theory corresponds to exhibiting an element of the type corresponding to that proposition.
...
One important difference is that the ``natural'' logic of type theory does not include certain classical principles such as the law of excluded middle or proof by contradiction, i.e., proving $A$ by deriving a contradiction from $\lnot A$.
We will see in \S\ref{sec:logic}, however, that we can add these principles as additional axioms.

In basic propositional logic, we translate propositions into types following the following rules:
\begin{center}
\begin{tabular}{c|l}
...
\end{tabular}
\end{center}
In each case, the translation is valid because the rules for constructing and using elements of the type on the right correspond to the rules for reasoning about the proposition on the left.
...
This correspondence between logical rules and type-theoretic rules allows us to translate any (constructively valid) proof written in English into the construction of an element of the corresponding type.
...
\end{quote}
Now, the type corresponding to our tautology~\eqref{eq:tautology1} is
...
\end{align*}
In particular, observe that the pattern ``assume $A$ and derive a contradiction'' is perfectly valid in constructive logic as a way (in fact, \emph{the} way) to prove ``not $A$''.
...
\emph{If not ($A$ and $B$) then (not $A$) or (not $B$)} is not constructively valid: we cannot, in general, construct an element of the corresponding type
...
\begin{ex}
Using the propositions as types interpretation, formally derive the double negation of the principle of excluded middle, i.e.\ prove \emph{not (not ($P$ or not $P$))}.
\end{ex}
