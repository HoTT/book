...
the concrete representation of these objects in topological spaces (for example,
...
methods and proofs.  Some of the proofs that we have done are fairly
...
\emph{cohomology groups}.  Equivalent spaces have isomorphic
...
continuity.  For example, he torus locally looks like the sphere, but it
...
including $\pi_1(\Sn ^1)$ (\cref{sec:pi1-s1-intro}), $\pi_{k<n}(\Sn ^n)$
(\cref{sec:pik-le-n}), $\pi_2(\Sn ^2)$ and $\pi_3(\Sn ^2)$ by way of the
Hopf fibration (\cref{sec:hopf}), and $\pi_n(\Sn ^n)$ by way of the
Freudenthal suspension theorem (\cref{sec:freudenthal}).  Next, we
discuss the van Kampen theorem (\cref{sec:van-kampen}), which
characterizes the fundamental group of pushouts, and the status of
Whitehead's principle (when is a map that induces an equivalence on all
homotopy groups an equivalence?) (\cref{sec:whitehead}).  Finally, we
include brief summaries of additional results that are not included in
the book, such as $\pi_{n+1}(\Sn ^n)$ for $n\ge 3$, the Blakers-Massey
theorem, and a construction of Eilenberg-Mac Lane spaces
(\cref{sec:moreresults}).
...
In this section we will describe a proof of van Kampen's theorem which uses the same encode-decode method that we used for $\pi_1(\Sn^1)$ in \autoref{sec:pi1-s1-intro}.
...
intriguing, because it is the first calcualtion of a homotopy group 
...
\bottomrule
\end{tabular}
\caption{Theorems from homotopy theory that we ahve proved, checked by
  hand (\humancheck) or by computer (\computercheck).}
  \label{tab:theorems}
\end{table}
}

The following people were responsible for creating the first proofs in
homotopy type theory of the theorems described in this chapter, or for
formalizing them with a computer proof assistant:
%
\begin{itemize}
...
encode-decode proof of $\pi_1(\Sn ^1)$ and the encode-decode
method.

\item 
Guillaume Brunerie gave the proofs of $\pi_{k<n}(\Sn ^ n)$, the
total space of the Hopf fibration, and $\pi_2(\Sn ^2)$.  $\pi_3(\Sn ^2)$
is a corollary of this and $\pi_n(\Sn ^n)$.  Dan Licata later gave 
encode-decode proofs of $\pi_{k<n}(\Sn ^ n)$ and $\pi_2(\Sn ^2)$, for
...
\item 
For the suspension definition of the spheres, Dan Licata gave an
encode-decode proof of $\pi_2(\Sn ^2)$ and used Freudenthal to prove
...
Almost all of these results were proved during the spring term at IAS in
2013.  Moreover, the interplay between homotopy theory and type theory
was crucial to their development.  For example, the first proof of
$\pi_1(\Sn ^1)$ followed a classical homotopy theoretic proof.  A
type-theoretic analysis of this proof resulted in the development of the
encode-decode method.  The first proof of $\pi_2(\Sn ^2)$ also followed
classical methods, but this led quickly to an encode-decode proof of the
result.  The encode-decode proof generalized to $\pi_n(\Sn ^n)$, which
...
reasoning about connectedness, and then Freudenthal was key to the
construction of Eilenberg-Mac Lane spaces.  The rapid development of
this series of results illustrates the promise of our new understanding
of the connections between these two subjects.
