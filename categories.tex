...
There has been a good deal of general theory developed for $\dagger$-cat\-e\-gor\-ies under classical\index{mathematics!classical} foundations.
...
One would have to be similarly careful in classical\index{mathematics!classical} category theory if one wanted to avoid the axiom of choice: any time we want to define a function, we need to characterize its values uniquely somehow.
...
In classical\index{mathematics!classical} category theory, all one can say is that this object is specified up to unique isomorphism, but in set-theoretic foundations this is not a sufficient amount of uniqueness to give us a function without invoking \choice{}.
...
\begin{proof}[Second proof]
  ...
  When $x$ varies along the equality $\id{j(\idtoiso(p))}{\map i p}$ for $p:a=a'$ and $y$ is $ib$, we require $(\blank\circ \idtoiso(p)) = \map{\hom_A(\blank,b)}{p}$, which follows by induction on $p$.
