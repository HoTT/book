...
\begin{thm}[Blakers-Massey Theorem]\label{Blakers-Massey}
  Assume two pointed spaces $(X,x_0)$ and $(Y,y_0)$, where $C : X \times Y
  \to \type$.  
  If $\sm{x:X} C(x,y_0)$ is $i$-connected and $\sm{y:Y} C(x_0,y)$ is
  $j$-connected, then for all $x:X$ and $y:Y$, the pushout constructor $\glue : C(x,y)
  \rightarrow X \sqcup^C Y$ is $i+j$-connected.  In
  particular, 
  $\trunc {i+j} {C(x,y)} = \trunc {i+j} {\id [X \sqcup^C Y] {(\inl(x))} {(\inr(y))}}$
  for all $x,y$.
\end{thm}
...
\begin{thm}[Covering spaces]\label{thm:covering-spaces}
  For a connected space $A$, there is an isomorphism between covering spaces over $A$ and sets with an action of $\pi_1(A)$.
\end{thm}
...
Shulamn gave the first proof of $\pi_1(\Sn ^1)$ in HoTT, the
