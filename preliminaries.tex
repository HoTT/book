...
$\UU_infty : \UU_\infty$ but as in set
...
To avoid paradoxes we introduce a hierarchy of universes
\[ \UU_0 : \UU_1 : \UU_2 : \dots \]
...
\footnote{This rule seems to break the principle that every element
  has a uniquely determined type. This could be avoided by introducing
  an explicit lifting operator. We simply omit writing liftings not to
  clutter notation.}.
...
One of the central features of type theory is the use of dependent
types, i.e. families of types dependening on a given type. However,
before introducing constructions on dependent types, in particular
$\Pi$-types and $\Sigma$-types we introduce there non-dependent 
counterparts, namely function types $A\to B$, product types $A \times
B$ and coproduct types $A + B$.

Before moving on to dependent types let us first review some types
which corresponds to standard set theoretic constructions: the
function type $A \to B$, the (cartesian) product
of two types $A\times B$ (or the type of binary tuples) and the coproduct
or disjoint union of types $A + B$.

\subsection{Function types}

Given $A,B$ types we can construct the (non-dependent) function type
$A\to B$. In type theory functions are a primitive concept which
cannot be reduced to the notion of a graph. Functions are usually
assumed to be constructive that is given a function $f : A \to B$ and
an element of the domain $a : A$ we can effectively compute an element
of the codomain $f(a) : B$. We introduce functions by explicit
defining equations $f(x) := b[x]$ where $b[x] : A$ is a term which may
use $x:A$. Then $f(a) \jdeq b[x:=a]$ where $b[x:=a]$ is $b[x]$ with
all occurences of $x$ replaced by $a$. Note that this substitution has
to be capture avoiding but it may consistently change names of bound
variables \cite{capture-avoiding}. 

If we want to avoid giving a name to a function we can use
$\lambda$-abstraction $\lambda_{x:A}b[x] : A \to
B$. $\lambda$-abstractions induce the judgmental equality 
$(\lambda_{x:A}b[x])(a) \defeq b[x:=a]$ ($\beta$-equality). Note that
we do not assume $\eta$-equality $\lambda_{x:A} f(x) \jdeq f$ (where
$x$ does not occur in $f$) but this is provable as propositional
...
$\lambda$-abstraction, i.e. we can write $\lambda n:\Nat.n+2 : \Nat
\to \Nat$.
...
functions - we will define this when we introduce the type. 

\subsection{Product types}

Given $A,B$ types we can construct the product type $A\times B$ whose
elements are pairs $(a,b)$ where $a : A$ and $b : B$. 
