...
This is strengthened if we rephrase it in the following way.
...
Of course, since the reflector of a reflective subuniverse lands in that subuniverse, any reflective subuniverse can be regarded as a modality.
Conversely, we have:
...
For the rest of this section, let $\modal$ be a modality.
We note that the universal property of a modality extends to a dependent one.

\begin{thm}\label{prop:lv_n_deptype_sec_equiv_by_precomp}
Let $A$ be a type and let $P:\modal(A)\to\modaltype$. Then the function
\begin{equation*}
(-\circ \mreturn^\modal_A) : \Big(\prd{z:\modal(A)}P(z)\Big) \to \Big(\prd{a:A}P(\mreturn^\modal_A(a))\Big)
\end{equation*}
is an equivalence.
\end{thm}
...
\end{equation*}
A type $A$ is said to be \define{\modal-connected} if the unique function
$A\to\unit$ is \modal-connected. Likewise, a type family
$P:A\to\type$ is said to be \define{\modal-connected} if $P(x)$ is \modal-connected for
every $x:A$. 
\end{defn}
...
\begin{eg}
  ...
  Every function is $(-2)$-connected, while a function is $(-1)$-connected precisely when it is surjective.
\end{eg}
...
\begin{defn}
  A function $f:A\to B$ is \define{modal} if the fiber $\hfib f b$ is modal for all $b:B$.
\end{defn}
...
When specialized to the $n$-truncation, this yields a factorization system system consisting of $n$-connected functions and $n$-truncated ones.
...
% modal, \emph{modal functions} for short.
...
\begin{proof}
First we have to show that $\im(P)$ is a mere proposition. Suppose that $\pairr{X,u}:\im(P)$, we are essentially done showing
that
...
\begin{proof}
By \autoref{prop:to_image_is_connected} we know that there is a term of $\mathsf{factorizations}_\modal(f)$, hence it is enough to
