...
\begin{ex}\label{ex:id-reflection-rule}
  Suppose we add to type theory the \emph{reflection rule} which says that for any type $A$ and element $x,y:A$, if there is a $p$ such that $p:\id[A]xy$, then $x\jdeq y$.
  Note that this is a \emph{rule} rather than an \emph{axiom}: it stipulates that from one judgment $p:\id[A]xy$ we can derive another judgment $x\jdeq y$.
  Prove that then for any $A$, $x$, and $p:\id[A]xx$, we have $p=\refl{x}$.
  (In \cref{thm:type-is-not-a-set} we will see that this conclusion contradicts the univalence axiom.)
\end{ex}
