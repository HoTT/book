...
For if we had a proof of ``for all $A$, $A$ or not $A$'' then we could extract from it an algorithm for deciding arbitrary propositions --- which would of course be nice, but it would also allow us to construct the Halting Oracle and other non-computable functions.
(This is a meta-theoretic argument for why such a statement is not provable.
In fact, with the univalence axiom we can actually \emph{disprove} this naive form of excluded middle; see \autoref{thm:not-lem}.
The point is that under univalence, any proof of ``for all $A$, $A$ or not $A$'' would have to be ``natural'' in $A$, which is impossible.)
