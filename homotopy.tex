...
notions, which are modeled by types and elements of types, particularly
the identity type.  The algebaric structure of paths and homotopies is
modeled by the $\infty$-groupoid structure on types, which is generated
...
the proofs feel elementary, and talk concretely about a space's
points/paths/homotopies.  At the same time, our approach abstracts from
the concrete representations in topological spaces (for example,
associativity of path concatention is proved by path induction, rather
than by reparametrization of maps from $[0,1] \to X$), and our proofs
apply in a variety of settings: Homotopy type theory has an
interpretation in Kan simplicial sets, which are one model of
$\infty$-groupoids.  Thus, our proofs apply to this model, and
transfering them along the geometric realization functor from simplicial
sets to topological spaces gives proofs of the classical statements.
...
that ``look like'' the category of $\infty$-groupoids
($(\infty,1)$-toposes), and, if so, proving a result in type theory will
show that it holds in these settings as well.  Additionally, our results
in this chapter suggest that type theory is a very convenient way to do
homotopy theory using $\infty$-groupoids: by using the rules for the
identity type, we can avoid the complicated combinatorics involved in
many definitions of $\infty$-groupoids/functors, and simply explicate as
much of the structure as is needed in any particular proof.

First, you can use proof assistants such as Agda and Coq
to check your proofs.  Second, we're starting to see examples where
working in type theory is suggesting new ways of doing proofs.  
...
Before presenting the technical results, we take a moment to review some
of the basic theorems of homotopy theory and give an overview of the
results proved in this chapter.  Homotopy theory arose as a branch of
algebraic topology, and concerns the use of tools from abstract algebra,
...
same, where ``the same'' means \emph{homotopy equivalence} (continuous
maps back and forth that compose to the identity up to homotopy---this
gives a little wiggle room to ``correct'' maps that don't exactly
compose to the identity.).  A central tool used to do this is the
\emph{algebraic invariants} associated with a space, which include its
\emph{homotopy groups} and \emph{homology} and \emph{cohomology groups}.
Equivalent spaces have isomorphic homotopy/(co)homology groups, so if
two spaces have different groups, then they are not equivalent.  Thus,
these algebraic invariants provide global information about a space,
which can be used to tell spaces apart, and complements the local
information provided by notions such as continuity.  For example, he
torus locally looks like the sphere, but it has a global difference,
because it as a hole in it, and this difference is visible in the
homotopy groups of these two spaces.
...
$x_0$ (paths from $x_0$ to $x_0$), considered up to homotopy, with the
...
the torus that goes around the donut's hole is not homotopic to the
identity, so there are non-trivial loops.
...
$\refl{x_0}$.  Then the homotopy classes of homotopies between $\refl{}$
...
One of the basic problems that algebraic topologists consider is
...
many homotopy groups of spheres are currently unknown.
...
% Saturation 0.20
% \definecolor{xA}{rgb}{1.0, 0.8, 0.8},    
% \definecolor{xB}{rgb}{1.0, 0.984615, 0.8}
% \definecolor{xC}{rgb}{0.830769, 1.0, 0.8}
% \definecolor{xD}{rgb}{0.8, 1.0, 0.953846}
% \definecolor{xE}{rgb}{0.8, 0.861538, 1.0}
% \definecolor{xF}{rgb}{0.923077, 0.8, 1.0}
% \definecolor{xG}{rgb}{1.0, 0.8, 0.892308}
% \definecolor{xH}{rgb}{1.0, 0.892308, 0.8}
% \definecolor{xI}{rgb}{0.923077, 1.0, 0.8}
% \definecolor{xJ}{rgb}{0.8, 1.0, 0.861538}
% \definecolor{xK}{rgb}{0.8, 0.953846, 1.0}
% \definecolor{xL}{rgb}{0.830769, 0.8, 1.0}
% \definecolor{xM}{rgb}{1.0, 0.8, 0.984615}
...
betwen spaces and $\infty$-groupoids introduced in \cref{cha:basics}.
...
generators creating 3-dimensional cells.  One explanation is that the
algebraic structure of an $\infty$-groupoid includes non-trivial
interactions between levels, such as the middle-four interchange law
(see \cref{thm:EckmannHilton}), and these interactions create elements
of higher homotopy groups.  Indeed, the generating element of $\pi_3(\Sn
^2)$ is constructed using this interchange law.
...
The reason the problem of calculating homotopy groups is interesting and
nontrivial is that the (higher) inductive definition of a type $X$
presents $X$ as a free $\infty$-groupoid, and this presentation
\emph{determines} but does not \emph{explicitly describe} the higher
identity types of $X$.  The identity types are populated by both the
generators ($\lloop$, for the circle), and all of the groupoid
...
create a complex structure.  Thus, the higher-inductive presentation of
a space allows you to pose the question "what does the identity type of
$X$ really turn out to be?", though it can take some significant
mathematics to answer it.  This is a higher-dimensional generalization
of a familiar fact in type theory: that characterizing the identity type
of $X$ can take some work, even if $X$ is an ordinary inductive type,
such as the natural numbers or booleans.  For example, the theorem that
$\mathsf{true}$ is different from $\mathsf{false}$ does not follow
immediately from the definition, see \autoref{sec:compute-coprod}.
...
Of course, \emph{a priori} these are only sets, but since $n\ge 1$, the path concatenation and inversion operations on $\Omega^n(A)$ induce operations on $\pi_n(A)$ making it into a group in a straightforward way.
If $n\ge 2$, then the group $\pi_n(A)$ is abelian, by the Eckmann--Hilton argument (\autoref{thm:EckmannHilton}).

It is sometimes convenient to also write $\pi_0(A) \defeq \trunc0 A$, but this case behaves somewhat differently: not only is it not a group, it is defined without reference to any basepoint in $A$.
...
Here is a sample of some well-known basic theorems of homotopy theory
that have been proved in HoTT; in most cases, they have been formally
verified with the aid of a computer.
...
\begin{proof}
  ...
  \end{itemize}
  We might call this type the \define{homotopical reals}; it plays the same role as the topological space $\mathbb{R}$ in the classical proof.
...
\begin{defn}
  A type $A$ is called \define{$n$-connected} if $\trunc nA$ is contractible.
\end{defn}
...
\begin{proof}
  ...
  \[(\trunc{n}{A \to B})=(\unit \to B)=B. \qedhere\]
\end{proof}
...
\begin{lem}
  If $A$ is $n$-truncated and $a:A$, then $\forall k>n,\ \pi_k(A,a)=\unit$.
\end{lem}
...
\end{cor}
\begin{proof}
  The sphere $\Sn$ is $n\mathord{-}1$-connected by \autoref{cor:sn-connected}.  Therefore
...
\begin{defn}
  Given a pointed space $X$, the \define{loop space} of $X$ is the pointed type
...
In this section we will define the \define{Hopf fibration} which was already
...
\begin{lem}\label{lem:hopf-construction}
  ...
  \define{Hopf construction}, over $\susp A$ with fiber $A$ and total space $A*A$.
\end{lem}
...
\define{join} $A*B$ of $A$ and $B$ is the pushout of the diagram
...
\pi_{k+1}(\Sn^{n+1}) &\jdeq \trunc{0}{\Omega^{k+1}(\Sn^{n+1})} \notag \\
                     ...
                     &= \Omega^k(\trunc{k}{(\Omega(\susp(\Sn^{n})))}) \tag{\autoref{thm:path-truncation}}\\
                     &= \Omega^k(\trunc{k}{\Sn^{n}}) \tag{\autoref{eq:freudenthal-for-spheres}}\\
                     &= \trunc{0}{\Omega^k(\Sn^{n})} \tag{\autoref{thm:path-truncation}}\\
...
\subsection{Van Kampen's theorem with a set of basepoints}
...
A map $f$ such that $\pi_k(f)$ is a bijection for all $k$ is called \define{$\infty$-connected} or a \define{weak equivalence}.
A type $Z$ is called \define{$\infty$-truncated} or \define{hypercomplete} if the induced map
...
The following people were responsible for creating the first proofs in homotopy type theory of
the theorems described in this chapter, or for formalizing them with a computer proof
assistant:
%
\begin{itemize}

\item 
Michael Shulman gave the first proof of $\pi_1(\Sn^1)$, which is the
homotopy-theoretic proof described above.  Dan Licata later discovered the
type-theoretic proof of $\pi_1(\Sn ^1)$ and the encode/decode
methodology.

\item 
Guillaume Brunerie gave the homotopy theoretic proofs of $\pi_{k<n}(\Sn ^ n)$, the
total space of the Hopf fibration, and $\pi_2(\Sn ^2)$.  $\pi_3(\Sn ^2)$
is a corollary of this and $\pi_n(\Sn ^n)$.  Dan Licata later gave the
type-theoretic proofs of $\pi_{k<n}(\Sn ^ n)$ and $\pi_2(\Sn ^2)$, for
the single-loop presentation of the spheres.

\item 
Dan Licata and Guillaume Brunerie gave the first proof of $\pi_n(\Sn ^n)$, for the
single-loop presentation of the spheres.

\item 
Peter Lumsdaine proved the Freudenthal Suspension Theorem; Dan Licata and
Peter Lumsdaine formalized this proof.  Peter Lumsdaine, Eric Finster, and Dan Licata proved
Blakers-Massey theorem.

\item 
For the suspension definition of the spheres, Dan Licata gave a
type-theoretic proof of $\pi_2(\Sn ^2)$ and used Freudenthal to prove
$\pi_{k<n}(\Sn ^n)$ and $\pi_n(\Sn ^n)$; and, using similar techniques,
constructed $K(G,n)$.

\item 
Michael Shulman proved Van Kampen's theorem; Kuen-Bang Hou formalized this proof.  

\item 
Dan Licata proved Whitehead's theorem for $n$-types.

\item Guillaume Brunerie proved $\pi_4(\Sn ^3)$.  

\item 
Kuen-Bang Hou formulated the proof of the characterization of covering
