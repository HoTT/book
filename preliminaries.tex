...
types.  Given universes $\UU$ and $\UU'$ and types $A,B : \UU$, we have
%
\begin{equation}
...
Note that the universes $\UU,\UU'$ may be different, i.e. given a product
in a universe $\UU_i$ we may use the eliminator for a type or family in a different universe 
$\UU_j$. This is a property of all the eliminators we are going to introduce.
...
We will also refer to it as the \define{two-sided identity type}, to refer to the fact that \emph{both} endpoints are existentially quantified.  The two-sided induction principle for the identity type then states that the total identity type is inductively generated by the elements of the form $(x,x,\refl{x})$:
...
The general form of the rule allows $C$ to depend on the witness $p:x=y$ as well.  In the premise, we can not only replace $y$ by $x$, but also simultaneously replace $p$ by reflexivity: to prove a property for all elements $(x,y,p)$ of the total identity type, it suffices to consider all the cases of the form $(x,x,\refl{x})$.  If we view types as like sets, it is unclear what this buys us, but in \autoref{cha:basics} we will see that it is essential to the homotopy interpretation, because it expresses that the total path space of a type is inductively generated by the loops at each point.  
...
This fnction is also called $J$.  In \cref{sec:fibrations}, we show that the recursor (indiscernability of identicals) follows from this induction principle.  
...
In some proofs, however, it is simpler to use an equation $p : \id{a}{b}$ by replacing all occurences of $b$ with $a$ (or vice versa), because it is sometimes easier to do the remainder of the proof for the specific element $a$ mentioned in the equality, than for a general unknown $x$.  This motivates a second induction principle for identity types.  As we show below, this second induction principle is equivalent to the first, but it is sometimes a more convenient packaging.

Given a type $A$ with an element $a:A$, we define the \define{singleton   type} of $A$ at $a$ to be the type
\[
\sm{y:A} \id[A]{a}{y}
\]
We will also call this the \define{one-sided identity type} of $A$ at $a$, to refer to the fact that only one endpoint is existentially quantified.  
Intuitively, this type is a ``singleton'' because it classifies elements of $A$ that are equal to $a$, and therefore, in a sense, contains only the element $a$.  However, it is very important to observe the difference between this singleton type and a singleton set $\{a\}$: A singleton set has exactly one element, namely $a$.  In contrast, the singleton type has elements $(b, p)$ where $p : \id[A] a b$.  Because of the homotopy interpretation of equality, the singelton type can have many different elements $b$ that are not strictly equal to $a$, but are only homotopic to it.  Nonetheless, the singleton type behaves like a singleton up to homotopy, in the sense that any two elements of it are identified by a path.  This follows from the second induction principle for the identity type, which states that the type of singletons in $A$ at $a$ is inductively generated by the element $(a,\refl{a})$.  
...
The one-sided induction principle says that we may prove a family $C$ for all singletons by covering the case for $(a,\refl{a})$.
Logically, this says that given an equality $\id{a}{b}$, we can substitute the occurences of $b$ chosen by $C$ with $a$.
Homotopically, it expresses the that the space of paths from a chosen point to an arbitrary endpoint is inductively generated by the reflexivity loop.

Packaged as function, the one-sided induction principle becomes:
...
One might wonder whether there is a also a ``0-sided'' induction principle for the identity type, with zero sides existentially quantified. Given a family
\[
C : \id{a}{b} \to \UU
\]
...
This principle is \textbf{not} provable in homotopy type theory.  Using it, one can easily prove that any $p : \id{a}{a}$ is equal to $\refl{a}$.  This and the univalence axiom create a contradiction, because univalence allows one to create provably disequal loops.
