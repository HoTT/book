...
$(\lam{x:A} y): A\to B$; if the element $y$ is represented by an expression involving variables, the variable
$x$ should be a new one, distinct from them.

We generally omit the type of the variable $x$ in a $\lambda$-abstraction and write $\lam{x}\Phi$, since the typing $x:A$ is inferrable from the judgment that the function $\lam x \Phi$ has type $A\to B$.
