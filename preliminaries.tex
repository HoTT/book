...
Apart from the two judgments $a : A$ and $a \jdeq b : A$, some type theories also have judgments expressing the fact that something is a type and that two types are equal. In our system these are expressed as $A : \type$ and $A \jdeq B : \type$, respectively. In particular, a family of types indexed by a type~$A$ is expressed as a function $A \to \type$, rather than being taken as a primitive notion. A related point is that we regard universes as types whose elements are themselves types, rather than codes for types, and we treat the universe hierarchy as cumulative in that every type in $\UU_i$ is also in $\UU_j$ for each $j\geq i$.

Another departure from other intensional type theories is that equality of functions be \emph{extensional}, see \autoref{axiom:funext} for the exact statement. While we include function extensionalty explicitly as an axiom, it also follows from the Univalence axiom and $\eta$-conversion, see \autoref{sec:univalence-implies-funext}, as well as from the existence of the interval, see \autoref{thm:interval-funext}.
