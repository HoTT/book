...
We begin with the following theorem, which will eventually supply the induction step in the proof of the truncated Whitehead's principle.
...
\begin{thm}\label{thm:whitehead0}
  ...
  \begin{enumerate}
  \item $\trunc0 f : \trunc0 A \to \trunc0 B$ is surjective\note{Should define ``surjective'' somewhere}, and
  \item for any $x,y:A$, the function $\apfunc f : (\id[A]xy) \to (\id[B]{f(x)}{f(y)})$ is an equivalence.
  \end{enumerate}
  Then $f$ is an equivalence.
\end{thm}
\begin{proof}
  Let $b:B$, we show that $\sm{x:A}(f(x)=b)$ to be contractible.
  Since $\trunc0 f$ is surjective, there merely exists an $a:A$ such that $\trunc 0 f(\tproj0a) = \tproj0b$.
  Since contractibility is a mere proposition, we may assume given such an $a$.
  Then we have $\tproj0{f(a)} = \trunc 0 f(\tproj0a) =\tproj0b$, hence $\tproj{-1}{f(a)=b}$.
  Again, since we are still trying to prove contractibility, we may assume $f(a)=b$.
  ...
  However, the latter equality rearranges to $\trans{r}{p} = q$, so that together with $r$ it exhibits $(x,p) = (y,q)$ in the fiber of $f$ over $b$.
\end{proof}
