...
We generally omit the type of the variable $x$ in a $\lambda$-abstraction and write $\lam{x}b$, since the typing $x:A$ is inferrable from the judgment that the function $\lam x b$ has type $A\to B$.
...
\index{rule}%
As mentioned in \autoref{sec:finite-product-types}, in general, a kind of type in type theory (such as function types, product types, identity types, etc.)\ is associated with four kinds of rules: \emph{formation rules},
...
or \emph{eliminators},
\index{eliminator}%
which specify how to use elements of such types to construct elements of other types; and \emph{conversion rules},
\index{rule!computation}%
\index{computation rule}%
which specify what happens when the introduction and elimination rules are combined. The computation rules include both what we have called \emph{computation rules} (also called \emph{reduction rules} or \emph{$\beta$-conversion rules}) and what we have called \emph{uniqueness principles} (also called \emph{$\eta$-conversion rules}).
