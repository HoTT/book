...
% \begin{equation*}
% \big(\prd{f:\Sn^{0}\to A} \mathsf{hNull}(f)\big)\eqvsym\prd{x,y:A} (x= y),
% \end{equation*}
...
all $a:A$, and \autoref{lem:susp-loop-adj}, which implies that $\Omega^{n+1}(A,a) \eqvsym \Map_*(\Sn^{n+1},(A,a))$, where $\Sn^{n+1}$ is
...
% A function $f:A\to B$ is $n$-connected if and only if for every modal function $g:X\to B$, the function
% \begin{equation*}
% \varphi :\big(\sm{h:B\to X} g\circ h\htpy \refl{B}\big)\to\big(\sm{k:A\to X} g\circ k\htpy f\big)
% \end{equation*}
...
Likewise, if $f,f^\prime:X\to Y$ and $g:Y\to X$ are functions and if $H:f\htpy f^\prime$ is a homotopy, then pointwise whiskering provides a homotopy $g\leftwhisker H:g\circ f\htpy g\circ f^\prime$.
