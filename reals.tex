...
define maps from reals into sets by induction. Explicitly, all reals have a mere property
$P$, provided that:
%
\begin{enumerate}
\item $P(\rcrat(q))$ for all $q : \Q$, and
\item for all $x : \Qp \to \RC$ satisfying~\eqref{eq:RC-cauchy}, if $\prd{\epsilon :
    \Qp} P(x_\epsilon)$ then $P(x)$.
\end{enumerate}
%
...
\item for every $x : \Qp \to \RC$ satisfying~\eqref{eq:RC-cauchy}, construct $f(x) : A$,
...
\begin{lem}\label{RC-sim-uniformity-properties}
  ...
  %
  \begin{enumerate}
  \item $u \sim_\epsilon u$ for all $u : \RC$ and $\epsilon : \Qp$,
  \item if $\delta < \epsilon$ and $u \sim_\delta v$ then $u \sim_\epsilon v$,
  \item $\rclim(x) \sim_{\delta + \epsilon} x_\epsilon$ for all $\delta, \epsilon : \Qp$
    and $x : \Qp \to \RC$ satisfying~\eqref{eq:RC-cauchy}.
  \end{enumerate}
\end{lem}
...
\begin{proof}
  ...
  %
  \begin{enumerate}
  \item It is obvious that $\rcrat(q) \sim_\epsilon \rcrat(q)$. To see that $\rclim(x)
  ...
  \item The statement is obvious for $u = \rcrat(q)$ and $v = \rcrat(r)$. When $u =
    \rcrat(q)$ and $v = \rclim(x)$ we proceed as follows. Because $\rcrat(q) \sim_\delta
    \rclim(x)$, there merely are $\eta, \theta : \Qp$ such that $\rcrat(q) \sim_{\delta - \eta -
      \theta} x_\eta$, but then by induction hypothesis $\rcrat(q) \sim_{\epsilon - \eta -
      \theta} x_\eta$, hence $\rcrat(q) \sim_\epsilon \rclim(x)$, as desired. The other
...
\begin{proof}
  ...
  \end{equation*}
  %
  and for $x : \Qp \to \RC$ satisfying~\eqref{eq:RC-cauchy}
  % 
  \begin{equation*}
  ...
  To see that $\bar{f}$ satisfies the Lipschitz condition, consider four cases, from which
  omit one symmetric case:
  %
  \begin{enumerate}
  ...
  Finally, suppose $f' : \RC \to \RC$ is also an extension of $f$ which satisfies the
  Lipschitz condition. By function extensionality $\bar{f} = f'$ follows from $\prd{u :
    \RC} \bar{f}(u) = f'(u)$, which we prove by induction on~$u$. Clearly,
  $\bar{f}(\rcrat(q)) = \rcrat(f(q)) = f'(\rcrat(q))$, so consider any $x : \Qp \to \RC$
  satisfying~\eqref{eq:RC-cauchy} and let $\delta, \epsilon : \Qp$. Because $x_{\delta/L}
  \sim_{(\delta + \epsilon)/L} x_{\epsilon/L}$ and $f'$ satisfies the Lipschitz condition
  %
  \begin{equation*}
    f'(x_{\delta/L}) = \bar{f}(x_{\delta/L}) \sim_{\delta + \epsilon} \bar{f}(x_{\epsilon})

    f'(x_{\delta/L}) \sim_{\delta + \epsilon} f'(x_{\epsilon/L})
  \end{equation*}
