...
\begin{tabular}{llp{11cm}}
  ...
  & 80-g8f95fa5
  & In the discussion of formation rules, the dependent function type
  example should be $\prd{x:A} B(x)$.\\

\autoref{sec:inductive-types}
&& Swap the order of the arguments in the type of the
 recursor $\rec{\nat}$
 from
$
\dprd{C:\UU} C \to (\nat \to C \to C) \to \nat \to
...
\)
\\
\autoref{sec:pat}, after
\autoref{eq:tautology2}
&
&
Remove ``unnamed'' from ``unnamed 
variables (hypotheses)
$x$ and $y$'' and
 ``unnamed hypothesis $z$''
% I did not dare to carry out this change, because it is such an obvious
% contradiction, maybe I am missing something.
\end{tabular}
%% THE ERRATA LIST SHOULD BE ORDERED BY OCCURRENCE IN THE BOOK,
%% at least for the benefit of those who bought the printed version,
%% AND IT SHOULD BE POSTED ON THE WEB PAGE OF THE BOOK.
