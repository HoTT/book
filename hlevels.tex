...
\begin{proof}
Let $\rho : \prd{x:X} R(x,x)$ be a witness for reflexivity of $R$ and let $f : \prd{x,y:X} R(x,y)
\to \id[X]{x}{y}$ be a witness that $R$ implies identity. To show that each $f(x,y) : R(x,y) \to \id[X]{x}{y}$ is an equivalence, it suffices to show that $f$
induces an equivalence of total spaces:
\begin{equation*}
\eqv{\sm{y:X}R(x,y)}{\sm{y:X}\id[X]{x}{y}}.
\end{equation*}
Note that the type on the right is contractible, so it suffices to show that
$\sm{y:X}R(x,y)$ is contractible. As the center of contraction we take the
pair $\pairr{x,\rho(x)}$.  It remains to show that
\begin{equation*}
\prd{y:X}{H:R(x,y)}\id{\pairr{y,H}}{\pairr{x,\rho(x)}}.
\end{equation*}
Since $R(x,y)$ is a mere proposition for every $x$ and $y$, the type above is equivalent to 
\begin{equation*}
\prd{y:X}R(x,y)\to \id[X]{y}{x},
\end{equation*}
which we get from $f$ and from the symmetry of identity.
\end{proof}
