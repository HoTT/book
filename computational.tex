...
paths $p,q : \id[A]{a}{a'}$.  However, the type \id[A]{-}{-}
respects equivalence:

\begin{thm}\label{thm:paths-respects-equiv}
If $f : A \to B$ is an equivalence, then $\apfunc{f} :
\id[A]{a}{a'} \to \id[B]{f(a)}{f(a')}$ for all $a,a'$.  
\end{thm}
\begin{proof}
The quasi-inverse is \apfunc{\opp f}, which cancels in both directions
because \apfunc{-} is functorial, so it suffices to cancel $f$ and $\opp{f}$.
\end{proof}

Thus, given a full characterization of $\id[A]{a}{a'}$,
$\id[{\id[A]{a}{a'}}]{p}{q}$ is determined as well.  
For example:
\begin{itemize}

\item Paths $p = q$, where $p,q : \id[A \times B]{w}{w'}$, are equialent
...
\item Paths $p = q$, where $p,q : \id[\prd{x:A} B(x)]{f}{g}$, are equialent
  to homotopies $\prd{x:A} \id[B(x)] {\happly(p)(x)}{\happly(q)(x)}$.  
\end{itemize}

Next, we consider $\transfib{x \mapsto \id[B(x)]{f(x)}{g(x)}}{p}{q}$,
i.e. transport in a path fibration. This is given by pre- and
post-composition, analogously to transport at $\to$.  
...
\]
\end{cor}
I.e. transporting with ${x \mapsto \id{c}{x}}$ is post-compostion, and
transporting with ${x \mapsto \id{x}{c}}$ is contravariant
pre-composition.  These may be familiar as the functorial actions of
$Hom(c,-)$ and $Hom(-,c)$.
...
e.g. Section~\ref{sec:pi1-s1-intro}.  
