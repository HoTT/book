...
\begin{proof}
  ...
  \]
  However, $\sm{g:A\to A} (g = \idfunc[A])$ is contractible to $\idfunc[A]$, so this type is equivalent to $(\idfunc[A] = \idfunc[A])$.
  And by function extensionality, this is equivalent to $\prd{x:A}(x=x)$.
\end{proof}
...
\begin{lem}\label{lem:autohtpy}
  ...
  \item For all $p,q:a=a$ we have $p\ct q = q \ct p$.\label{item:autohtpy3}
  \end{enumerate}
...
\begin{proof}
  ...
  Now, we would like to define $f$ by assigning to each $x$ the path $\opp{g(x)} \ct q \ct g(x)$, but this does not work because $g(x)$ does not inhabit $a=x$ but rather $\brck{a=x}$, and the type $x=x$ may not be a mere proposition.
  Instead we define, for each $x:A$, the type
  \[ B(x) \defeq \sm{r:x=x} \prd{s:a=x} (r = \opp s \ct q\ct s) .\]
  We claim that $B(x)$ is always a mere proposition.
  Since this is itself a mere proposition, we may assume that $g(x) = \bproj p$ for some $p:a=x$.
...
\begin{lem}
  There is a function $f:A\to B$ such that $\qinv(f)$ is not a mere proposition.
\end{lem}
\begin{proof}
