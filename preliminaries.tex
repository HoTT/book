...
where $b(x)$ is a term of type $B(x)$ for $x:A$. So if $a$ is a term of type $A$ we may substitute $a$ for $x$ in the defining equation to give a definitional equality
  \[ f(a)\jdeq b(a):B(a)\]
and we have $f(a):B(a)$. We use the same syntax for substitution and function application hence this rule may look a bit tautological. Here $f(a)$ is function application whle $b(a)$ is the substitution of $a$ for $x$.
...
\[(\lam{x:A}b(x))(a) \jdeq b(a)\]
($\beta$-equality). Note that
...
Functions in several arguments correspond to iterated function types,
