...
The second kind of defined constant is used to specify a (parameterized) mapping $f(x_1,\dots,x_n,x)$ out of a type
whose elements are generated by zero or more primitive constants.  For each such primitive constant $c$ there is a defining equation of the form
\[
...
\indexdef{inference rule!of substitution}%
and
\define{weakening},
\indexdef{inference rule!of weakening}%
...
\index{term!normalizable}%
\index{normalization}%
(respectively, \define{strongly
normalizable})
\index{term!strongly normalizable}%
