...
\begin{eg}
  The type $\emptyt$ is a set, for given any $x,y:\emptyt$ we may deduce anything we like, by the recursion principle of $\emptyt$.
\end{eg}
...
On the other hand, the empty type \emptyt is also (vacuously) a mere proposition.
...
\indexdef{subtype}%
\symlabel{subset}
If $\setof{x:A | Q(x)}$ is another subset of $A$, then we say that $P$ is \define{contained}
\indexdef{containment!of subsets}%
in $Q$, and write $P\subseteq Q$, if $\prd{x:A}(P(x)\to Q(x))$.
...
But with propositional resizing, we can define the power set to be $A\to\Omega$, which is then independent of $\UU$.
...
\begin{defn}\label{defn:contractible}
  ...
  \indexdef{type!contractible}%
  \indexdef{contractible!type}%
  or a \define{singleton},
  \indexdef{type!singleton}%
  \indexdef{singleton type}%
...
This is the approach taken in logic enriched type theory~\cite{aczel2002collection} and in some presentations of the internal languages of toposes and related categories (e.g.~\cite{jacobs1999categorical,elephant}), as well as in the proof assistant \Coq.\index{proof assistant!Coq@\textsc{Coq}}
