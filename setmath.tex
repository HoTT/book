...
%Writing $\setof{ x : V | x \in v }$ for the \emph{$0$-type of elements} of any $v \in V$, and invoking univalence for mere propositions, we therefore have that a class $C : V \to \prop$ is a $V$-set just in case $$\exis{v \in V} C = \setof{ x : V | x \in v },$$ as expected.
...
\item Separation requires a definition.  Recall that $V$ lives in the same universe $\UU'$ as the base universe $\UU$ from which it is built. 
%Thus, a priori, the same is true for the identity types $v=_V w$ for any $v,w:V$.  
A class $C: V\to \prop$ will be called \define{$\UU$-small} if all of its values $C(x)$ lie in $\UU$. Let us say that a class $C$ is \define{separable} if for any $a:V$ the class $\setof{x | x\in a \wedge C(x)}$ is a $V$-set.

We now claim that if the class $C: V \to \prop$ is $\UU$-small, then it is separable.
