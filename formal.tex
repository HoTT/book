...
named constants, such as $\isequiv$, as a matter of convenience.  Informally, we
can think of these constants simply as abbreviations of their definitions, but
the situation is a bit more subtle than this. 
...
But from a practical perspective, we don't want to annotate each application of
$\circ$ with applications to $A,B,C$; we would simply like to write $g\circ f$.
Here, then, $\circ$ is not, strictly speaking, an abbreviation of its
definition: it has additional \define{implicit arguments} which we choose to
suppress.
...
In this section, we describe the features of homotopy type theory which are not
present in ordinary Martin-L\"{o}f type theory---function extensionality, the
univalence axiom, and higher inductive types. We will present these in the style
of \autoref{sec:syntax-more-formally}, though \autoref{sec:syntax-informally}
...
\begin{cor}
 For the system in \autoref{sec:syntax-informally}, provability is  decidable.
\end{cor}
