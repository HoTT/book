...
by the rules for the identity type.  Using the higher inductive types
...
The really nice thing about using HoTT to work
with $\infty$-groupoids is that you don't start by defining "an
$\infty$-groupoid is..." as one would in any other framework---which is
fortunate, because describing the structure of an $\infty$-groupoid in
something like set theory is notoriously difficult.    
...
Our proofs do imply the
classical statements about topological spaces: Homotopy type theory has
an interpretation in Kan simplicial sets, which are one model of
$\infty$-groupoids.  Interpreting our synthetic proofs in this model,
and then transfering them along geometric realization functor from
simplicial sets to topological spaces, gives proofs of the classical
statements.  Moreover, though this is work in progress, it seems likely
that we will be able to interpret type theory in a wide variety of other
categories that ``look like'' the category of $\infty$-groupoids
($(\infty,1)$-toposes), and, if so, proving a result in type theory will
show that it holds in these settings as well. 

It remains to be seen how
much homotopy theory we can do synthetically, but there are already some
positive indications, which are listed below.


Before presenting the technical results, we take a moment to review some
of the basic theorems of homotopy theory. Homotopy theory arose as a
branch of algebraic topology, and concerns the use of tools from
abstract algebra, such as group theory, to investigate properties of
spaces.  One question homotopy theorists investigate is how to tell
whether two spaces are the same, where ``the same'' means \emph{homotopy
  equivalence} (continuous maps back and forth that compose to the
identity up to homotopy---this gives a little wiggle room to ``correct''
maps that don't exactly compose to the identity.).  A central tool used
to do this is the \emph{algebraic invariants} associated with a space,
...
\begin{defn}\label{def-of-homotopy-groups}
  Given $n\ge 1$ and $(A,a)$ a pointed type, we define the \textbf{homotopy groups} of $A$
  at $a$ by
