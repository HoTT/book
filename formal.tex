...
We say that a term is {\em in normal form} if it cannot be reduced, and that a 
...
\and
  ...
  {\jdeqtp\Gm{(\lamu{x:A} b)(a)}{b[a/x]}{B[a/x]}}
\end{mathparpagebreakable}
...
\begin{mathparpagebreakable}
  ...
  {\oftp\Gm{f}{\tprd{x:A} B}}
  {\jdeqtp\Gm{f}{(\lamu{x:A}f(x))}{\tprd{x:A} B}}
\end{mathparpagebreakable}
...
There are two basic ways of introducing an axiom such as function
extensionality, or any other axiom which does not introduce new syntax or new
...
the axioms of homotopy type theory to be part of the core theory, always
present.

We begin by introducing some definitions that allow us to state the function
extensionality axiom and the univalence axiom.

Given $f:A \to B$ and $g : B \to C$, we let $g \circ f$ stand for the function $\lamu{x:A} g(f(x)$ of type $A \to C$.

Given $f:A \to B$ and $g:A \to B$, recall that $f \htpy g$ stands for $\prd{x:A} \id[B] {f(x)} {g(x)}$; see \autoref{defn:homotopy}.

The rule 
\begin{mathparpagebreakable}
  \inferrule*[]
  {\oftp \Gm f {A \to B} \\ \oftp \Gm x A  \\ \oftp \Gm y A }
  {\oftp \Gm {\apfunc f} {(\id[A] x y) \to (\id[B] {f(x)} {f(y)})}}
\end{mathparpagebreakable}
...
\begin{mathparpagebreakable}
  \inferrule*[]
  {\oftp \Gm A {\UU_i} \\ \oftp \Gm B {\UU_i} \\ \oftp \Gm f {A \to B}}
  {\oftp \Gm {\ishae(f)} {\UU_i}}
\end{mathparpagebreakable}
is derivable from the others if we let $\ishae(f)$ (see \autoref{defn:ishae}) serve as abbreviation for
\[
  \sm{g:B\to A}{\eta: g \circ f \sim \idfunc[A]}{\epsilon:f \circ g \sim \idfunc[B]} \prd{x:A} \apfunc{f}{(\eta x)} = \epsilon(fx)
\]
The type $\ishae(f)$ represents the proposition that the map $f : X \to Y$ is a
half-adjoint equivalence.  As in \autoref{sec:concluding-remarks}, we let
$\isequiv(f)$ serve as an abbreviation for $\ishae(f)$, and we say simply that
$f$ is an equivalence.
...
\begin{mathparpagebreakable}
  ...
  {\oftp\Gm{f}{\tprd{x:A} B} \\
   \oftp\Gm{g}{\tprd{x:A} B}}
  {\oftp\Gm{\isequiv\text{-}\happly}{\isequiv(\happly_{f,g})}}
\end{mathparpagebreakable}

The rule
\begin{mathparpagebreakable}
  \inferrule*[]
  {\oftp \Gm p {\id[\UU_i] A B}}
  {\oftp \Gm {\idtoeqv(\UU_i,A,B,p)} {\eqv A B}}
\end{mathparpagebreakable}
is derivable from the others if we let $\idtoeqv(\UU_i,A,B,p)$ (see \autoref{eq:uidtoeqv}) serve as an abbreviation for
\[
...
\]

In order to assert the univalence axiom (see \autoref{axiom:univalence}), we introduce a new
constant, ${\idtoeqv\text{-}\isequiv}$, and the following new family of
inference rules, one for each $i$.
\begin{mathparpagebreakable}
  \inferrule*[right=$\UU_i$-\textsc{univ}]
  {\oftp\Gm{A}{\UU_i} \\
   \oftp\Gm{B}{\UU_i}}
  {\oftp\Gm{\idtoeqv\text{-}\isequiv}{\isequiv(\lamu{p : \id[\UU_i] A B} \idtoeqv(\UU_i,A,B,p))}}
\end{mathparpagebreakable}
