...
We finish this section by a third construction of the quotient of a set $A$ by an equivalence relation $R$. We
consider the category with objects $A$ and hom-sets $R$. The Rezk completion (see \autoref{sec:rezk}) of this category is the quotient.
...
\begin{rmk}
  ...
  axioms for the \emph{Elementary Theory of the Category of
    Sets}~\cite{lawvere:etcs-long,Lawvere/Rosenburgh}.
\end{rmk}

\begin{proof}
  Since AC implies LEM, we have a Boolean elementary topos with choice by \autoref{thm:settopos}.  We leave the proof of well-pointedness as
...
The proof in~\cite{Mines/R/R:1988} uses the power set operation, although it can be made predicative by using typical ambiguous universes to
create the illussion of a large power object; see \autoref{subsec:prop-subsets}.
