...
\begin{proof}[First proof]
  ...
  \end{equation*}
  Thus, the induction principle for identity types gives us an element $J_{D,d}(x,y,p): (y= x)$ for each $p:(x= y)$.
  We can now define the desired function $\opp{(\blank)}$ to be $\lam{p} J_{D,d}(x,y,p)$, i.e.\ we set $\opp{p} \defeq J_{D,d}(x,y,p)$.
...
facts about $\mathsf{transport}$ proven in the preceeding sections to
