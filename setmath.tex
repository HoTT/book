...
\[ \left(X\to \prd{a:A} B(a) \right) \eqvsym \left(\prd{a:A} (X\to B)\right) .\]
...
equivalent to $\prd{z:A} R (x,z)\eqvsym R (y,z)$. Since $R $ is an equivalence relation, the latter space is equivalent to $R (x,y)$. To
...
&\big(\fall{a:A}\exis{b:B} f(a)  \bisim g(b)\big) \land \big(\fall{b:B}\exis{a:A} f(a) \bisim g(b)\big)
\end{align*}
\end{defn}
%
