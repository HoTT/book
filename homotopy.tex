...
\section{Freudenthal's Suspension Theorem}

Recall that if $\pairr{A,a_0}$ and $\pairr{B,b_0}$ are pointed types, then
the wedge $A\wedge B$ is defined to be the pushout of $A\stackrel{a_0}{\leftarrow}
\unit\stackrel{b_0}{\rightarrow} B$. 

\begin{lem}
Suppose that $\pairr{A,a_0}$ and $\pairr{B,b_0}$ are $n$- and $m$-connected
pointed types, respectively. Then the canonical map $A\wedge B\to A\times B$
is $(n+m+2)$-connected. \note{or $(n+m)$-connected?}
\end{lem}

\begin{thm}[Freudenthal's Suspension Theorem]
Suppose that $X$ is $n$-connected and pointed, with $n\geq 0$. Then the canonical
map $X\to \Omega\susp(X)$ is $2n$-connected.
\end{thm}
