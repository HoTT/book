...
Show that if we define $\ind{A+B}$ as $A + B \defeq \sm{x:\bool} \rec{\bool}(\UU,A,B,x)$, then the definitional equalities stated for $\ind{A+B}$ in \autoref{sec:coproduct-types} hold.
\end{ex}

\begin{ex}\label{ex:prod-via-bool}
Show that if we define $\ind{A\times B}$ as $A \times B \defeq \prd{x:\bool}\rec{\bool}(\UU,A,B,x)$, then the definitional equalities state for $\ind{A\times B}$ in \autoref{sec:finite-product-types} hold propositionally (i.e.\ using equality types).
\end{ex}
