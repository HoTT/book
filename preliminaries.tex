...
\]
\end{description}
This says that any construction $C$ represented by a family of types respects equality, in the sense that when $C$ is applied to equal elements of $A$, there is an associated  function between the resulting types. The defining equality states that the function associated to reflexivity is the identity function.
...
Formally, we write $C$ as family with three arguments, $x$ and $y$ of type $A$ and $p$ of type $\id[A]{x}{y}$.  However, such a family is equivalent (by currying) to a one-argument family, whose argument is an element of the path space ($\sm{x:A}{y:A} \id[A]{x}{y}$). Indeed, our notational convention of writing the application of the ternary family $C$ to three arguments as $C(x,y,p)$ suppresses the distinction.
...
However, this principle is false in homotopy type theory.  Indeed, it easily implies that any $p : \id{a}{a}$ is equal to $\refl{a}$, which contradicts univalence by \autoref{UA-implies-not-K}.  Thus, while path induction states that the path space is inductively generated (it is the least reflexive relation), and based path induction says that the based path space is inductively generated (it is the least predicate on $A$ that holds for $a$), there is no principle saying that the type of \emph{loops} is inductively generated.
