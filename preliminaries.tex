...
we call their \emph{cartesian product}. Its elements are
pairs $\tup{a}{b} : A \times B$ where $a:A$ and $b:B$. We also
introduce a nullary product type, called the \emph{unit type} $\unit
...
\[ f(x,y) \defeq g(x)(y). \] 
For example, in this way we can prove the principle of \emph{surjective pairing}, i.e.\ we can construct a function
...
we need here is to know that there is a reflexivity element $\refl{x} : x =_A x$ for any $x:A$.
