...
We can also compute the identity, inverses, and composition in $\Pi$-types:
\begin{align*}
...
Given a type $X$, a path $p:\id[X]{y}{z}$, dependent types $A,B:X\to \type$, and a function $f : A(y) \to B(y)$,  we have
\begin{align*}
  \transfib{A\to B}{p}{f} &=
  \; \Big(x \mapsto \transfib{B}{p}{f(\transfib{A}{\opp p}{x})}\Big)
\end{align*}
where $A\to B$ denotes abusively the dependent type $X\to \type$ defined by $(A\to B)(x) \defeq (A(x)\to B(x))$.

The dependent case is similar, but more complicated.
Suppose given $X$ and $p$ as before, a dependent type $A:X\to \type$, and a doubly dependent type $B:\prd{x:X} (A(x)\to\type)$, and suppose $f : \prd{a:A(y)} B(y,a)$.
Then
\begin{align*}
  \transfib{\Pi_A(B)}{p}{f} =
...
\end{array}
\]
All of this can be proven easily by path induction.
