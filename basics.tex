...
\begin{proof}
  ...
  \begin{align*}
    ...
    &&\text{(by \autoref{thm:transport-compose})}\\
    &= \refl{a_0} \ct c
    &&\text{(by \autoref{cor:transport-path-prepost})}\\
    &= c. && \qedhere
  \end{align*}
\end{proof}
...
\begin{defn}
Given a type $A$, the type \semigroupstr{A} of \define{semigroup structures} with carrier $A$ is defined by
\[
\semigroupstr{A} \defeq \sm{m:A \to A \to A} \prd{x,y,z:A} m(x,m(y,z)) = m(m(x,y),z).
\]
%
A \define{semigroup} is a type together with such a structure:
%
\[
...
= & (\transfib{X \mapsto (X \to X \to X)}{\ua(e)}{m}, 
   \transfibf{(X,m) \mapsto \mathsf{Assoc}(X,m)}{(\pairpath(\ua(e),\refl{}))}{a}) \label{eq:transport-semigroup-step1}
\end{align}
...
that preserves the multiplcation, and arrive at the conclusion that equality of
...
conclusion applies more generally: in HoTT, all constructions of
...
The definition of identity types and the elimination rule $J$ are due to Martin-L\"of \cite{Martin-Lof-1979,martin-lof:bibliopolis}.
...
The actual homotopical interpretation, with identity types as path spaces, and dependent types as fibrations, is due to \cite{AW}, who used the formalism of Quillen model categories.  An interpretation in (strict) $\infty$-groupoids was also given in the thesis \cite{mw:thesis}.
...
Operations such as $\transfib{P}{p}{-}$ and $\apfunc{f}$, and one good notion of equivalence, were first studied extensively in type theory 
by Voevodsky, using the proof assistant \Coq.
...
It has a homotopical aspect as well, related to the theory of classifying spaces in topology; see~\cite{klv:ssetmodel}.
...
\begin{ex}\label{ex:equality-reflection}
  ...
  Prove that for any $p:x=x$ we have $p\jdeq \refl{x}$, and hence every type is a set in the sense of \autoref{sec:basics-sets}.
\end{ex}
