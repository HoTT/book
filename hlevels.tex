...
\begin{rmk}
 The number $n$ in \autoref{def:hlevel} ranges over all integers greater than or equal to $-2$. One should note however that the induction principle for type $\Nat$ applies in this case. Indeed, we could alternatively defined a predicate $\istype{(k\mbox{-}2)}$ for $k : \Nat$. This justifies the correctness of the above definition and allows us to use the type-theoretic induction principle for $\Nat$ in the computation of $n$-types (for which we can take $n = -2$ as the base case).
