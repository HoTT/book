...
Given a type $A$, we define the \define{total identity type} of $A$ to be the type
\[
\sm{x:A}{y:A} \id[A]{x}{y}
\]
We call this the total identity type because, from a homotopical point of view, it is the total space of the identity type, viewed as a fibration over two copies of $A$.
We will also refer to it as the \define{two-sided identity type}, to refer to the fact that \emph{both} sides of the equation are existentially quantified.  The two-sided induction principle for the identity type then states that the total identity type is inductively generated by the elements of the form $(x,x,\refl{x})$:
...
The general form of the rule allows $C$ to depend on the witness $p:x=y$ as well.  In the premise, we can not only replace $y$ by $x$, but also simultaneously replace $p$ by reflexivity: to prove a property for all elements $(x,y,p)$ of the total identity type, it suffices to consider all the cases of the form $(x,x,\refl{x})$.  If we view types as like sets, it is unclear what this buys us, but in \autoref{cha:basics} we will see that it is essential to the homotopy interpretation, because it expresses that the total path space of a type is inductively generated by the constant loops at each point.  

If we package the two-sided induction principle into a function, it becomes:
...
This function is also called $J$.  In \cref{sec:fibrations}, we show that the recursor (indiscernability of identicals) follows from this induction principle.  

\subsection{One-sided induction}

Given a proof $p : \id{a}{b}$, the
two-sided induction principle allows you to equate $a$ and $b$ by replacing them \emph{both} with the same unknown element $x$: the induction principle says that to prove a family
...
Given a type $A$ with an element $a:A$, we define the \define{singleton   type} of $A$ at $a$ to be the type
\[
\sm{y:A} \id[A]{a}{y}
\]
We will also call this the \define{one-sided identity type} of $A$ at $a$, to refer to the fact that only one side of the equation is existentially quantified.  
Intuitively, this type is a ``singleton'' because it classifies elements of $A$ that are equal to $a$, and therefore, in a sense, contains only the element $a$.  However, it is very important to observe the difference between this singleton type and a singleton set $\{a\}$: A singleton set has exactly one element, namely $a$.  In contrast, the singleton type has elements $(b, p)$ where $p : \id[A] a b$.  Because of the homotopy interpretation of equality, the singleton type can have many different elements $b$ that are not strictly equal to $a$, but are only connected to it by a path.  Nonetheless, the singleton type behaves like a singleton up to homotopy, in the sense that any two elements of it are identified by a path, i.e., it is contractible.  This follows from the second induction principle for the identity type, which states that the singleton type of $A$ at $a$ is inductively generated by the element $(a,\refl{a})$.  
...
The one-sided induction principle says that we may prove a family $C$ for all elements of the singleton type at $a$ in $A$ by covering the case for $(a,\refl{a})$.
Logically, this says that given an equality $\id{a}{b}$, we can substitute the occurrences of $b$ chosen by $C$ with $a$.
Homotopically, it expresses the that the space of paths from a chosen point to an arbitrary endpoint is inductively generated by the constant loop.

Packaged as a function, the one-sided induction principle becomes:
...
\subsection{Zero-sided induction}

One might wonder whether there is a also a ``0-sided'' induction principle for the identity type, with no sides existentially quantified. Given a family
\[
C : \id{a}{b} \to \UU
\]
...
This principle is \textbf{not} provable in homotopy type theory.  Using it, one can easily prove that any $p : \id{a}{a}$ is equal to $\refl{a}$.  This and the univalence axiom create a contradiction, because univalence allows one to create provably unequal loops.

Thus, while the two-sided induction principle says that the total path type is inductively generated (it is the least reflexive relation), and the one-sided induction principle says that the singleton type is inductively generated (it is the least predicate on $A$ that holds for $a$), there is no principle saying that the type of \emph{loops} is inductively generated.  

\subsection{Equivalence of two-sided and one-sided induction}
...
Another proof is to observe that any use of one-sided induction is an instance of $\indidone{A}$, and to define
\begin{align*}
...
This was suggested originally by de Bruijn~\cite{debruijn}, Howard~\cite{howard}, Tait~\cite{tait}, and Martin-L\"{o}f~\cite{martin-lof}.
