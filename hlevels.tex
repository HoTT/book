...
\begin{lem}\label{lem:hlevel_if_inhab_hlevel}
 Given $n \geq 1$ and $X : \type$. If, given any inhabitant of $X$ it follows that $X$ is
   of h-level $n$, then $X$ is of h-level $n$.
\end{lem}
...
\begin{rmk}\label{rem:is_contr_implies_isaprop_and_element}
 The converse of \autoref{thm:hprop_inhab_contr} is also true; this follows from cumulativity of h-levels (\autoref{thm:hlevel_cumulative}).
\end{rmk}

We next turn towards giving some less obvious examples of h-propositions:

\begin{thm}\label{thm:isaprop_isofhlevel}
 For any $n : \Nat$ and any type $X$, the type $\isofhlevel(n)(X)$ is an h-proposition.
\end{thm}
...
From presevation of h-levels under products it follows that the type $\prod_{x' : X} \idtype[X]{\pi_1 (c')} {x'}$
...
\[\prod_{X : \Type} \ishProp (\isofhlevel(n)(X)) \to \prod_{X : \Type} \ishProp (\isofhlevel(n+1)(X)) \]
To show the conclusion of this implication, we need to show that for any type $X$, the type
    \[\prod_{x, x' : X}\isofhlevel(n)(x = x')\]
is an h-proposition. By \autoref{thm:hlevel_prod} it suffices to show that for any $x, x' : X$, the type $\isofhlevel(n)(x =_X x')$ is an h-proposition.
...
In this section we show that the type of types of h-level $n$ is itself of h-level $n+1$. Formally, we may define the type:
 \[\HLevel(n) \defeq \sum_{X : \type} \ \isofhlevel(n)(X) \]

In particular, we define $\prop \defeq \HLevel(1)$ and $\set \defeq \HLevel(2)$.
...
\begin{proof}[Proof of \autoref{thm:hleveln_of_hlevelSn}]
 ...
 is an inclusion (i.e.~its fibers are h-propositions). Since h-levels are preserved under $\prod$ and hence under the arrow type, this reduces to an assumption that $X'$ is of h-level $n$.
\end{proof} 