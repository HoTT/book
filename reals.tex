...
no division by zero because we cunningly added one to the natural denominator~$a$. Here
too we have a canonical choice of representatives, namely fractions in lowest terms. Thus
we may apply \autoref{lem:quotient-when-canonical-representatives} to obtain a set \Q,
which again has a decidable equality.
...
Let us just remember that there is an entirely unproblematic construction of the ordered
field of rational number \Q, with a decidable equality and decidable order. The field \Q
can also be characterized as the initial one.
...
representatives. Higher-inductive types are the perfect solution. (It would be possible to
perform the same construction in constructive set theory as well.)
...
\begin{proof}
  ...
  \item The second statement has four cases, one of which is easy and two are symmetric,
    ...
    %
    \begin{itemize}
    \item Suppose $\rclim(x) = \rclim(y)$ and $\inl(q) \sim_\epsilon \inr(\rclim(x))$. We
      need to prove that $\inl(q) \sim_\epsilon \inr(\rclim(y))$. Observe that there are
...
modulus of convergence. Then $\inl \circ q \circ M : \Qp \to \Q + \RC$ is a Cauchy
approximation and so $\rclim(\inl \circ q \circ m)$ is a real number. Thus the rationals
$\Q$ embed into $\RC$ via constant sequences, and various famous real numbers $\sqrt{2}$,
$\pi$, $e$, \dots as well because they are are all limits of Cauchy sequences of
rationals.


\textbf{TODO:} Discuss whether we got a set. Did we? Should we truncate?
