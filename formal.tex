...
We define these judgments inductively using inference rules---to conclude the
judgment below the horizontal line, it is necessary to prove the judgments above
the line. A valid derivation of a judgment is a tree constructed from these
...
In \autoref{cha:typetheory}, we introduced non-dependent functions $A\to B$ in
...
In \autoref{cha:typetheory}, we needed $\to$ and $\prdsym$ types in order to
...
\and
  ...
  {\oftp{\G,\tmtp x\unit}{C}{\UU_i} \\
   \oftp{\G,\tmtp y\unit}{c}{C[y/x]} \\
   \oftp\G{z}{\unit}}
  {\oftp\G{\ind{\unit}(x.C,y.c,z)}{C[y/x]}}
\and
  \inferrule*[right=$\unit$-\comp]
  {\oftp{\G,\tmtp x\unit}{C}{\UU_i} \\
   \oftp{\G,\tmtp y\unit}{c}{C[y/x]}}
  {\jdeqtp\G{\ind{\unit}(x.C,y.c,\ttt)}{c[\ttt/y]}{C[\ttt/x]}}
\end{mathparpagebreakable}
