...
including $\pi_1(\Sn ^1)$ (\autoref{sec:pi1-s1-intro}), $\pi_{k<n}(\Sn ^n)$
(\autoref{sec:pik-le-n}), $\pi_2(\Sn ^2)$ and $\pi_3(\Sn ^2)$ by way of the
Hopf fibration (\autoref{sec:hopf}), and $\pi_n(\Sn ^n)$ by way of the
Freudenthal suspension theorem (\autoref{sec:freudenthal}).  Next, we
discuss the van Kampen theorem (\autoref{sec:van-kampen}), which
characterizes the fundamental group of a pushout, and the status of
Whitehead's principle (when is a map that induces an equivalence on all
homotopy groups an equivalence?) (\autoref{sec:whitehead}).  Finally, we
...
\pi_{k+1}(\Sn^{n+1}) &\jdeq \trunc{0}{\Omega^{k+1}(\Sn^{n+1})} \notag \\
                     ...
                     &= \Omega^k(\trunc{k}{(\Omega(\susp(\Sn^{n})))}) \tag{by \autoref{thm:path-truncation}}\\
                     &= \Omega^k(\trunc{k}{\Sn^{n}}) \tag{by \eqref{eq:freudenthal-for-spheres}}\\
                     &= \trunc{0}{\Omega^k(\Sn^{n})} \tag{by \autoref{thm:path-truncation}}\\
...
\bottomrule
\end{tabular}
\caption{Theorems from homotopy theory that we have proved, checked by
...
\item 
Guillaume Brunerie gave the first proof of $\pi_{k<n}(\Sn ^ n)$ for the
suspension definition of the spheres. Dan Licata later gave an
encode-decode proof for the single-loop presentation of the spheres.

\item Peter Lumsdaine gave the first construction of the Hopf fibration.
  Guillaume Brunerie gave the proof that the total space of the Hopf
  fibration is $\Sn ^3$, and its corollaries $\pi_2(\Sn ^2)$ and
  $\pi_3(\Sn ^2)$.  Dan Licata later gave an encode-decode proof of
  $\pi_2(\Sn ^2)$, for the single-loop presentation of the spheres.
