...
\indid{A}
  \begin{aligned}[t]
    \big(
    &\big(\lamu{x,y:A}{p:\id[A]{x}{y}} \tprd{C : \prd{z:A}{p : \id[A]{x}{z}} \UU} C(x,\refl{x}) \to C(y,p)\big),\\
    &(\lamu{C:\prd{z:A}{p:\id[A]{x}{z}} \UU}\lamu{x:C(x,\refl{x})} x),
     a, x, p, C, c\big) 
   \end{aligned}
\end{narrowmultline*}
...
model $\indidb{A}$ with $C : \prd{x:A}{p : \id[A]{a}{x}} \UU_i$, we need
to use $\indid{A}$ with 
%
